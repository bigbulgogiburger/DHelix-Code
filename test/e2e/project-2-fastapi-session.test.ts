...
- Be concise. Create files directly.`,
  ...
  it("Turn 1: Create FastAPI project structure", async () => {
    await sendTurn(
      messages,
...
Create all files now.`,
  ...
  it("Turn 2: Create models and schemas", async () => {
    await sendTurn(
      messages,
      `Create:
...
2. schemas.py with Pydantic models:
  ...
  it("Turn 3: Create auth utilities and router", async () => {
    await sendTurn(
      messages,
      `Create:
...
Include main.py update to register the auth router.`,
  ...
  it("Turn 4: Create CRUD todo router", async () => {
    await sendTurn(
      messages,
...
Register this router in main.py. Make sure main.py creates tables on startup with Base.metadata.create_all.`,
  ...
  it("Turn 5: Create pytest tests", async () => {
    await sendTurn(
      messages,
...
Use a helper function to register+login and get the auth header.`,
  ...
  it("Turn 6: Install deps and run tests, fix failures", async () => {
    await sendTurn(
      messages,
      `In ${projectDir}:
...
Be sure to fix any import errors, missing modules, or logic bugs.`,
  ...
  it("Turn 7: Final verification and file count", async () => {
    await sendTurn(
      messages,
