...
```
$ dhelix
  Dhelix Code — AI coding assistant for local/external LLMs
...
dhelix는 Node.js 위에서 동작합니다. **버전 20 이상**이 필요합니다.
...
dhelix는 AI 모델과 통신해야 합니다. 두 가지 방법이 있습니다:
...
```bash
# 1. 저장소 복제
git clone https://github.com/bigbulgogiburger/dhelix.git
...
# 5. 전역 명령어로 등록 (어디서든 'dhelix' 입력 가능)
npm link
```

설치가 끝나면 어디서든 `dhelix`를 실행할 수 있습니다.

> `npm link`를 하지 않으면 `node bin/dhelix.mjs`로 직접 실행할 수도 있습니다.
...
설정은 `~/.dhelix/config.json`에 저장되므로, **다음 실행부터는 바로 시작**됩니다.
...
```bash
dhelix --api-key sk-proj-여기에키 --model gpt-4o
```
...
```bash
# 텍스트 출력
dhelix -p "package.json의 의존성 목록을 알려줘"

# JSON 출력
dhelix -p "src/ 폴더의 TypeScript 파일 개수" --output-format json

# 스트리밍 JSON
dhelix -p "이 코드를 리뷰해줘" --output-format stream-json
```
...
> `~/.dhelix/keybindings.json`에서 단축키를 변경할 수 있습니다.
...
| 옵션                     | 설명                             | 예시                                   |
| ------------------------ | -------------------------------- | -------------------------------------- |
| `-m, --model <모델>`     | 사용할 AI 모델 지정              | `dhelix -m gpt-4o`                     |
| `-u, --base-url <URL>`   | API 서버 주소                    | `dhelix -u http://localhost:11434/v1`  |
| `-k, --api-key <키>`     | API 키 직접 전달                 | `dhelix -k sk-proj-...`                |
| `-v, --verbose`          | 상세 로그 출력                   | `dhelix -v`                            |
| `-c, --continue`         | 마지막 대화 이어하기             | `dhelix -c`                            |
| `-r, --resume <ID>`      | 특정 세션 복원                   | `dhelix -r abc123`                     |
| `-p, --print <프롬프트>` | 헤드리스 모드 (UI 없이)          | `dhelix -p "설명해줘"`                 |
| `--output-format <형식>` | 출력 형식: text/json/stream-json | `dhelix -p "..." --output-format json` |
| `--add-dir <경로>`       | 추가 디렉토리 포함 (모노레포용)  | `dhelix --add-dir ../shared`           |
| `--version`              | 버전 출력                        | `dhelix --version`                     |
| `--help`                 | 도움말 출력                      | `dhelix --help`                        |
...
```bash
dhelix --base-url http://localhost:11434/v1 --model qwen3:8b
```
...
```bash
# .env
DHELIX_BASE_URL=http://localhost:11434/v1
DHELIX_MODEL=qwen3:8b
```

이후 그냥 `dhelix`만 실행하면 됩니다.

---

## 내 프로젝트에 맞게 설정하기

### DHELIX.md — AI에게 프로젝트 규칙 알려주기

프로젝트 루트에 `DHELIX.md`를 만들면 AI가 프로젝트를 더 잘 이해합니다:
...
dhelix는 여러 곳에서 설정을 읽고, 아래로 갈수록 우선합니다:

```
1. 내장 기본값
2. ~/.dhelix/config.json       ← 사용자 전역 설정
3. .dhelix/settings.json       ← 프로젝트 설정
4. .env 환경변수                ← DHELIX_*, OPENAI_*
...
| `DHELIX_API_KEY`  | dhelix 전용 키 (OPENAI_API_KEY보다 우선) | `sk-proj-...`               |
| `DHELIX_MODEL`    | 기본 모델                                | `gpt-4o`                    |
| `DHELIX_BASE_URL` | API 서버 주소                            | `http://localhost:11434/v1` |
| `DHELIX_VERBOSE`  | `true`로 설정 시 상세 로그               | `true`                      |
...
`.dhelix/skills/my-skill/SKILL.md`:
...
네! 아무 프로젝트 폴더에서 `dhelix`를 실행하면 됩니다. 프로젝트를 수정하지 않아도 동작합니다. `DHELIX.md`를 추가하면 더 좋은 결과를 얻을 수 있습니다.
...
1. 모델 이름이 정확한가요?
   ```bash
   # 자주 쓰이는 모델 이름
   dhelix -m gpt-4o-mini     # OpenAI (저렴)
   dhelix -m gpt-4o          # OpenAI (고성능)
   ```
