# dhelix vs Claude Code — v6 종합 비교 분석

> **분석일**: 2026-03-11
> **이전 분석**: v5 (2026-03-11, 9.3/10)
> **Claude Code 기준 버전**: v2.1.72 (2026-03-11 기준)
> **dhelix 기준**: main branch (178 소스 파일, 156,724줄 / 127 테스트, 28,761줄)
> **분석자 관점**: Anthropic Claude Code 핵심 개발자로서의 기술적 평가
> **리서치 방법**: Claude Code 공식 문서 전수 분석 + dhelix 코드베이스 전체 정독
...
dhelix의 `spawnSubagent()`는 같은 Node.js 프로세스 내에서 별도 agent loop을 실행하는 것이므로
아키텍처적으로 근본적인 차이가 있다.
...
| **Path-specific Rules**   | ✅ `.claude/rules/*.md` + `paths:` frontmatter | ⚠️ `.dhelix/rules/*.md` (paths 미지원) |
...
| 2   | **서브에이전트 정의 파일** (`.dhelix/agents/*.md`) | ★★★★★  | ★★★    | 멀티에이전트 +1.0  | 없음   |
| 3   | **@import 구문** (DHELIX.md 내)                    | ★★★★   | ★★     | 프로젝트 지침 +0.3 | 없음   |
...
1. **프로세스 분리 vs. 인프로세스**: Claude Code는 프로세스 수준 분리. dhelix는
   Node.js 단일 프로세스이므로 **Worker Thread** 기반 분리가 현실적.
2. **Shared Task List**: 파일 기반 (`~/.dhelix/teams/{name}/tasks/`) + 파일 락킹.
...
1. **Agent Teams는 "더 많은 서브에이전트"가 아니다**: Claude Code의 Agent Teams와 Subagents는
   ...
   Agent Teams의 각 teammate는 완전히 독립된 세션이다. dhelix에서 이를 구현하려면
...
2. **서브에이전트 정의 파일이 "숨은 핵심"**: Claude Code가 서브에이전트를 `.claude/agents/*.md`
   ...
   메모리, 훅, MCP 서버를 모두 설정할 수 있다. dhelix의 하드코딩된 `agent-types.ts`로는
...
3. **MCP가 "도구 브릿지"를 넘어 "생태계 플랫폼"**: Claude Code의 MCP는 단순히 외부 도구를
   ...
   Plugin 번들링까지 포함하는 **풀 플랫폼**이다. dhelix의 stdio-only MCP는 이 생태계의
   10% 정도만 활용하고 있다.
...
> dhelix v6은 코드 변경 없이 Claude Code v2.1.72를 정밀 재분석하여, **MCP 생태계(5.5/10)**,
> **Agent Teams 아키텍처 부재**, **서브에이전트 선언적 정의/메모리/훅 미지원**이라는
> 세 가지 핵심 격차를 가시화했다. 이는 Tier 1→2→3 단계별 구현의 설계 청사진이 된다.

### dhelix의 차별화된 강점

Claude Code에는 없는 dhelix만의 강점도 재확인:
...
- v5 분석 문서: `docs/dhelix-vs-claude-code-v5.md`
...
- dhelix 소스 코드 분석: 178 소스 파일 (156,724줄), 127 테스트 파일 (28,761줄)
