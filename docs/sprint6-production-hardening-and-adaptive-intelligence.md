...
| #   | 트렌드                                                                 | 출처                         | 영향                                | dhelix 적용                          |
| --- | ---------------------------------------------------------------------- | ---------------------------- | ----------------------------------- | ------------------------------------ |
| 1   | **Dynamic Context Discovery** — 도구 이름만 로드, 상세는 사용 시 fetch | Cursor 블로그                | **토큰 46.9% 감소** (프로덕션 검증) | → A6: Lazy Tool Loading              |
| 2   | **Observation Masking** — 도구 출력 strip, 추론 히스토리 유지          | JetBrains Research (2025.12) | SE 에이전트에 최적 compaction       | → B4: Observation Masking Compaction |
| 3   | **Few-Shot Tool Examples** — 2-5개 예시로 실패→완벽 성공               | DataCamp, Agenta.ai          | LOW/MEDIUM 모델 성공률 극대화       | → A1 강화: few-shot 포함             |
| 4   | **Constrained Decoding** — 생성 시점에서 유효하지 않은 토큰 마스킹     | vLLM 0.8.5, Outlines         | 소형 모델의 JSON 출력 100% 보장     | → A7: Structured Output Enforcement  |
| 5   | **Ralph Loop Circuit Breaker** — 무한 루프 방지, 3회 무변경 시 중단    | Geoffrey Huntley             | 토큰 낭비 방지, 장시간 세션 안정    | → B5: Agent Loop Circuit Breaker     |
| 6   | **Generator-Critic Pattern** — 생성 에이전트 + 검증 에이전트 분리      | AWS, Google ADK              | 코드 품질 향상                      | → B6: Built-in Code Review Agent     |
| 7   | **npm OIDC Provenance** — 토큰 없는 trusted publishing                 | Snyk, npm                    | 99% OSS 악성코드가 npm              | → C6: OIDC Publish                   |
| 8   | **SLM Fine-Tuning** — 1B 모델 tool calling 10%→79% (15분 학습)         | DataEnthusiast.io            | 소형 모델 비용 70-80% 절감          | → Post v1.0: 커스텀 SLM              |
| 9   | **Compaction vs Summarization** — 환경에서 재읽기 가능한 정보만 제거   | MorphLLM                     | 컨텍스트 품질 유지                  | → B4에 반영                          |
| 10  | **Micro-MCP Pattern** — 단일 목적 MCP 서버 게이트웨이 조합             | GitHub MicroMCP              | 보안 격리 + 독립 배포               | → Post v1.0: MCP 게이트웨이          |
    ...
    if (this.state.hasError) {
      return (
        <Box flexDirection="column" padding={1}>
          ...
          <Text color="yellow">Press Ctrl+C to exit, or restart dhelix.</Text>
        </Box>
      );
    }
...
### A6. Lazy Tool Loading (Cursor Dynamic Context Discovery 패턴)

**배경**: Cursor는 도구 이름만 시스템 프롬프트에 포함하고, 도구를 실제 사용할 때 상세 스키마를 fetch합니다. 이 패턴으로 **전체 에이전트 토큰 46.9% 감소**를 달성했습니다 (프로덕션 검증).

```typescript
// src/tools/lazy-tool-loader.ts (신규)

/**
 * Cursor의 Dynamic Context Discovery 패턴을 적용합니다.
 *
 * 1단계: 시스템 프롬프트에 도구 이름 + 1줄 설명만 포함
 * 2단계: 모델이 도구를 선택하면, 전체 스키마를 동적으로 로드
 * 3단계: 도구 출력을 파일로 변환하여 컨텍스트 재사용 가능
 *
 * HIGH tier: 전체 스키마 포함 (기존 동작 유지)
 * MEDIUM tier: 상위 10개 도구만 전체 스키마, 나머지 이름만
 * LOW tier: 모든 도구 이름만, 사용 시 스키마 fetch
 */
export class LazyToolLoader {
  private readonly fullSchemaCache = new Map<string, ToolSchema>();

  getToolSummaries(tools: readonly ToolDefinition[], tier: CapabilityTier): readonly ToolSummary[] {
    if (tier === "high") {
      // HIGH: 전체 스키마 반환 (기존 동작)
      return tools.map((t) => ({ ...t, schemaLoaded: true }));
    }

    // MEDIUM/LOW: 이름 + 1줄 설명만
    return tools.map((t) => ({
      name: t.name,
  ...
  async loadFullSchema(toolName: string): Promise<ToolSchema | null> {
    if (this.fullSchemaCache.has(toolName)) {
      return this.fullSchemaCache.get(toolName)!;
    }
    const tool = this.registry.get(toolName);
    if (tool) {
      this.fullSchemaCache.set(toolName, tool.schema);
      return tool.schema;
    }
    return null;
  }
...
**효과**: MEDIUM/LOW tier에서 시스템 프롬프트 토큰 **40-50% 절감** 예상.

| 파일                                | 변경                           | 규모  |
| ----------------------------------- | ------------------------------ | ----- |
| `src/tools/lazy-tool-loader.ts`     | **신규**                       | ~80줄 |
| `src/core/system-prompt-builder.ts` | lazy summary 통합              | ~15줄 |
| `src/core/agent-loop.ts`            | 도구 선택 시 full schema fetch | ~10줄 |
| 테스트                              | 신규                           | ~40줄 |

---

### A7. Structured Output Enforcement (소형 모델 JSON 보장)

**배경**: vLLM 0.8.5와 Ollama가 constrained decoding을 지원합니다. 소형 모델에서 도구 호출 JSON이 100% 유효하도록 보장합니다.

```typescript
// src/llm/structured-output.ts (신규)

/**
 * 소형 모델의 도구 호출 시 JSON Schema 기반 출력 강제.
 *
 * Ollama: response_format: { type: "json_object", schema: toolSchema }
 * vLLM: guided_json 파라미터
...
export function buildStructuredOutputConfig(
  ...
  // HIGH tier는 native function calling 사용 (강제 불필요)
  if (tier === "high") return null;

  switch (provider) {
    case "ollama":
      return { format: "json", response_format: { type: "json_object" } };
    case "vllm":
      return { guided_json: JSON.stringify(toolSchema) };
    default:
...
| 파일                                     | 변경                   | 규모  |
| ---------------------------------------- | ---------------------- | ----- |
| `src/llm/structured-output.ts`           | **신규**               | ~60줄 |
...
**목표**: Sprint 5의 recovery-strategy.ts를 **실제 실행**하는 자가 복구 루프 + Circuit Breaker + Observation Masking 구현.
...
### B4. Observation Masking Compaction (JetBrains Research 적용)

**배경**: JetBrains Research (2025.12)에 따르면 SE 에이전트에서 **Observation Masking** (도구 출력 strip, 추론 히스토리 유지)이 가장 효과적인 compaction 전략입니다. 도구 출력은 환경에서 다시 읽을 수 있으므로 제거해도 무손실입니다.

```typescript
// src/core/observation-masking.ts (신규)

/**
 * JetBrains Research 기반 Observation Masking.
 *
 * 원칙: "정보가 환경에서 재읽기 가능하면 히스토리에서 제거해도 무손실"
 *
 * 제거 대상 (tool role messages):
 * - file_read 결과 (파일을 다시 읽을 수 있음)
 * - grep_search 결과 (다시 검색 가능)
 * - glob_search 결과 (다시 검색 가능)
 * - bash_exec 출력 (명령을 다시 실행 가능, 단 부수효과 주의)
 *
 * 유지 대상:
 * - assistant reasoning (추론 과정)
 * - user messages (사용자 의도)
 * - tool calls (어떤 도구를 호출했는지)
 * - file_edit/write 결과 (부수효과 기록)
 */
export function applyObservationMasking(
  messages: readonly ChatMessage[],
  options?: { readonly keepRecentN?: number },
): ChatMessage[] {
  const keepRecent = options?.keepRecentN ?? 5; // 최근 5개 메시지는 유지
  const total = messages.length;

  return messages.map((msg, i) => {
    // 최근 N개는 무조건 유지
    if (i >= total - keepRecent) return msg;

    // tool role의 읽기 전용 출력만 마스킹
    if (msg.role === "tool" && isReadOnlyToolOutput(msg)) {
      return {
        ...msg,
        content: `[Output masked — re-read if needed. Original: ${getOutputSize(msg)} chars]`,
      };
    }
...
**효과**: 장시간 세션에서 컨텍스트 사용량 **50-70% 감소** (추론 품질 유지).

| 파일                              | 변경         | 규모  |
| --------------------------------- | ------------ | ----- |
| `src/core/observation-masking.ts` | **신규**     | ~80줄 |
| `src/core/context-manager.ts`     | masking 통합 | ~10줄 |
| 테스트                            | 신규         | ~40줄 |

---

### B5. Agent Loop Circuit Breaker (Ralph Loop 패턴)

**배경**: Geoffrey Huntley의 Ralph Loop 패턴은 무한 반복을 방지합니다. 에이전트가 진전 없이 반복할 때 자동으로 중단합니다.

```typescript
// src/core/circuit-breaker.ts (신규)

/**
 * Agent Loop Circuit Breaker.
 *
 * 중단 조건:
 * 1. 3회 연속 파일 변경 없음 (no progress)
 * 2. 5회 연속 같은 에러 반복 (stuck in error loop)
 * 3. 출력 품질 70%+ 감소 (regression detected)
 * 4. 총 반복 횟수 > maxIterations (hard limit)
 *
 * 상태: CLOSED (정상) → OPEN (중단) → HALF_OPEN (재시도)
 * cooldown: 30초 후 HALF_OPEN으로 전환
 */
export class CircuitBreaker {
  ...
  private lastFileSnapshot: Set<string> = new Set();

  recordIteration(result: IterationResult): void {
    // 파일 변경 감지
    if (
      result.filesModified.size === 0 &&
      this.lastFileSnapshot.size === result.filesModified.size
    ) {
      this.noProgressCount++;
    } else {
      this.noProgressCount = 0;
    }
    this.lastFileSnapshot = result.filesModified;
    ...
    // Circuit breaker 판단
    if (this.noProgressCount >= 3 || this.repeatedErrorCount >= 5) {
      this.state = "open";
    }
  }
  ...
  getStatus(): { readonly state: string; readonly reason?: string } {
    if (this.state === "open") {
      if (this.noProgressCount >= 3)
        return { state: "open", reason: "No file changes for 3 iterations" };
      if (this.repeatedErrorCount >= 5)
        return { state: "open", reason: "Same error repeated 5 times" };
    }
...
| 파일                          | 변경                 | 규모  |
| ----------------------------- | -------------------- | ----- |
| `src/core/circuit-breaker.ts` | **신규**             | ~80줄 |
| `src/core/agent-loop.ts`      | circuit breaker 통합 | ~15줄 |
| 테스트                        | 신규                 | ~50줄 |

---

### B6. Built-in Code Review Agent (Generator-Critic 패턴)

**배경**: GitHub에서 AI 코드 리뷰가 5건 중 1건을 차지합니다 (6천만 건+, 1년간 10배 성장). Generator-Critic 패턴으로 코드 생성과 검증을 분리합니다.

```typescript
// src/core/code-review-agent.ts (신규)

/**
 * 내장 코드 리뷰 에이전트.
 * Dual-Model Router의 "review" phase에서 자동 활성화됩니다.
 *
 * 리뷰 관점:
 * 1. 보안 (시크릿 노출, 인젝션, XSS)
 * 2. 정확성 (로직 오류, 엣지 케이스)
 ...
 * 4. 성능 (불필요한 반복, 메모리 누수)
 */
export async function reviewChanges(diff: string, config: ReviewConfig): Promise<ReviewResult> {
  const reviewPrompt = buildReviewPrompt(diff, config.focusAreas);

  // Dual-Model: architect 모델로 리뷰 실행
  const review = await config.client.chat({
...
| 파일                            | 변경                  | 규모   |
| ------------------------------- | --------------------- | ------ |
| `src/core/code-review-agent.ts` | **신규**              | ~100줄 |
| `src/commands/review.ts`        | `/review` 커맨드 연동 | ~20줄  |
| 테스트                          | 신규                  | ~40줄  |
 ...
 * 파일: ~/.dhelix/audit.jsonl (append-only)
 */
...
const PROMPT_INJECTION =
  /(?:ignore.*(?:previous|above|prior)|you are now|new instructions?|system prompt)/i;
```
...
export async function checkForUpdates(currentVersion: string): Promise<UpdateInfo | null> {
  ...
  const latest = await fetchLatestVersion("dhelix");
  await setLastCheckTimestamp(Date.now());

  if (latest && semver.gt(latest, currentVersion)) {
    return { current: currentVersion, latest, updateCommand: "npm update -g dhelix" };
  }
  return null;
}
```
...
Agent 2: "adaptive-schema"    → A1 Adaptive Tool Schema + A5 Adaptive Context + A6 Lazy Tool Loading
Agent 3: "prompt-cache"       → A2 System Prompt Cache + A3 Token Counter + A4 Cache Monitor
Agent 4: "structured-output"  → A7 Structured Output Enforcement
Agent 5: "self-healing"       → B1 Recovery Executor + B2 Tool Retry + B3 Adaptive GC
Agent 6: "smart-loop"         → B4 Observation Masking + B5 Circuit Breaker + B6 Code Review Agent
Agent 7: "security-harden"    → C1 Entropy Scanner + C2 Audit Log + C3 Key Masking
Agent 8: "injection-sandbox"  → C4 Injection Extension + C5 Sandbox Warning
Agent 9: "dx-analytics"       → D1 Analytics + D2 Doctor + D3 Coverage + D4 Update Checker
```
...
- [ ] Audit Log: 권한 결정이 `~/.dhelix/audit.jsonl`에 기록됨
