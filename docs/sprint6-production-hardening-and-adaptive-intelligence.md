...
export class CircuitBreaker {
  ...
  recordIteration(result: IterationResult): void {
    // 파일 변경 감지
    if (result.filesModified.size === 0 && this.lastFileSnapshot.size === result.filesModified.size) {
  ...
  getStatus(): { readonly state: string; readonly reason?: string } {
    if (this.state === "open") {
      if (this.noProgressCount >= 3) return { state: "open", reason: "No file changes for 3 iterations" };
      if (this.repeatedErrorCount >= 5) return { state: "open", reason: "Same error repeated 5 times" };
    }
