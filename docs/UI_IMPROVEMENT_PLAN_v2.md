# dbcode Improvement Plan v2 — Full-Stack Production Roadmap
...
8명의 전문가 에이전트가 dbcode 코드베이스(27,287줄, 22개 모듈)를 병렬로 심층 분석했습니다.
...
**Proposed**: `~/.dbcode/models.json` for user-defined overrides. Fall back to built-in registry.
...
**Fix**: 4th option persisting to `.dbcode/permissions.json`.
...
**Proposed**: `PermissionAuditLog` recording all check results to `~/.dbcode/audit.jsonl`.
...
**Proposed**: Expose dbcode as ACP-compatible agent for IDE integration.
...
- **Effort**: 2-3 days | **Impact**: MEDIUM
- **Files**: New `.dbcode/agents/` defaults
