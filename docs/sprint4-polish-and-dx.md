...
**dhelix 현재 상태**: `src/commands/doctor.ts` — 65줄, **4개 체크만** (Node, Git, Git repo, Model config). Claude Code 대비 큰 갭.
...
dhelix doctor
  ...
  ✓ Config: ~/.dhelix/config.json valid
...
```
/export [path]
→ Exported session to ./dhelix-session-2026-03-14.md
```
