...
export async function postprocess(
  ...
  if (result.output.length > config.maxOutputLength) {
    ...
    return {
      output: truncated + (spilloverPath
        ...
        if (!classified.retryable || attempt >= config.maxRetries) {
          ...
          if (classified.correctable) {
            const correction = await this.retrier.attemptCorrection(
              tool.name, correctedArgs, error as Error, context.workingDirectory
            );
            if (correction) {
...
export function emitToolStream(
...
const batchFileOpsSchema = z.object({
  ...
  ])).min(1).max(20),    // 최소 1개, 최대 20개 동시 처리
...
const applyPatchSchema = z.object({
  ...
  dry_run: z.boolean().default(false).describe(
    'If true, returns what would change without modifying files.'
  ),
  fuzz_factor: z.number().min(0).max(3).default(0).describe(
    'Number of context lines that can mismatch (0 = exact match required).'
  ),
});
```
...
const gitOpsSchema = z.object({
  operation: z.discriminatedUnion('type', [
    ...
    z.object({ type: z.literal('diff'), ref: z.string().optional(), staged: z.boolean().optional() }),
    z.object({ type: z.literal('log'), count: z.number().default(10), format: z.enum(['oneline', 'full']).default('oneline') }),
    z.object({ type: z.literal('blame'), file_path: z.string(), start_line: z.number().optional(), end_line: z.number().optional() }),
    z.object({ type: z.literal('show'), ref: z.string() }),
    z.object({ type: z.literal('branch'), list: z.boolean().default(true) }),
    z.object({ type: z.literal('stash'), action: z.enum(['list', 'push', 'pop', 'apply']).default('list') }),
    z.object({ type: z.literal('commit'), message: z.string(), files: z.array(z.string()).optional() }),
...
const packageManagerSchema = z.object({
  operation: z.discriminatedUnion('type', [
    ...
    z.object({ type: z.literal('install'), packages: z.array(z.string()), dev: z.boolean().default(false) }),
...
export function jsonToZod(jsonSchema: Record<string, unknown>): z.ZodSchema {
  ...
  switch (type) {
    case 'string': return buildStringSchema(jsonSchema);
    case 'number':
    case 'integer': return buildNumberSchema(jsonSchema);
    case 'boolean': return z.boolean();
    case 'null': return z.null();
    case 'object': return buildObjectSchema(jsonSchema);
    case 'array': return buildArraySchema(jsonSchema);
    default:
      // oneOf/anyOf 처리
...
*End of document. Last updated: 2026-04-04*
