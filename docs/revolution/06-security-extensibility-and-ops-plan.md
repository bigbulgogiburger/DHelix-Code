...
const COMMAND_DICTIONARY: ReadonlyMap<string, CommandArity> = new Map([
  ['git', {
    command: 'git',
    subcommands: ['status', 'diff', 'log', 'add', 'commit', 'push', 'pull', 'checkout', 'reset', 'rebase', 'branch'],
...
interface PolicyProvenance {
  ...
  readonly source: 'builtin' | 'user-policy' | 'project-policy' | 'admin-policy' | 'persistent-approval';
...
const resolveTier = (source: ExtensionSource): TrustTier => {
  ...
  if (source.type === 'mcp' && source.scope === 'project')
    return TrustTier.ProjectShared;
