...
> Claude Code의 핵심 개발자 경험(DX)을 정의하는 기능이며, dbcode와의 체감 품질 차이의 70%를 차지한다.

### 0.1 문제 정의

Claude Code는 매 도구 호출마다 **작업의 의미(what)를 헤더에 명시**하고, **결과를 구조화된 트리 형태**로 표시한다.
dbcode는 현재 `[✓] Edited src/foo.ts — Added 3 lines` 같은 **플랫 텍스트**만 표시한다.
...
| 요소 | Claude Code | dbcode 현재 | 격차 수준 |
|------|-------------|-------------|-----------|
...
| 도구 | 현재 dbcode | 목표 (Claude Code 스타일) |
|------|-------------|--------------------------|
...
현재 dbcode의 `ToolCallBlock`에는 `isExpanded` prop이 이미 존재하지만, **키보드 단축키와 연결되지 않았다**.
...
2. **작업 투명성**: Claude Code는 "지금 무엇을 하고 있는지"가 헤더만 봐도 즉시 파악된다.
   dbcode는 상태 아이콘(`[✓]`)과 동사(`Edited`, `Read`)를 읽어야 한다.
