...
```typescript
// 세션 종료 시 승인 내역을 ~/.dbcode/session-approvals.json에 저장
// 다음 세션 시작 시 로드하여 복원
export async function saveSessionApprovals(approvals: Map<string, boolean>): Promise<void> {
  const path = join(homedir(), ".dbcode", "session-approvals.json");
...
Sprint 3 완료 시 dbcode vs Claude Code 격차:
...
**Sprint 3 완료 시 dbcode_develop_v2.md의 Critical + Important 격차가 100% 해소됩니다.**
...
- dbcode에서는 **프롬프트 + 권한 시스템 이중 보호**: 시스템 프롬프트에 read-only 지시 + 권한 모드 `plan`에서 `confirm`/`dangerous` 도구 차단
- Claude Code처럼 `.dbcode/plans/` 디렉토리에 계획 파일 저장 가능 (선택)
...
**결정: Claude Code 패턴 — `.dbcode/worktrees/<name>`**

- `.gitignore`에 `.dbcode/worktrees/` 추가 권장
- 브랜치명: `dbcode-worktree-<name>`
