...
| 4   | `/init` 명령 시 DBCODE.md가 프로젝트 루트에 생성되지 않음          | **HIGH**     |
...
LLM hallucination — dbcode 버그 아님.
...
## Issue 4: `/init` 명령 시 DBCODE.md 미생성

### 근본 원인

**Issue 1과 동일한 원인.** `/init`은 LLM에 분석 프롬프트를 주입하여
코드베이스를 탐색한 후 `file_write`로 DBCODE.md를 생성하게 합니다.

로그에서 LLM이 Read/List 도구만 반복 실행하고 `file_write`를 호출하지 못한 것은,
**4096 토큰 제한** 때문에 긴 DBCODE.md 내용을 담은 `file_write` tool call을
완성하지 못했기 때문입니다.
