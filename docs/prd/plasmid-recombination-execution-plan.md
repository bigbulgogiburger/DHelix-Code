...
- [x] Phase 0 self-dogfood 결과 → `.dhelix/research/phase-0-results/` 에 커밋 완료 (`b6028df`, 2026-04-23)
- [x] 4 Adjustments 를 Phase 1 W3/W4/W5 task 목록에 등록 (§5.2 table 반영 완료)

### 5.2 Phase 1 Week-by-Week (5주, Phase 0 Adjustments 반영)

| 주 | Focus | 주요 산출물 | 연관 ADJ | 참조 설계서 |
|---|------|----------|---------|----------|
| W1 | Plasmid schema + loader + eval-seeds schema | Zod `plasmidCoreSchema` + `plasmidExtensionsSchema` + `plasmidEvalSeedSchema` (`src/plasmids/schema/*`), frontmatter parser, loader | — | P-1.1, P-1.23 |
| W2 | I-8 Hermeticity 3층 방어 + ModelCapabilities | `RUNTIME_BLOCKED_PATTERNS` (instruction-loader + tool guardrail + telemetry), `ModelCapabilities` 5 신규 필드 + Ollama probe | — | P-1.14 v0.2, P-1.18 v0.2 |
| W3 | `/plasmid` 서브커맨드 + Template 3-layer + **industry patterns** | 11 서브커맨드, `primitives/` 6개, `patterns/` 기본 3개 + **`foundational-legal`, `foundational-security`, `team-governance`** | **ADJ-3** | P-1.5 v0.2, P-1.4 v0.2 |
| W4 | `/plasmid` Quick mode + **시간 UX 재표현** + Fail-to-draft | 3-step quick flow, Quick 시간 표기 "20초 draft 생성 + 사용자 편집 5-15분" UX 적용, recovery 7 error → strategy 매핑 | **ADJ-1** | P-1.5 v0.2, P-1.7 v0.2 |
| W5 | Entry Gate 4종 검증 + **Alpha Gate 시작** | Cloud/Local/Hermeticity/Alpha 4 gate 통과, **3-5명 external user 섭외 + POC 실행** (Phase 0 delta 분석) | **ADJ-4** | go-no-go §Week 5 |

**Note**: ADJ-2 (L4 auto-gen priority 역전) 은 Phase 1 scope 아님 — Phase 2 W5 validation framework 구현 시점에 P-1.16 §Q2 반영. Phase 1 에서는 eval-seeds schema (P-1.23) 에 L3 auto-gen priority 룰 주석 추가만.

### 5.3 Phase 1 Exit Gate (Phase 2 진입) — **4 Gate · v0.4 (Alpha split + Local LLM 일반화)**

**Cloud Gate**:
- [ ] 10 plasmid recombination (Phase 1은 Validation 없이 Compile까지만) < 3분
- [ ] Static validation (stage 5) pass ≥90%

**Local Gate (user-controlled LLM, v0.4 일반화 — sub-tier 1+ 통과 시 PASS)**:

*Sub-tier A — strict-local* (Ollama llama3.1:8b 최소, fully offline):
- [ ] 동일 시나리오 < 10분
- [ ] Static validation pass ≥85%
- [ ] **localhost 외 일체 network traffic 0** (cascade OFF, tcpdump 검증)

*Sub-tier B — self-hosted* (예: 사내 GLM45AirFP8 / on-prem inference server, primary for v0.4):
- [ ] 동일 시나리오 < 5분 (reasoning model overhead 포함)
- [ ] Static validation pass ≥85%
- [ ] **외부 cloud provider 향 traffic 0** (cascade OFF, tcpdump — 사내망 endpoint traffic 은 허용)
- [ ] WebFetch / WebSearch 가 self-hosted endpoint 외 호출 안 함

**Hermeticity Gate**:
- [ ] I-8 3층 방어 동작 검증 (manual: agent에게 `cat .dhelix/plasmids/*.md` 시도 → 차단 확인)
- [ ] `plasmid.runtime_access_attempt` metric 발행 확인

**Alpha Gate — v0.4 split (Engineering + Market, hybrid 프로토콜)**:

*Engineering Alpha (Claude, 9 Track A-I)*:
- [ ] Phase 0 POC plasmid 3종 (P01-anna / P03-chris / P05-emma) 실제 schema → recombination → cure 통과
- [ ] I-8 adversarial 7+종 (encoding / injection / symlink / path traversal / tool smuggling) 차단 검증
- [ ] Schema fuzzing 50+ malformed plasmid 처리 검증
- [ ] /cure 3-way merge 5+ scenario PASS
- [ ] Cloud Gate + Local Gate sub-tier (사용자 환경) 실측 데이터 commit (`.dhelix/research/phase-1-engineering-alpha/`)
- [ ] Telemetry `plasmid.runtime_access_attempt` 실발행 확인

*Market Alpha (real human ≥1, hybrid)*:
- [ ] Real human ≥1 명이 dhelix install + plasmid 1개 작성 + recombination 시도 + post-survey 완료
- [ ] H3 (시간) re-validation: 1 명 실측 시간 + 멘탈 모델 fit feedback
- [ ] H4 (user-controlled LLM) re-validation: strict-local 또는 self-hosted user ≥0 명 — 0 명이면 "Phase 6 dogfood 로 defer" 명시
- [ ] Phase 0 self-dogfood 결과와 delta 분석 (`phase-1-market-delta.md`)

**Phase 6 dogfood 로 흡수**: 잔여 인원 분량 (3-5명 → 1+명 으로 완화한 차액) 은 v0.4 release 후 Phase 6 dogfood 로 이전. Pivot scenario A/B/C 는 살려둠 — Phase 6 결과로 발동 가능.

**4 Gate 모두 통과 시에만 Phase 2 진입**. Local Gate 는 사용자 환경에서 *최소 1 sub-tier* 통과로 충족.

### 5.4 Phase 1 완료 후 수행 (Deferred)

본 §5 의 **Phase 1 Week-by-Week 정밀화 + PRD 동기화 작업** 은 Phase 1 전체 개발 완료 후 retrospective 와 함께 일괄 수행한다. 현 시점 (Phase 1 W0) 에서는 §5.1 Setup + §5.2 ADJ 매핑 + §5.3 Gate 체크박스만 활성화 상태.

**Phase 1 완료 후 수행할 작업**:

- [ ] **PRD §13 Phase 1 체크리스트 업데이트** — 현 9개 항목에 ADJ-3 (industry templates) + Alpha Gate 명시 추가, 완료된 항목 [x] 체크
- [ ] **§5.2 Week-by-Week 실제 진행 기록** — 각 W1-W5 실제 소요 시간 + blocker + scope 조정 기록 (Living Document §10.2 원칙)
- [ ] **Phase 0 self-dogfood 결과 재검증** — Alpha Gate 에서 확보한 external data 와 delta 분석 → `go-no-go-decision.md` 에 "Alpha Validation Update" 섹션 추가
- [ ] **Hypothesis re-evaluation** — H1-H4 를 external data 기반으로 재판정. Self-dogfood tentative 상태 해제 또는 pivot trigger
- [ ] **PRD v0.4 전면 통합** — 각 v0.2 설계서의 "영향" 섹션을 PRD 본문으로 승격 (adaptive budget 공식, `secondary-bucket` / `location.after` / `language` / `privacy` / `secondary-bucket` / `share` / `location` schema 필드, `CAP_TIER_THRESHOLD_B` 상수 등)
- [ ] **ADJ-2 (L4 auto-gen priority 역전)** 반영 — Phase 2 W5 validation framework 구현 시점에 P-1.16 §Q2 로 승격
- [ ] **비-Critical 설계서 → v0.3 승격** — Phase 1 구현 중 발견한 새 Open Questions 해결 + 실구현 경험 반영
- [ ] **retrospective 문서 작성** — `docs/retro/phase-1-retro.md` — "무엇이 예상대로 되고 무엇이 어긋났나"

**착수 조건**: Phase 1 Exit Gate 4종 모두 통과 → Phase 2 W1 착수 전 1주 retrospective window 확보.
