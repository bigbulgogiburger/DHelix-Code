...
export default function SandboxBubblewrapPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">Bubblewrap 래퍼</span>
            </h1>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">bubblewrap.ts</code>는 Linux의 bubblewrap(bwrap)
                명령을 감싸는 고수준 래퍼입니다. Docker 같은 컨테이너보다 가볍고, 루트 권한 없이
                프로세스를 격리할 수 있습니다.
                <code className="text-cyan-600">linux.ts</code>가 환경 감지와 기본 실행을
                담당한다면, 이 모듈은 더 정교한 마운트 관리, WSL2 Windows 홈 디렉토리 지원,
                AbortController 기반 타임아웃 관리 등 고급 기능을 제공합니다.
              </p>
              <p>
                마운트(mount) 구조는 &quot;최소 권한 원칙&quot;을 따릅니다. 시스템 경로는 읽기
                전용으로, 프로젝트 디렉토리만 읽기/쓰기로 마운트하며, 홈 디렉토리의 설정 폴더는 읽기
                전용으로 접근합니다. 일부 배포판에서는 /lib, /bin 등이 심볼릭 링크인 경우를 자동으로
                처리합니다.
              </p>
              <p>
                WSL2 환경에서는 Windows 홈 디렉토리(
                <code className="text-cyan-600">/mnt/c/Users/...</code>)를 자동 감지하여 읽기
                전용으로 마운트합니다. 사용자 지정 경로 마운트도{" "}
                <code className="text-cyan-600">allowedPaths</code>를 통해 지원합니다.
              </p>
            </div>
  ...
  style POLICY fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "호스트(실제) 파일 시스템의 경로",
                },
                {
                  name: "sandboxPath",
                  type: "string | undefined",
                  required: false,
                  desc: "샌드박스 내부에서의 경로 (생략 시 hostPath와 동일)",
                },
                {
                  name: "writable",
                  type: "boolean",
                  required: true,
                  desc: "쓰기 허용 여부 (true면 --bind, false면 --ro-bind)",
                },
              ]}
            />
            ...
            <ParamTable
              params={[
                ...
                { name: "cwd", type: "string", required: false, desc: "작업 디렉토리" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "프로젝트 디렉토리 (읽기/쓰기로 마운트)",
                },
                {
                  name: "homeDir",
                  type: "string",
                  required: false,
                  desc: "홈 디렉토리 (기본값: os.homedir())",
                },
                {
                  name: "tmpDir",
                  type: "string",
                  required: false,
                  desc: "임시 디렉토리 (기본값: os.tmpdir())",
                },
                {
                  name: "timeoutMs",
                  type: "number",
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              bubblewrap 명령의 인수 배열을 생성합니다. 시스템 경로 존재 여부를 확인하고, 심볼릭
              링크를 처리하며, WSL2 Windows 홈 디렉토리를 자동 감지합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span>{" "}
              <span className="fn">generateBubblewrapArgs</span>({"\n"}
              {"  "}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              bubblewrap 샌드박스 안에서 명령을 실행합니다. AbortController 기반 타임아웃 관리, 권한
              에러 처리, 비정상 종료 코드 처리를 포함합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span>{" "}
              <span className="fn">executeBubblewrapped</span>({"\n"}
              {"  "}
              <span className="prop">config</span>: <span className="type">BubblewrapConfig</span>
              {"\n"}): <span className="type">Promise</span>&lt;
              <span className="type">SandboxResult</span>&gt;
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                <code className="text-cyan-600">generateBubblewrapArgs()</code>는{" "}
                <strong>async</strong>입니다. 각 시스템 경로와 홈 디렉토리 하위 경로의 존재 여부를
                비동기로 확인하기 때문입니다.
              </li>
              <li>
                타임아웃 시 종료 코드 <code className="text-cyan-600">124</code>를 반환합니다. 이는
                Linux의 <code className="text-cyan-600">timeout</code> 명령과 동일한 규약입니다.
              </li>
              <li>
              ...
              <li>
                <code className="text-cyan-600">.local</code> 디렉토리는 예외적으로 읽기/쓰기로
                마운트됩니다 (일부 도구가 여기에 기록하기 때문).
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">executeBubblewrapped()</code>로 격리된 환경에서 명령을
              실행합니다.
            </p>
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">result</span> ={" "}
              <span className="kw">await</span> <span className="fn">executeBubblewrapped</span>(
              {"{"}
              {"\n"}
              {"  "}
              ...
              <span className="prop">timedOut</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">error</span>(
              <span className="str">&quot;명령이 타임아웃되었습니다&quot;</span>);
              {"\n"}
              {"}"} <span className="kw">else if</span> (<span className="prop">result</span>.
              <span className="prop">exitCode</span> !== <span className="num">0</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">error</span>(
              <span className="str">`종료 코드: ${"{"}</span>
              <span className="prop">result</span>.<span className="prop">exitCode</span>
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> bwrap가 설치되어 있지 않으면 <code>BubblewrapError</code>를
              던집니다. 사전에 <code>linux.ts</code>의 <code>checkLinuxSandboxReady()</code>로 설치
              상태를 확인하는 것을 권장합니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">allowedPaths</code>를 사용하여 추가 경로를 마운트할 수
              있습니다. 각 경로의 읽기/쓰기 여부와 샌드박스 내부 경로를 개별적으로 지정할 수
              있습니다.
            </p>
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">result</span> ={" "}
              ...
              {"  "}
              <span className="prop">projectDir</span>:{" "}
              <span className="str">&quot;/home/user/ml-project&quot;</span>,{"\n"}
              {"  "}
              ...
              {"\n"}
              {"      "}
              <span className="prop">hostPath</span>:{" "}
              <span className="str">&quot;/data/datasets&quot;</span>,{"\n"}
              {"      "}
              <span className="prop">writable</span>: <span className="kw">false</span>,{" "}
              <span className="cm">{"// 읽기 전용"}</span>
              {"\n"}
              {"    "}
              {"}"},{"\n"}
              {"    "}
              ...
              {"      "}
              <span className="prop">hostPath</span>:{" "}
              <span className="str">&quot;/data/output&quot;</span>,{"\n"}
              {"      "}
              <span className="prop">sandboxPath</span>:{" "}
              <span className="str">&quot;/output&quot;</span>,{" "}
              <span className="cm">{"// 내부 경로 변경"}</span>
              {"\n"}
              {"      "}
              <span className="prop">writable</span>: <span className="kw">true</span>,{" "}
              <span className="cm">{"// 쓰기 허용"}</span>
              {"\n"}
              {"    "}
              {"}"},{"\n"}
              {"  "}],
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">env</code> 옵션으로 샌드박스 내부에 환경 변수를
              설정합니다.
            ...
            <DeepDive title="심볼릭 링크 자동 처리">
              <p className="mb-3">
                ...
                <code className="text-cyan-600">/lib</code> 등이 심볼릭 링크입니다 (예:{" "}
                <code className="text-cyan-600">/bin</code> &rarr;{" "}
                <code className="text-cyan-600">/usr/bin</code>).
              </p>
              <p className="mb-3">
                <code className="text-cyan-600">generateBubblewrapArgs()</code>는 이를 자동으로
                감지하여, 실제 경로가 없으면 <code className="text-cyan-600">--symlink</code>{" "}
                플래그로 심볼릭 링크를 생성합니다:
              </p>
              <CodeBlock>
              ...
              <p className="mt-3 text-amber-600">
                이 처리가 없으면 일부 배포판에서 &quot;No such file or directory&quot; 에러가
                발생합니다.
              </p>
            </DeepDive>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">generateBubblewrapArgs()</code>가 생성하는 샌드박스
              내부의 파일 시스템 마운트 구조입니다.
            </p>
  ...
  style TMP fill:#fef3c7,stroke:#f59e0b,color:#92400e`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">executeBubblewrapped()</code>는 다양한 에러 유형을
              구분하여 처리합니다.
            </p>
            <CodeBlock>
              ...
              {"  "}
              <span className="cm">
                {"// 4. code가 number → 비정상 종료 코드 (정상 결과로 반환)"}
              </span>
              {"\n"}
              {"  "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">AbortController:</strong> 타임아웃 관리에{" "}
                <code className="text-cyan-600">AbortController</code>와{" "}
                <code className="text-cyan-600">setTimeout</code>을 이중으로 사용합니다.{" "}
                <code className="text-cyan-600">finally</code>에서 타이머를 반드시 정리합니다.
              </p>
              <p>
                <strong className="text-gray-900">종료 코드 124:</strong> Linux의{" "}
                <code className="text-cyan-600">timeout</code> 명령과 동일한 규약으로, 타임아웃으로
                종료되었음을 나타냅니다.
              </p>
              <p>
                <strong className="text-gray-900">비정상 종료 코드:</strong> 명령 자체가 실행은
                되었지만 실패한 경우(예: 종료 코드 1), 에러를 던지지 않고{" "}
                <code className="text-cyan-600">SandboxResult</code>로 반환합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                ...
                <code className="text-cyan-600">allowedPaths</code>를 사용하여 필요한 경로를 추가
                마운트하세요.
              </p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">/mnt/c/Users/...</code> 경로로 접근 가능한지
                확인하세요. 쓰기가 필요하면 <code className="text-cyan-600">allowedPaths</code>에
