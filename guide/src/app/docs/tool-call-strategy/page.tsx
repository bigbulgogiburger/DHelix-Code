...
export default function ToolCallStrategyPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">ToolCallStrategy</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="infra" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              도구 호출 전략 선택기 &mdash; 모델 능력에 따라 native / text-parsing / two-stage 중
              적절한 도구 호출 방식을 자동 선택하는 모듈입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                <strong>네이티브 전략(Native)</strong>은 OpenAI의{" "}
                <code className="text-cyan-600">tool_calls</code> 필드를 사용하는 표준 방식으로,
                GPT-4o, Claude 등 대부분의 상용 모델이 지원합니다.
                <strong>텍스트 파싱 전략(Text Parsing)</strong>은 XML 태그로 도구 호출을 텍스트에
                포함하는 폴백 방식으로, Llama3 등 네이티브 function calling을 지원하지 않는 모델에서
                사용됩니다.
              </p>
              <p>
                ...
                <code className="text-cyan-600">forceStrategy()</code>로 전략을 강제할 수도
                있습니다.
              </p>
            </div>
  ...
  style LLM fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 여러 언어를 사용하는 통역사를 떠올리세요. 영어를 아는
              상대에게는 영어로 직접 대화하고(네이티브), 영어를 모르는 상대에게는 수화나 그림으로
              의사소통합니다(텍스트 파싱). ToolCallStrategy는 각 모델의 &quot;언어 능력&quot;에 맞는
              통역 방식을 자동으로 선택합니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              ...
              <code className="text-cyan-600">tools</code> 필드에 들어가거나(네이티브), 시스템
              메시지에 텍스트로 주입됩니다(텍스트 파싱).
            </p>
            <ParamTable
              params={[
                {
                  ...
                  required: true,
                  desc: "도구 정의가 반영된 메시지 배열",
                },
                {
                  name: "tools",
                  type: "readonly ToolDefinitionForLLM[]",
                  required: false,
                  desc: "도구 정의 (네이티브 전략에서만 사용, 텍스트 파싱에서는 undefined)",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              요청에 도구 정의를 포함시키는 전처리 단계입니다. 네이티브 전략은 tools 필드에 그대로
              포함하고, 텍스트 파싱 전략은 시스템 메시지에 텍스트로 주입합니다.
            </p>
            <CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              LLM 응답에서 도구 호출을 추출합니다. 네이티브 전략은 toolCalls 배열에서 직접 추출하고,
              텍스트 파싱 전략은 텍스트에서 XML 패턴으로 파싱합니다.
            </p>
            <CodeBlock>
            ...
            <ParamTable
              params={[
                { name: "content", type: "string", required: true, desc: "LLM의 텍스트 응답" },
                {
                  ...
                  required: true,
                  desc: "LLM이 반환한 tool_calls (네이티브에서만 사용)",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              도구 실행 결과를 대화 메시지로 변환합니다. 네이티브 전략은 &quot;tool&quot; 역할
              메시지로, 텍스트 파싱 전략은 &quot;user&quot; 역할 메시지에 XML 형식으로 포함합니다.
            </p>
            <CodeBlock>
            ...
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">selectStrategy</span>(
            ...
            <CodeBlock>
              ...
              {"  "}
              <span className="prop">strategyName</span>:{" "}
              <span className="str">&quot;native&quot;</span> |{" "}
              <span className="str">&quot;text-parsing&quot;</span>,{"\n"}):{" "}
              <span className="type">ToolCallStrategy</span>
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                <code className="text-cyan-600">selectStrategy()</code>는 매 호출마다{" "}
                <strong>새 인스턴스</strong>를 생성합니다. 상태를 공유하지 않으므로 캐싱이
                불필요합니다.
              </li>
              <li>
                Responses API 전용 모델(<code className="text-cyan-600">isResponsesOnlyModel</code>
                )은
                <code className="text-cyan-600">supportsTools</code> 플래그와 무관하게 항상 네이티브
                전략을 사용합니다.
              </li>
              <li>
                텍스트 파싱 전략에서는 <code className="text-cyan-600">tools</code> 필드가
                <code className="text-cyan-600">undefined</code>로 반환됩니다. 도구 정의는 이미
                시스템 프롬프트에 포함되어 있습니다.
              </li>
              <li>
                <code className="text-cyan-600">forceStrategy()</code>는 프로덕션 코드에서 사용하지
                마세요. 모델-전략 불일치로 도구 호출이 실패할 수 있습니다.
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              기본 사용법 &mdash; 전략 자동 선택
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              가장 일반적인 사용 패턴입니다. 모델 이름을 전달하면 자동으로 적절한 전략이 선택됩니다.
            </p>
            <CodeBlock>
              <span className="kw">import</span> {"{ "}
              <span className="fn">selectStrategy</span>
              {" }"} <span className="kw">from</span>{" "}
              <span className="str">&quot;./tool-call-strategy.js&quot;</span>;{"\n"}
              {"\n"}
              ...
              <span className="prop">messages</span>, <span className="prop">prepared</span>.
              <span className="prop">tools</span>);
              {"\n"}
              {"\n"}
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">resultMsgs</span> ={" "}
            ...
            <Callout type="warn" icon="!">
              ...
              <code>formatToolResults()</code>는 반드시 <strong>같은 전략 인스턴스</strong>의
              메서드를 사용해야 합니다. 네이티브로 준비하고 텍스트 파싱으로 추출하면 결과가 엉망이
              됩니다.
            </Callout>
            ...
            <CodeBlock>
              <span className="kw">import</span> {"{ "}
              <span className="fn">forceStrategy</span>
              {" }"} <span className="kw">from</span>{" "}
              <span className="str">&quot;./tool-call-strategy.js&quot;</span>;{"\n"}
              {"\n"}
              <span className="cm">
                {"// 텍스트 파싱 전략을 강제 — 네이티브 지원 모델이더라도"}
              </span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">textStrategy</span> ={" "}
              <span className="fn">forceStrategy</span>(
              <span className="str">&quot;text-parsing&quot;</span>);
              {"\n"}
              <span className="fn">expect</span>(<span className="prop">textStrategy</span>.
              <span className="prop">name</span>).<span className="fn">toBe</span>(
              <span className="str">&quot;text-parsing&quot;</span>);
            </CodeBlock>

            <Callout type="tip" icon="*">
              <strong>팁:</strong> <code>strategy.name</code> 프로퍼티로 현재 어떤 전략이
              선택되었는지 런타임에 확인할 수 있습니다. 로깅이나 디버깅에 유용합니다.
            </Callout>

            <DeepDive title="Responses API 모델의 특별 처리">
              <p className="mb-3">
                <code className="text-cyan-600">isResponsesOnlyModel()</code>로 식별되는 Responses
                API 전용 모델은
                <code className="text-cyan-600">supportsTools</code> 플래그가{" "}
                <code className="text-red-600">false</code>로 설정되어 있더라도 항상 네이티브 전략을
                사용합니다.
              </p>
              <p className="text-gray-600">
                이는 Responses API가 자체적으로 tool calling을 지원하기 때문입니다.
                <code className="text-cyan-600">selectStrategy()</code> 내부에서{" "}
                <code className="text-cyan-600">isResponsesOnlyModel()</code>
                체크가 <code className="text-cyan-600">supportsTools</code> 체크보다{" "}
                <strong>먼저</strong> 실행되어 전략 불일치를 방지합니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style TEXT fill:#fee2e2,stroke:#ef4444,color:#991b1b,stroke-width:2px`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">selectStrategy()</code>의 전체 구현입니다. 간결하지만
              Responses API 모델에 대한 방어 로직이 포함되어 있습니다.
            </p>
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">selectStrategy</span>(
              <span className="prop">modelName</span>: <span className="type">string</span>):{" "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="fn">isResponsesOnlyModel</span>(
              <span className="prop">modelName</span>)) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">return new</span>{" "}
              ...
              <span className="prop">supportsTools</span>) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">return new</span>{" "}
              <span className="fn">NativeFunctionCallingStrategy</span>();
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">[1]</strong> Responses API 전용 모델은
                supportsTools 플래그와 무관하게 항상 네이티브 전략을 사용합니다. 이 체크가 먼저
                실행되어 전략 불일치를 방지합니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong> model-capabilities 모듈에서 모델의
                능력 정보를 조회합니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong> supportsTools가 true이면 네이티브,
                false이면 텍스트 파싱 전략을 반환합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                전략 불일치가 원인일 수 있습니다.{" "}
                <code className="text-cyan-600">strategy.name</code>을 확인하세요:
              </p>
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                <li>
                  <strong>네이티브 전략인데 모델이 tool_calls를 반환하지 않는 경우:</strong> 해당
                  모델의 <code className="text-cyan-600">supportsTools</code>가 잘못 설정되었을 수
                  있습니다.
                  <code className="text-cyan-600">model-capabilities</code>를 확인하세요.
                </li>
                <li>
                  <strong>텍스트 파싱인데 모델이 XML 형식을 따르지 않는 경우:</strong> 모델이 시스템
                  프롬프트의 지시를 따르지 않을 수 있습니다. 더 강력한 모델로 교체하거나 프롬프트를
                  조정하세요.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">supportsTools</code> 플래그가 등록되어 있는지
                확인하세요. 미등록 모델은 기본값을 사용하는데, 기본값이{" "}
                <code className="text-red-600">false</code>이면 텍스트 파싱 전략이 선택됩니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                반환값을 변수에 저장하고 해당 인스턴스를 사용하고 있는지 확인하세요. 또한 에이전트
                루프 내부에서 <code className="text-cyan-600">selectStrategy()</code>가 다시
                호출되어 강제 전략을 덮어쓰고 있지 않은지 확인하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
