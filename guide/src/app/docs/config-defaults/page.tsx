...
export default function ConfigDefaultsPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mb-3">
              <span className="text-gray-900">Config Defaults</span>
            </h1>
            <p className="text-[16px] text-gray-600 max-w-[640px]">
              설정 계층의 최하위 레벨(Level 1) &mdash; 하드코딩된 기본값입니다. 사용자나 프로젝트
              설정이 없을 때 적용되는{" "}
              <span className="text-cyan-600 font-semibold">폴백(fallback) 안전망</span>으로, 아무
              설정 없이도 앱이 정상 동작하도록 보장합니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              <code className="text-cyan-600 text-xs">DEFAULT_CONFIG</code>는 5단계 설정 병합에서
              가장 낮은 우선순위(Level 1)를 가집니다. 모든 설정 키에{" "}
              <strong className="text-gray-900">합리적인 기본값</strong>을 제공하여, 사용자가
              아무것도 설정하지 않아도 앱이 즉시 동작합니다. 모델명은{" "}
              <code className="text-cyan-600 text-xs">constants.ts</code>의{" "}
              <span className="text-violet-600 font-semibold">환경변수 기반 단일 소스</span>에서
              가져옵니다.
            </p>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                <code className="text-violet-600 text-xs">AppConfig</code> 타입의 완전한 기본 설정
                객체입니다. 모든 필드에 값이 있으므로 어떤 설정이 누락되어도 이 값이 폴백됩니다.
              </p>
            </div>
                ...
                params={[
                  {
                    ...
                    type: "string",
                    required: true,
                    desc: "OpenAI 호환 API 엔드포인트 (LLM_DEFAULTS에서 가져옴)",
                  },
                  {
                    name: "llm.model",
                    type: "string",
                    required: true,
                    desc: "AI 모델명 (DEFAULT_MODEL — 환경변수 기반)",
                  },
                  {
                    name: "llm.temperature",
                    type: "number",
                    required: true,
                    desc: "응답 창의성 (기본: 0.0 — 결정적)",
                  },
                  {
                    name: "llm.maxTokens",
                    type: "number",
                    required: true,
                    desc: "최대 생성 토큰 수 (LLM_DEFAULTS에서 가져옴)",
                  },
                  {
                    name: "llm.contextWindow",
                    type: "number",
                    required: true,
                    desc: "컨텍스트 윈도우 크기 (기본: 1,000,000)",
                  },
                  {
                    name: "llm.timeout",
                    type: "number",
                    required: true,
                    desc: "API 타임아웃 ms (기본: 120,000 — bash 도구와 일치)",
                  },
                ]}
              />
            </div>
                ...
                params={[
                  {
                    ...
                    type: '"local"',
                    required: true,
                    desc: "보안 모드 (기본: local — 로컬 전용)",
                  },
                  {
                    name: "security.secretScanning",
                    type: "boolean",
                    required: true,
                    desc: "비밀키 스캔 (기본: true)",
                  },
                  {
                    name: "security.inputFiltering",
                    type: "boolean",
                    required: true,
                    desc: "입력 필터링 — 프롬프트 인젝션 방지 (기본: true)",
                  },
                  {
                    name: "security.outputFiltering",
                    type: "boolean",
                    required: true,
                    desc: "출력 필터링 — 민감 정보 마스킹 (기본: true)",
                  },
                  {
                    name: "security.auditLogging",
                    type: "boolean",
                    required: true,
                    desc: "감사 로깅 (기본: false — 필요 시 활성화)",
                  },
                  {
                    name: "security.rateLimit",
                    type: "object",
                    required: true,
                    desc: "분당 60 요청, 일일 1M 토큰 제한",
                  },
                ]}
              />
            </div>
            ...
            <div className="mb-4">
              ...
              <Callout type="warn" icon="⚠️">
                ...
                <code className="text-red-600 text-xs">editorModel</code>은 defaults.ts에 직접
                하드코딩되어 있습니다. 이 값들은{" "}
                <code className="text-cyan-600 text-xs">constants.ts</code>의 단일 소스 패턴을
                따르지 않으므로 변경 시 주의가 필요합니다.
              </Callout>
            </div>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              <code className="text-cyan-600 text-xs">loadConfig()</code>의 병합 기반(base)으로
              사용됩니다. 다른 레이어의 설정이 이 위에 deepMerge로 얹혀집니다.
            </p>
            ...
            <DeepDive title="기본값 변경 시 영향 범위">
              <p className="mb-3">
                <code className="text-cyan-600 text-xs">DEFAULT_CONFIG</code>를 변경하면 설정
                파일이나 환경변수를 지정하지 않은 <strong>모든 사용자</strong>에게 영향을 미칩니다.
              </p>
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                  <div className="flex gap-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <span>
                      모델명, API URL, 보안 설정 등 &mdash; 모든 사용자의 LLM 연결과 보안에 직접
                      영향
                    </span>
                  </div>
                </div>
  ...
  style SCHEMA fill:#f1f5f9,stroke:#8b5cf6,color:#1e293b`}
            ...
            <Callout type="info" icon="📝">
              ...
              <code className="text-cyan-600 text-xs">schema.ts</code>의 Zod{" "}
              <code className="text-cyan-600 text-xs">.default()</code> 값과
              <code className="text-cyan-600 text-xs"> defaults.ts</code>의 값이 일부 다를 수
              있습니다.
              <code className="text-cyan-600 text-xs"> defaults.ts</code>는{" "}
              <code className="text-cyan-600 text-xs">constants.ts</code>에서 동적으로 가져오지만,
              Zod <code className="text-cyan-600 text-xs">.default()</code>는 import 시점의 정적
              값입니다.
            </Callout>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    ...
                    <code className="text-cyan-600 text-xs">.default()</code>는 import 시점에
                    평가되므로 하드코딩된 값입니다. 반면{" "}
                    <code className="text-cyan-600 text-xs">defaults.ts</code>는{" "}
                    <code className="text-cyan-600 text-xs">constants.ts</code>를 통해 환경변수를
                    반영합니다. 실제 병합 시에는{" "}
                    <code className="text-cyan-600 text-xs">defaults.ts</code> 값이 사용됩니다.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    bash 도구의 실행 타임아웃과 일치시키기 위함입니다. 복잡한 코드 생성 요청은 LLM
                    응답에 2분 이상 걸릴 수 있으므로, API 타임아웃이 너무 짧으면 유효한 응답이
                    잘립니다.
                  </p>
                </div>
              </div>
