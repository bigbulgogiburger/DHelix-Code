...
export default function TeammateStatusPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">TeammateStatus</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="cli" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              멀티 에이전트 팀의 팀원(서브에이전트) 상태를 세 가지 수준으로 표시하는 컴포넌트
              모음입니다.
            </p>
          </div>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                ...
                팀원들의 상태를 시각적으로 보여주는 세 가지 컴포넌트를 제공합니다. 타입 정의, 헬퍼
                함수, 컴포넌트가 하나의 파일에 모두 포함되어 있습니다.
              </p>
              <p>
                세 가지 표시 수준:
                ...
                <code className="text-cyan-600">TeamMemberRow</code>는 개별 멤버의 상태를
                표시합니다.
              </p>
              <p>
  ...
  style TMS fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              표시용 팀 멤버 데이터입니다. 코어의 상세한 에이전트 데이터에서 UI에 필요한 필드만
              추출한 것입니다.
            </p>
            <ParamTable
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "멤버 이름 (최대 20자, 초과 시 말줄임)",
                },
                {
                  name: "role",
                  type: "string",
                  required: true,
                  desc: "멤버 역할 (예: researcher, coder)",
                },
                {
                  name: "status",
                  type: "string",
                  required: true,
                  desc: "상태 (pending, running, completed, failed, cancelled)",
                },
                {
                  name: "elapsed",
                  type: "number",
                  required: false,
                  desc: "경과 시간 (밀리초, 0보다 클 때만 표시)",
                },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "id", type: "string", required: true, desc: "팀 세션 고유 ID" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "팀 이름 (최대 40자, 초과 시 말줄임)",
                },
                {
                  name: "status",
                  type: "string",
                  required: true,
                  desc: '팀 상태 ("active", "running", "completed" 등)',
                },
                {
                  name: "members",
                  type: "readonly TeamMemberSummary[]",
                  required: true,
                  desc: "소속 멤버 배열",
                },
                {
            ...
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">formatElapsedTime(ms)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              밀리초를 사람이 읽기 쉬운 문자열로 변환합니다.
            </p>
            <CodeBlock>
              <span className="kw">export function</span>{" "}
              <span className="fn">formatElapsedTime</span>(<span className="prop">ms</span>:{" "}
              <span className="type">number</span>): <span className="type">string</span>
              {"\n"}
              <span className="cm">
                {'// 예시: 5000 → "5s", 150000 → "2m 30s", 4500000 → "1h 15m"'}
              </span>
            </CodeBlock>

            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">getTeamProgress(members)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              팀 멤버 중 완료된 수와 전체 수를 반환합니다.
            </p>
            <CodeBlock>
              <span className="kw">export function</span>{" "}
              <span className="fn">getTeamProgress</span>(<span className="prop">members</span>:{" "}
              <span className="type">readonly TeamMemberSummary</span>[]): {"{"}
              {"\n"}
              {"  "}
              <span className="kw">readonly</span> <span className="prop">completed</span>:{" "}
              <span className="type">number</span>;{"\n"}
              {"  "}
              <span className="kw">readonly</span> <span className="prop">total</span>:{" "}
              <span className="type">number</span>;{"\n"}
              {"}"}
            </CodeBlock>

            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">countActiveTeams(teams)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              활성 팀 수를 계산합니다. <code className="text-cyan-600">&quot;active&quot;</code>{" "}
              또는
              <code className="text-cyan-600">&quot;running&quot;</code> 상태인 팀만 카운트합니다.
            </p>
            <CodeBlock>
              <span className="kw">export function</span>{" "}
              <span className="fn">countActiveTeams</span>(<span className="prop">teams</span>:{" "}
              <span className="type">readonly TeamSessionSummary</span>[]):{" "}
              <span className="type">number</span>
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="kw">export function</span>{" "}
              <span className="fn">getAggregateMemberCounts</span>(
              <span className="prop">teams</span>:{" "}
              <span className="type">readonly TeamSessionSummary</span>[]): {"{"}
              {"\n"}
              {"  "}
              <span className="kw">readonly</span> <span className="prop">completed</span>:{" "}
            ...
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">TeammateIndicatorProps</h4>
            <ParamTable
              params={[
                {
                  name: "teams",
                  type: "readonly TeamSessionSummary[]",
                  required: true,
                  desc: "표시할 팀 세션 배열 (비어 있으면 null 반환)",
                },
              ]}
            />

            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">TeammateDetailPanelProps</h4>
            <ParamTable
              params={[
                {
                  name: "teams",
                ...
                },
                {
                  ...
                  type: "boolean",
                  required: false,
                  desc: "확장 모드 여부 (기본값: true, false면 헤더+진행률만 표시)",
                },
              ]}
            />

            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">TeamMemberRowProps</h4>
            <ParamTable
              params={[
                {
                  name: "member",
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                ...
                <code className="text-cyan-600">TeammateDetailPanel</code> 모두 팀 배열이 비어
                있으면
                <code className="text-cyan-600">null</code>을 반환합니다.
              </li>
              <li>
                상태 키가 <code className="text-cyan-600">TaskViewPanel</code>과 다릅니다. 여기서는{" "}
                <code className="text-cyan-600">&quot;running&quot;</code>을 사용하고,
                TaskViewPanel은 <code className="text-cyan-600">&quot;in_progress&quot;</code>를
                사용합니다.
              </li>
              <li>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">TeammateIndicator</code>는 상태 바 영역에 한 줄로 팀
              요약을 표시합니다.
            </p>
            <CodeBlock>
              <span className="kw">import</span> {"{"}{" "}
              <span className="type">TeammateIndicator</span> {"}"} <span className="kw">from</span>{" "}
              <span className="str">&quot;./TeammateStatus.js&quot;</span>;{"\n"}
              {"\n"}
              <span className="cm">{"// 상태 바에 팀 요약 표시"}</span>
              {"\n"}&lt;<span className="type">TeammateIndicator</span>{" "}
              <span className="prop">teams</span>={"{"}teams{"}"} /&gt;
              {"\n"}
              {"\n"}
            ...
            <Callout type="warn" icon="!">
              ...
              초록색으로 바뀌고, 아직 진행 중이면 노란색으로 표시됩니다. 팀이 없으면(
              <code>teams.length === 0</code>) 아무것도 렌더링하지 않습니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">TeammateDetailPanel</code>은 팀마다 테두리가 있는
              박스로 멤버 행을 표시합니다.
            </p>
            <CodeBlock>
              <span className="kw">import</span> {"{"}{" "}
              <span className="type">TeammateDetailPanel</span> {"}"}{" "}
              ...
              {"\n"}&lt;<span className="type">TeammateDetailPanel</span>{" "}
              <span className="prop">teams</span>={"{"}teams{"}"}{" "}
              <span className="prop">expanded</span>={"{"}false{"}"} /&gt;
            </CodeBlock>
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="fn">formatElapsedTime</span>(<span className="num">5000</span>);
              {"    "}
              <span className="cm">{'// → "5s"'}</span>
              {"\n"}
              <span className="fn">formatElapsedTime</span>(<span className="num">150000</span>);
              {"  "}
              <span className="cm">{'// → "2m 30s"'}</span>
              {"\n"}
              <span className="fn">formatElapsedTime</span>(<span className="num">4500000</span>);{" "}
              <span className="cm">{'// → "1h 15m"'}</span>
              {"\n"}
              {"\n"}
              <span className="cm">{"// 팀 진행률 확인"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">progress</span> ={" "}
              <span className="fn">getTeamProgress</span>(<span className="prop">team</span>.
              <span className="prop">members</span>);
              {"\n"}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="str">`${"{"}</span>
              <span className="prop">progress</span>.<span className="prop">completed</span>
              <span className="str">
                {"}"}/${"{"}
              </span>
              <span className="prop">progress</span>.<span className="prop">total</span>
              <span className="str">{"}"} 완료`</span>);
            </CodeBlock>

            <DeepDive title="경과 시간 포맷팅 로직 상세">
              <p className="mb-3">
                <code className="text-cyan-600">formatElapsedTime</code>은 세 가지 단위로
                변환합니다:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>60초 미만:</strong> <code>&quot;Ns&quot;</code> (초만 표시)
                </li>
                <li>
                  <strong>60분 미만:</strong> <code>&quot;Nm Ns&quot;</code> (분 + 초, 초가 0이면
                  분만)
                </li>
                <li>
                  <strong>60분 이상:</strong> <code>&quot;Nh Nm&quot;</code> (시 + 분, 분이 0이면
                  시만)
                </li>
              </ul>
              <p className="mt-3 text-amber-600">
                음수 입력에 대해서는 <code>&quot;0s&quot;</code>를 반환합니다. 밀리초를 초로 변환할
                때 <code>Math.floor</code>를 사용하므로 소수점은 버림됩니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              이 파일의 세 컴포넌트는 서로 다른 수준의 정보를 제공하며, 화면의 다른 영역에
              배치됩니다.
            </p>
  ...
  style HELPER4 fill:#fef3c7,stroke:#f59e0b,color:#1e293b`}
            ...
            <CodeBlock>
              ...
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">displayTeamName</span> ={" "}
              <span className="fn">truncateString</span>(<span className="prop">team</span>.
              <span className="prop">name</span>, <span className="num">40</span>);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">isTeamDone</span> ={" "}
              <span className="prop">progress</span>.<span className="prop">completed</span> ==={" "}
              <span className="prop">progress</span>.<span className="prop">total</span>
              {"\n"}
              {"    "}&& <span className="prop">progress</span>.<span className="prop">total</span>{" "}
              ...
              {"  "}
              <span className="kw">const</span> <span className="prop">borderColor</span> ={" "}
              <span className="prop">isTeamDone</span> ?{" "}
              ...
              <span className="prop">borderStyle</span>=
              <span className="str">&quot;single&quot;</span>{" "}
              <span className="prop">borderColor</span>={"{"}borderColor{"}"}&gt;
              {"\n"}
              {"      "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                ...
                <code className="text-cyan-600">getTeamProgress</code>로 완료 멤버 수와 전체 멤버
                수를 계산합니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong> 팀 이름이 40자를 초과하면
                말줄임(…)으로 잘립니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong> 모든 멤버가 완료되면 테두리가 초록색,
                아니면 노란색입니다. 멤버가 0명인 팀은 노란색입니다.
              </p>
              <p>
                <strong className="text-gray-900">[4]</strong>{" "}
                <code className="text-cyan-600">expanded</code>가{" "}
                <code className="text-cyan-600">false</code>면 멤버 행과 하단 진행률이 숨겨지고,
                헤더에 축약 진행률이 표시됩니다.
              </p>
            </div>
          </section>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">TeammateIndicator</code>는{" "}
                <code className="text-cyan-600">teams</code> 배열이 비어 있으면{" "}
                <code className="text-cyan-600">null</code>을 반환합니다. 팀 데이터가 올바르게
                전달되고 있는지 확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">TeamMemberRow</code>는 기본적으로 이름을 20자로
                제한합니다.
                <code className="text-cyan-600">maxNameLength</code> prop을 전달하면 제한을 변경할
                수 있지만, 기본 상수 <code className="text-cyan-600">MAX_DISPLAY_NAME_LENGTH</code>
                는 20으로 고정되어 있습니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                <code className="text-cyan-600">countActiveTeams</code>는{" "}
                <code className="text-cyan-600">&quot;active&quot;</code>
                또는 <code className="text-cyan-600">&quot;running&quot;</code> 상태인 팀만
                카운트합니다. 팀의 <code className="text-cyan-600">status</code> 필드가 이 두 값 중
                하나인지 확인하세요.
              </p>
              <Callout type="tip" icon="*">
                <code>&quot;completed&quot;</code>, <code>&quot;failed&quot;</code>,{" "}
                <code>&quot;cancelled&quot;</code> 상태의 팀은 활성 팀으로 카운트되지 않습니다.
              </Callout>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">TeamMemberRow</code>는{" "}
                <code className="text-cyan-600">elapsed</code>가
                <code className="text-cyan-600">undefined</code>이거나 0 이하일 때 경과 시간을
                표시하지 않습니다. 밀리초 단위의 양수값을 전달해야 합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
