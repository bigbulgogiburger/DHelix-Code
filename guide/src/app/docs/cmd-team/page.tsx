...
export default function CmdTeamPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">/team 팀 오케스트레이션</span>
            </h1>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                ...
                복잡한 작업을 분담하는 <strong>팀 오케스트레이션</strong> 명령어입니다. 예를 들어,
                &quot;보안 분석&quot;, &quot;성능 리뷰&quot;, &quot;타입 검사&quot;를 각각 다른
              ...
              <p>
                ...
                <code className="text-cyan-600">completed</code>(또는{" "}
                <code className="text-red-600">failed</code>)로 진행되며, 각 멤버(에이전트)는
                독립적인 실행 상태를 가집니다.
              </p>
            </div>
  ...
  style AGENTS fill:#fef3c7,stroke:#f59e0b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> /team은 프로젝트 매니저입니다. 여러 전문가(에이전트)에게 각자의
              역할을 배정하고, 진행 상황을 모니터링하며, 필요하면 작업을 취소할 수 있습니다. 각
              전문가는 독립적으로 일하지만, 의존 관계를 설정하여 순서를 제어할 수도 있습니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                ...
                { name: "type", type: "string", required: true, desc: "에이전트 타입" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "에이전트에게 전달할 프롬프트",
                },
                {
                  name: "status",
                  type: "MemberStatus",
                  required: true,
                  desc: "현재 실행 상태 (pending | running | completed | failed | cancelled)",
                },
                {
                  name: "dependsOn",
                  type: "string[]",
                  required: false,
                  desc: "이 멤버가 의존하는 다른 멤버의 ID 배열",
                },
              ]}
            />
            ...
            <ParamTable
              params={[
                ...
                { name: "name", type: "string", required: true, desc: "팀 이름" },
                {
                  ...
                  type: "TeamStatus",
                  required: true,
                  desc: "팀 상태 (creating | active | completing | completed | failed)",
                },
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              팀 관리 기능을 제공하는 의존성 인터페이스입니다. 팩토리 함수에서 주입받습니다.
            </p>
            <CodeBlock>
              ...
              <span className="type">TeamConfig</span>): <span className="type">Promise</span>&lt;
              <span className="type">TeamSession</span>&gt;
              {"\n"}
              {"  "}
              <span className="fn">executeTeam</span>(<span className="prop">teamId</span>:{" "}
              <span className="type">string</span>): <span className="type">Promise</span>&lt;
              <span className="type">TeamResult</span>&gt;
              {"\n"}
              {"  "}
              <span className="fn">cancelTeam</span>(<span className="prop">teamId</span>:{" "}
              <span className="type">string</span>): <span className="type">Promise</span>&lt;
              <span className="type">void</span>&gt;
              {"\n"}
              {"  "}
              <span className="fn">getTeamStatus</span>(<span className="prop">teamId</span>:{" "}
              <span className="type">string</span>):{" "}
              <span className="type">TeamSession | undefined</span>
              {"\n"}
              {"  "}
              <span className="fn">getActiveTeams</span>():{" "}
              <span className="type">TeamSession[]</span>
              {"\n"}
              {"  "}
              ...
              params={[
                {
                  ...
                  type: "생성",
                  required: false,
                  desc: "새 팀 생성 — LLM에게 멤버 구성을 안내하는 프롬프트 주입",
                },
                {
                  name: "/team status [ID]",
                  type: "조회",
                  required: false,
                  desc: "전체 활성 팀 또는 특정 팀의 멤버별 상태 표시",
                },
                {
                  name: "/team cancel <ID>",
                  type: "취소",
                  required: false,
                  desc: "실행 중인 팀 취소 (완료/실패 팀은 취소 불가)",
                },
                {
                  name: "/team list",
                  type: "목록",
                  required: false,
                  desc: "모든 팀 테이블 (ID, 이름, 상태, 멤버 진행률, 생성 시각)",
                },
                {
                  name: "/team summary <ID>",
                  type: "요약",
                  required: false,
                  desc: "팀 상세 요약 — 전체 실행 결과 리포트",
                },
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              ...
              <li>
                ...
                <code>&quot;TEAM_COMMAND_ERROR&quot;</code>입니다.
              </li>
              <li>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>/team create</code>에 팀 이름을 반드시 지정하세요. 이름이
              없으면 에러가 발생합니다. 따옴표로 감싸면 공백이 포함된 이름도 사용 가능합니다.
            </Callout>
            ...
            <Callout type="tip" icon="*">
              <strong>팁:</strong> 멤버 상태 아이콘의 의미: ○ pending (대기), ◐ running (실행 중), ●
            ...
            <DeepDive title="팩토리 패턴과 의존성 주입">
              <p className="mb-3">
                /team 명령어는 팩토리 함수{" "}
                <code className="text-cyan-600">createTeamCommand()</code>로 생성됩니다. 이 패턴을
                사용하는 이유:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>테스트 용이성:</strong> AgentTeamManager를 Mock으로 교체 가능
                </li>
                <li>
                  <strong>느슨한 결합:</strong> 명령어가 팀 관리 구현체에 직접 의존하지 않음
                </li>
                <li>
                  <strong>지연 초기화:</strong> 등록 시점에 Manager가 준비되어야 하므로 팩토리로
                  제어
                </li>
              </ul>
              <CodeBlock>
                ...
                {"\n"}
                <span className="kw">const</span> <span className="prop">teamCmd</span> ={" "}
                <span className="fn">createTeamCommand</span>(
                <span className="prop">teamManager</span>);
                {"\n"}
                <span className="prop">registry</span>.<span className="fn">register</span>(
                <span className="prop">teamCmd</span>);
              </CodeBlock>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style M_CANCEL fill:#f1f5f9,stroke:#64748b,color:#1e293b`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              /team cancel 핸들러는 팀 상태를 확인한 후 취소를 수행합니다. 이미 종료된 팀은 취소할
              수 없도록 가드합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">handleCancel</span>(
              <span className="prop">idArg</span>, <span className="prop">teamManager</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">team</span> ={" "}
              <span className="prop">teamManager</span>.<span className="fn">getTeamStatus</span>(
              <span className="prop">idArg</span>);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="prop">team</span>.
              <span className="prop">status</span> ==={" "}
              <span className="str">&quot;completed&quot;</span> ||{" "}
              <span className="prop">team</span>.<span className="prop">status</span> ==={" "}
              <span className="str">&quot;failed&quot;</span>) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">return</span> {"{"} <span className="prop">success</span>:{" "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">await</span> <span className="prop">teamManager</span>.
              <span className="fn">cancelTeam</span>(<span className="prop">idArg</span>);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">affected</span> ={" "}
              <span className="prop">team</span>.<span className="prop">members</span>.
              <span className="fn">filter</span>({"\n"}
              {"    "}
              <span className="prop">m</span> =&gt; <span className="prop">m</span>.
              <span className="prop">status</span> ==={" "}
              <span className="str">&quot;running&quot;</span> || <span className="prop">m</span>.
              <span className="prop">status</span> ==={" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">formatElapsed()</strong> &mdash; 시작~종료 경과
                시간을 &quot;12s&quot;, &quot;2m 30s&quot;, &quot;1h 3m&quot; 형식으로 포맷합니다.
              </p>
              <p>
                <strong className="text-gray-900">formatTimestamp()</strong> &mdash; 타임스탬프를
                &quot;2m ago&quot;, &quot;1h ago&quot;, &quot;3d ago&quot; 형식의 상대 시간으로
                변환합니다.
              </p>
              <p>
                <strong className="text-gray-900">formatTeamTable()</strong> &mdash; 팀 목록을 ID,
                이름, 상태, 멤버 진행률, 생성 시각 컬럼의 테이블로 포맷합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                <code>/team create</code>는 팀을 생성하고 LLM에게 멤버 구성을 안내하는 프롬프트를
                주입합니다. LLM이 작업 역할을 정의하고 에이전트를 스폰할 때까지 기다려주세요. 이전
                대화에서 작업 내용을 설명했는지 확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code>/team status &lt;ID&gt;</code>로 현재 상태를 먼저 확인하세요. 취소는{" "}
                <code>active</code> 또는 <code>creating</code> 상태에서만 가능합니다.
              </p>
            </div>
