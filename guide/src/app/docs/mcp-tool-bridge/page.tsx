...
export default function MCPToolBridgePage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">MCPToolBridge</span>
            </h1>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                변환 과정에서 세 가지 핵심 작업이 이루어집니다: (1) 이름 네임스페이싱 (
                <code className="text-cyan-600">mcp__서버명__도구명</code> 형태), (2) JSON Schema를
                Zod 스키마로 변환 (런타임 타입 검증), (3) 실행 프록시 생성 (호출 시 MCP 클라이언트를
                통해 원격 실행).
              </p>
              <p>
                대용량 출력에 대한 3-Tier 제한 시스템을 포함하며, 에러 발생 시 유형별 recovery
                힌트를 LLM에 제공하여 적절한 대응을 유도합니다.
              </p>
            </div>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              브리지 동작 중 발생하는 에러 클래스입니다. 에러 코드{" "}
              <code className="text-cyan-600">&quot;MCP_BRIDGE_ERROR&quot;</code>로 식별됩니다.
            </p>
              ...
              params={[
                {
                  ...
                  type: "number",
                  required: true,
                  desc: "최대 출력 토큰 수 (기본 25,000). 환경변수 MAX_MCP_OUTPUT_TOKENS로 변경 가능",
                },
                {
                  name: "MCP_TOOL_TIMEOUT_MS",
                  type: "number",
                  required: true,
                  desc: "도구 실행 타임아웃 (기본 120초). 환경변수 MCP_TOOL_TIMEOUT으로 변경 가능",
                },
                {
                  name: "LARGE_OUTPUT_THRESHOLD",
                  type: "number",
                  required: true,
                  desc: "대용량 출력 파일 저장 임계값 (400,000자)",
                },
                {
                  name: "PREVIEW_SIZE",
                  type: "number",
                  required: true,
                  desc: "대용량 출력 시 LLM에 전달할 미리보기 크기 (2,000자)",
                },
              ]}
            />
            ...
            <CodeBlock>
              <span className="kw">constructor</span>(<span className="prop">toolRegistry</span>:{" "}
              <span className="type">ToolRegistry</span>)
            </CodeBlock>
            <ParamTable
              params={[
                {
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              MCP 클라이언트에서 도구를 발견하고 레지스트리에 등록합니다. 도구 목록 변경 알림
              핸들러도 자동으로 등록됩니다.
            </p>
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">registerTools</span>(
              ...
              params={[
                {
                  ...
                  type: "MCPClient",
                  required: true,
                  desc: "도구를 가져올 MCP 클라이언트",
                },
                {
                  name: "serverName",
                  type: "string",
                  required: true,
                  desc: "MCP 서버 이름 (네임스페이싱에 사용)",
                },
              ]}
            />
            ...
            <CodeBlock>
              <span className="fn">getServerTools</span>(<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>): <span className="type">readonly string[]</span>
            </CodeBlock>

            {/* getRegisteredServers */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">getRegisteredServers()</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              도구가 등록된 모든 서버 이름을 반환합니다.
            </p>
            <CodeBlock>
              <span className="fn">getRegisteredServers</span>():{" "}
            ...
            <CodeBlock>
              <span className="fn">shouldDeferTools</span>(
              <span className="prop">mcpToolTokens</span>: <span className="type">number</span>,{" "}
              <span className="prop">maxContextTokens</span>: <span className="type">number</span>):{" "}
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                네임스페이싱 규칙: <code className="text-cyan-600">mcp__서버명__도구명</code>{" "}
                &mdash; 이중 밑줄(<code>__</code>)로 구분됩니다. 서버명이나 도구명에 이중 밑줄이
                포함되면 충돌할 수 있습니다.
              </li>
              <li>
                이미 등록된 도구는 건너뜁니다 (재연결 시 중복 방지). 현재 레지스트리는 도구{" "}
                <strong>제거</strong>를 지원하지 않으므로 새 도구만 추가됩니다.
              </li>
              <li>
                모든 MCP 도구의 권한 레벨은{" "}
                <code className="text-cyan-600">&quot;confirm&quot;</code>으로 설정됩니다.
                사용자에게 실행 확인을 요청합니다.
              </li>
              <li>
                <code className="text-cyan-600">jsonSchemaToZod()</code>는 기본 타입(object, string,
                number, boolean, array)만 지원합니다. 복잡한 JSON Schema(<code>oneOf</code>,{" "}
                <code>allOf</code> 등)는
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">bridge</span> ={" "}
              <span className="kw">new</span> <span className="fn">MCPToolBridge</span>(
              ...
              <span className="fn">registerTools</span>(<span className="prop">client</span>,{" "}
              <span className="str">&quot;github&quot;</span>);
              {"\n"}
              <span className="cm">
                {'// → ["mcp__github__create_issue", "mcp__github__search", ...]'}
              </span>
              {"\n"}
              {"\n"}
              ...
              <span className="prop">bridge</span>.<span className="fn">getServerTools</span>(
              <span className="str">&quot;github&quot;</span>);
              {"\n"}
              <span className="kw">const</span> <span className="prop">servers</span> ={" "}
              <span className="prop">bridge</span>.<span className="fn">getRegisteredServers</span>
              ();
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>registerTools()</code>는 <code>client.listTools()</code>
              를 호출하므로, MCPClient가 <code>&quot;connected&quot;</code> 상태여야 합니다.
            ...
            <Callout type="tip" icon="*">
              <strong>팁:</strong> <code>shouldDeferTools()</code>로 MCP 도구가 컨텍스트의 10%를
              초과하는지 미리 확인할 수 있습니다. 초과하면 지연 로딩(deferred)으로 전환하여 토큰을
              절약하세요.
            </Callout>
          </section>
        </RevealOnScroll>
  ...
  style T3 fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              핵심 코드 분석 &mdash; 에러 분류와 Recovery 힌트
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              MCP 도구 실행 실패 시, 에러 메시지를 분류하여 LLM에게 적절한 대응 방법을 안내합니다.
            </p>
            <CodeBlock>
              <span className="kw">type</span> <span className="type">MCPErrorType</span> ={" "}
              <span className="str">&quot;timeout&quot;</span> |{" "}
              <span className="str">&quot;connection&quot;</span> |{" "}
              <span className="str">&quot;permission&quot;</span> |{" "}
              <span className="str">&quot;server_error&quot;</span> |{" "}
              <span className="str">&quot;unknown&quot;</span>;{"\n"}
              {"\n"}
              <span className="kw">function</span> <span className="fn">classifyMCPError</span>(
              <span className="prop">errorMsg</span>: <span className="type">string</span>):{" "}
              <span className="type">MCPErrorType</span> {"{"}
              {"\n"}
              {"  "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">timeout:</strong> &quot;재시도하지 말고 사용자에게
                알리세요&quot;
              </p>
              <p>
                <strong className="text-gray-900">connection:</strong> &quot;서버가 실행 중인지
                /mcp로 확인하세요&quot;
              </p>
              <p>
                <strong className="text-gray-900">permission:</strong> &quot;재시도하지 말고 대안을
                제시하세요&quot;
              </p>
              <p>
                <strong className="text-gray-900">server_error:</strong> &quot;일시적 문제일 수
                있으니 사용자에게 재시도 의사를 확인하세요&quot;
              </p>
            </div>

            <DeepDive title="jsonSchemaToZod() 변환 규칙 상세">
              <p className="mb-3">
                MCP의 JSON Schema를 Zod 스키마로 재귀적으로 변환합니다. 지원하는 타입과 Zod 매핑:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <code className="text-cyan-600">&quot;object&quot;</code> &rarr;{" "}
                  <code>z.object(shape).passthrough()</code> &mdash; 속성별 재귀 변환, 추가 필드
                  허용
                </li>
                <li>
                  <code className="text-cyan-600">&quot;string&quot;</code> &rarr;{" "}
                  <code>z.string()</code>
                </li>
                <li>
                  <code className="text-cyan-600">&quot;number&quot; / &quot;integer&quot;</code>{" "}
                  &rarr; <code>z.number()</code>
                </li>
                <li>
                  <code className="text-cyan-600">&quot;boolean&quot;</code> &rarr;{" "}
                  <code>z.boolean()</code>
                </li>
                <li>
                  <code className="text-cyan-600">&quot;array&quot;</code> &rarr;{" "}
                  <code>z.array(itemSchema)</code>
                </li>
                <li>
                  <strong>알 수 없는 타입</strong> &rarr; <code>z.record(z.unknown())</code> (자유
                  형태 폴백)
                </li>
              </ul>
              <p className="mt-3 text-amber-600">
                <code>required</code> 배열에 없는 속성은 자동으로 <code>.optional()</code>이
                적용됩니다.
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                <li>
                  <code className="text-cyan-600">[OUTPUT TRUNCATED]</code> 표시: Tier 2 &mdash;{" "}
                  <code>MAX_MCP_OUTPUT_TOKENS</code> 환경변수를 늘려보세요.
                </li>
                <li>
                  <code className="text-cyan-600">&lt;persisted-output&gt;</code> 표시: Tier 3
                  &mdash; 전체 출력이 임시 파일로 저장되었습니다. 경로를 확인하세요.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">MCP_TOOL_TIMEOUT</code> 환경변수를 높여보세요. 예:{" "}
                <code className="text-cyan-600">MCP_TOOL_TIMEOUT=300000</code> (5분)
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                도구 이름이 네임스페이싱되어 있는지 확인하세요. MCP 도구는{" "}
                <code className="text-cyan-600">mcp__서버명__도구명</code> 형태로 등록됩니다.
                <code className="text-cyan-600">getServerTools()</code>로 등록된 도구 목록을
                확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">z.record(z.unknown())</code>로 폴백됩니다. 이 경우
                타입 검증이 느슨해지지만, 도구 실행 자체는 정상적으로 동작합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
