  ...
  return (
      ...
      <div className="center-narrow">
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-5">
              LLM 모델마다 도구 호출 지원 여부, 시스템 메시지 형식, temperature 파라미터 사용 가능
              여부 등이 다릅니다. 이 모듈이 없으면 모든 모델을 동일하게 취급하게 되어, o1 모델에
              temperature를 보내거나 Llama3에 function calling을 시도하는 등의 런타임 오류가
              발생합니다.
            </p>

            <Callout type="info" icon="💡">
              <span className="text-gray-900 font-semibold">왜 중요한가?</span> Agent Loop, System
              Prompt Builder, LLM Client 모두 이 모듈을 통해 모델별 동작을 결정합니다. 새 모델을
              추가할 때 이 파일 하나만 수정하면 됩니다.
            </Callout>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              모델의 전반적인 성능 수준을 나타내는 유니언 타입입니다. 프롬프트 스타일, 컨텍스트
              전략, 도구 호출 방식을 결정하는 데 사용됩니다.
            </p>
            <CodeBlock>
              ...
              params={[
                {
                  ...
                  type: "boolean",
                  required: true,
                  desc: "도구(function calling) 지원 여부. false이면 텍스트 파싱으로 대체",
                },
                {
                  name: "supportsSystemMessage",
                  type: "boolean",
                  required: true,
                  desc: "system 역할 메시지 지원 여부. false이면 user 메시지로 변환",
                },
                {
                  name: "supportsTemperature",
                  type: "boolean",
                  required: true,
                  desc: "temperature 파라미터 지원 여부. o1/o3 추론 모델은 미지원",
                },
                {
                ...
                },
                {
                  ...
                  type: "number",
                  required: true,
                  desc: "최대 컨텍스트 윈도우 크기 (입력+출력 합산 토큰 수)",
                },
                {
                  name: "maxOutputTokens",
                  type: "number",
                  required: true,
                  desc: "한 번의 응답에서 생성할 수 있는 최대 토큰 수",
                },
                {
                  name: "tokenizer",
                  type: '"cl100k" | "o200k" | "llama"',
                  required: true,
                  desc: "사용할 토크나이저 종류",
                },
                {
                  name: "useDeveloperRole",
                  type: "boolean",
                  required: true,
                  desc: "o1/o3 추론 모델 전용 — system을 developer 역할로 변환",
                },
                {
                  name: "pricing",
                  type: "ModelPricingInfo",
                  required: true,
                  desc: "토큰 가격 정보 (미지 모델은 $1/$3 기본값)",
                },
                {
                  name: "useMaxCompletionTokens",
                  type: "boolean",
                  required: true,
                  desc: "true: max_completion_tokens 사용, false: max_tokens 사용 (레거시)",
                },
                {
                  name: "capabilityTier",
                  type: "CapabilityTier",
                  required: true,
                  desc: "능력 티어 — 적응형 프롬프트/컨텍스트 전략에 사용",
                },
                {
                  name: "supportsCaching",
                  type: "boolean",
                  required: true,
                  desc: "명시적 프롬프트 캐싱 지원 여부 (Anthropic 전용)",
                },
                {
                  name: "supportsThinking",
                  type: "boolean",
                  required: true,
                  desc: "Extended Thinking(확장 사고) 지원 여부 (Claude 모델)",
                },
                {
                  name: "defaultThinkingBudget",
                  type: "number",
                  required: true,
                  desc: "기본 사고 예산 (토큰 수, 0이면 컨텍스트 기반 자동 계산)",
                },
              ]}
            />
            ...
            <Callout type="warn" icon="⚠️">
              ...
              <span className="font-mono text-cyan-600">useDeveloperRole</span>은 OpenAI API의 세대
              차이를 처리하기 위한 플래그입니다. 레거시 모델(GPT-3.5, GPT-4)은{" "}
              <span className="font-mono text-amber-600">max_tokens</span>를, 최신 모델은{" "}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              모델 이름을 받아 해당 모델의 능력 정보를 반환하는 메인 조회 함수입니다. 내부의{" "}
              <span className="font-mono text-cyan-600">MODEL_OVERRIDES</span> 배열에서 정규식
              패턴을 순서대로 매칭하여 첫 번째 일치하는 설정을 기본값에 병합(spread)합니다.
            </p>
            <ParamTable
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: '모델 이름 (예: "gpt-4o", "claude-sonnet-4-20250514", "llama3.1")',
                },
              ]}
            />
            <p className="text-[13px] text-gray-600 mt-2 mb-1">
              <strong className="text-gray-900">반환값:</strong>{" "}
              <span className="font-mono text-violet-600">ModelCapabilities</span> 객체. 일치하는
              패턴이 없으면 안전한 기본값을 반환합니다.
            </p>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="text-[#e6edf3]">caps</span> <span className="text-[#ff7b72]">=</span>{" "}
              <span className="text-[#ffa657]">getModelCapabilities</span>
              <span className="text-[#e6edf3]">(</span>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              LLM Client에서 실제로 사용하는 패턴입니다. 모델마다 다른 API 파라미터를 자동으로
              적용합니다.
            </p>
            <CodeBlock>
              <span className="text-[#ff7b72]">const</span>{" "}
              ...
              <span className="text-[#e6edf3]">
                {"? { max_completion_tokens: caps.maxOutputTokens }"}
              </span>
              {"\n"}
              {"    "}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              기본값에서 덮어쓸 속성을 추가합니다. 기본값(
              <span className="font-mono text-cyan-600">DEFAULTS</span>)에서 변경이 필요한 속성만
              지정하면 됩니다.
            </p>
            <CodeBlock>
              <span className="text-[#8b949e]">
                {"// MODEL_OVERRIDES 배열에 새 모델 추가 예시"}
              </span>
              {"\n"}
              <span className="text-[#e6edf3]">[</span>
              {"\n"}
              ...
              <span className="text-[#e6edf3]">
                {": { inputPerMillion: 1.5, outputPerMillion: 5 },"}
              </span>
              {"\n"}
              {"    "}
            ...
            <Callout type="danger" icon="🚨">
              <span className="text-gray-900 font-semibold">패턴 순서가 중요합니다!</span> 정규식
              매칭은 배열 순서대로 이루어지며, 첫 번째 일치하는 패턴이 사용됩니다. 더 구체적인
              패턴을 먼저 배치해야 합니다. 예를 들어{" "}
              <span className="font-mono text-cyan-600">gpt-4o-mini</span>가{" "}
            ...
            <DeepDive title="능력 티어(Capability Tier) 상세">
              <p className="mb-4">
                능력 티어는 모델의 전반적인 성능 수준을 3단계로 분류합니다. Agent Loop, System
                Prompt Builder 등 여러 모듈이 이 티어를 기준으로 프롬프트 스타일과 컨텍스트 전략을
                결정합니다.
              </p>
              ...
              <Callout type="tip" icon="✅">
                <span className="text-gray-900 font-semibold">Tip:</span> 로컬 모델(Ollama)을 사용할
                때 pricing을{" "}
                <span className="font-mono text-cyan-600">{`{ inputPerMillion: 0, outputPerMillion: 0 }`}</span>
                으로 설정하면 비용 추적에서 제외됩니다.
              </Callout>
            </DeepDive>
          </section>
        </RevealOnScroll>
    ...
    style OUTPUT fill:#ede9fe,stroke:#8b5cf6,color:#5b21b6`}
            ...
            <CodeBlock>
              ...
              {"\n"}
              {"  "}
              <span className="text-[#79c0ff]">supportsSystemMessage</span>
              <span className="text-[#e6edf3]">: </span>
              <span className="text-[#79c0ff]">true</span>
              <span className="text-[#e6edf3]">,</span>
              {"\n"}
              ...
              {"\n"}
              {"  "}
              <span className="text-[#79c0ff]">supportsStreaming</span>
              <span className="text-[#e6edf3]">: </span>
              <span className="text-[#79c0ff]">true</span>
              <span className="text-[#e6edf3]">,</span>
              {"\n"}
              ...
              <span className="text-[#79c0ff]">maxContextTokens</span>
              <span className="text-[#e6edf3]">: </span>
              <span className="text-[#79c0ff]">128_000</span>
              <span className="text-[#e6edf3]">,</span>
              {"  "}
              ...
              <span className="text-[#79c0ff]">maxOutputTokens</span>
              <span className="text-[#e6edf3]">: </span>
              <span className="text-[#79c0ff]">4096</span>
              <span className="text-[#e6edf3]">,</span>
              {"\n"}
              {"  "}
              <span className="text-[#79c0ff]">tokenizer</span>
              <span className="text-[#e6edf3]">: </span>
              <span className="text-[#a5d6ff]">{'"o200k"'}</span>
              <span className="text-[#e6edf3]">,</span>
              {"\n"}
              {"  "}
              <span className="text-[#79c0ff]">capabilityTier</span>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              <span className="font-mono text-cyan-600">
                [RegExp, Partial{"<ModelCapabilities>"}]
              </span>{" "}
              쌍의 읽기 전용 배열입니다. 현재 등록된 모델 그룹:
            </p>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              단순한 선형 탐색(linear scan)입니다. MODEL_OVERRIDES 배열을 순회하면서 첫 번째로
              매칭되는 패턴의 재정의 값을 DEFAULTS에 spread 병합합니다.
            </p>
            <CodeBlock>
            ...
            <Callout type="tip" icon="💡">
              <span className="text-gray-900 font-semibold">설계 포인트:</span> 매번 새 객체를 반환(
              <span className="font-mono text-cyan-600">{"{ ...DEFAULTS }"}</span>)하므로 호출자가
              반환값을 수정해도 원본 DEFAULTS에 영향을 주지 않습니다. 불변성(immutability) 원칙을
              지키는 패턴입니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-sm font-bold text-gray-900 mb-2 flex items-center gap-2">
                  <span className="text-amber-600">Q.</span>새 모델을 추가했는데 기본값(DEFAULTS)이
                  적용됩니다
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <span className="text-emerald-600 font-bold">A.</span> 정규식 패턴이 모델 이름과
                  일치하는지 확인하세요. 모든 패턴은 대소문자 무시(
                  <span className="font-mono text-cyan-600">/i</span>) 플래그를 사용합니다. 패턴
                  테스트:{" "}
                  <span className="font-mono text-cyan-600">/^my-model/i.test("my-model-v2")</span>
                  가 <span className="font-mono text-emerald-600">true</span>를 반환하는지
                  확인하세요. 또한 더 일반적인 패턴이 위에 있어서 먼저 매칭되고 있을 수 있습니다.
                </p>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <span className="text-emerald-600 font-bold">A.</span> MODEL_OVERRIDES 배열에서
                  패턴 순서를 확인하세요.{" "}
                  <span className="font-mono text-cyan-600">/^gpt-4o-mini/i</span>가{" "}
                  <span className="font-mono text-cyan-600">/^gpt-4o/i</span>보다{" "}
                  <strong className="text-gray-900">위에(먼저)</strong> 배치되어야 합니다. 첫 번째
                  매칭이 사용되므로, 구체적인 패턴이 범용 패턴보다 항상 앞에 와야 합니다.
                </p>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <span className="text-emerald-600 font-bold">A.</span> 로컬 모델의 pricing을{" "}
                  <span className="font-mono text-cyan-600">{`{ inputPerMillion: 0, outputPerMillion: 0 }`}</span>
                  으로 설정해야 합니다. 이 값이 없으면 기본 가격($1/$3)이 적용되어 로컬 모델임에도
                  비용이 계산됩니다. Llama, Phi, Gemma, Qwen 등 이미 등록된 로컬 모델은 이미 0으로
                  설정되어 있습니다.
                </p>
              </div>
            </div>
