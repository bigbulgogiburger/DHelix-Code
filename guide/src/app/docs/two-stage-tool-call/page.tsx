...
export default function TwoStageToolCallPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">TwoStageToolCall</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="infra" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              2단계 도구 호출 (계획 &rarr; 실행) &mdash; 저능력 모델의 자연어 의도를 정규식 패턴
              매칭으로 구조화된 도구 호출로 변환하는 모듈입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                일부 소형 로컬 모델(Tiny Llama, Phi-2 등)은 네이티브 function calling이나 XML 형식의
                도구 호출도 제대로 수행하지 못하고, 자연어로 의도를 표현하는 경우가 있습니다. 예를
                들어 <code className="text-cyan-600">&quot;read file src/main.ts&quot;</code>라고
                응답하는 식입니다.
              </p>
              <p>
                ...
                <strong>2단계(실행)</strong>에서 이 모듈이 정규식으로 의도를 파싱하여 구조화된 도구
                호출로 변환합니다.
              </p>
              <p>
  ...
  style EXEC fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 아이가 &quot;저 그 인형 갖고 싶어!&quot;라고 손가락으로
              가리키면, 부모가 아이의 의도를 해석하여 정확한 인형을 집어주는 것과 같습니다.
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: '매칭된 도구 이름 (예: "file_read", "bash_exec")',
                },
                {
                  name: "extractedArgs",
                  type: "Record<string, unknown>",
                  required: true,
                  desc: '추출된 인자 — 정규식 캡처 그룹에서 추출 (예: { file_path: "src/main.ts" })',
                },
                {
                  name: "requiredParams",
              ...
              params={[
                {
                  ...
                  type: "RegExp",
                  required: true,
                  desc: "자연어 텍스트를 매칭하는 정규식",
                },
                { name: "tool", type: "string", required: true, desc: "매칭 시 사용할 도구 이름" },
                {
                  name: "argMap",
                  type: "(m: RegExpMatchArray) => Record<string, unknown>",
                  required: true,
                  desc: "정규식 매칭 결과에서 인자를 추출하는 함수",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              7가지 핵심 도구에 대한 자연어 의도 패턴 목록입니다. 각 패턴은 특정 동사로 시작하는
              자연어 명령을 매칭합니다.
            </p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mt-3">
              <table className="w-full text-[13px]">
                ...
                <tbody className="text-gray-600">
                  ...
                  <tr className="border-b border-gray-100">
                    <td className="px-4 py-2 font-mono text-cyan-600">
                      search [for] &lt;pattern&gt;
                    </td>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              자연어 텍스트를 분석하여 구조화된 도구 호출 의도를 추출합니다. 패턴 목록을 순서대로
              매칭하며, 첫 번째로 일치하는 패턴을 사용합니다. 매칭된 도구가{" "}
              <code className="text-cyan-600">availableTools</code>에 존재하는지도 확인합니다.
            </p>
            <CodeBlock>
              <span className="kw">function</span>{" "}
              <span className="fn">parseNaturalLanguageIntent</span>({"\n"}
              {"  "}
              <span className="prop">text</span>: <span className="type">string</span>,{"\n"}
              {"  "}
              <span className="prop">availableTools</span>:{" "}
              <span className="type">readonly ToolDefinition[]</span>,{"\n"}):{" "}
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                패턴 매칭은 <strong>순서 우선</strong>입니다.{" "}
                <code className="text-cyan-600">&quot;search for files *.ts&quot;</code>는
                <code className="text-cyan-600">grep_search</code>로 매칭됩니다(
                <code className="text-cyan-600">search</code> 패턴이
                <code className="text-cyan-600">find</code> 패턴보다 먼저 정의되어 있으므로).
              </li>
              <li>
                <code className="text-cyan-600">confidence</code>는 현재 <strong>고정값 0.9</strong>
                입니다. 향후 패턴 복잡도나 매칭 품질에 따라 동적으로 계산될 수 있습니다.
              </li>
              <li>
                매칭된 도구가 <code className="text-cyan-600">availableTools</code>에 없으면{" "}
                <code className="text-cyan-600">null</code>을 반환합니다. 권한 설정으로 비활성화된
                도구는 매칭에서 제외됩니다.
              </li>
              <li>
                정규식 매칭은 대소문자를 구분하지 않습니다(<code className="text-cyan-600">/i</code>{" "}
                플래그). &quot;Read file&quot;과 &quot;read file&quot; 모두 매칭됩니다.
              </li>
              <li>
                이 모듈은 <strong>단일 인자</strong>만 추출할 수 있습니다. 여러 인자가 필요한
                도구(예: file_edit의 old_string + new_string)는 완전히 지원되지 않습니다.
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="str">&quot;./two-stage-tool-call.js&quot;</span>;{"\n"}
              {"\n"}
              <span className="cm">{"// LLM이 자연어로 응답한 경우"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">llmResponse</span> ={" "}
              <span className="str">
                &quot;I&apos;ll read file src/main.ts to understand the structure&quot;
              </span>
              ;{"\n"}
              {"\n"}
              <span className="cm">{"// 자연어에서 도구 호출 의도 추출"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">intent</span> ={" "}
              <span className="fn">parseNaturalLanguageIntent</span>(
              <span className="prop">llmResponse</span>,{" "}
              <span className="prop">availableTools</span>);
              {"\n"}
              {"\n"}
              ...
              <span className="prop">intent</span>.<span className="prop">toolName</span>);
              {"      "}
              <span className="cm">{'// "file_read"'}</span>
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="prop">intent</span>.<span className="prop">extractedArgs</span>);
              {"  "}
              <span className="cm">{'// { file_path: "src/main.ts" }'}</span>
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="prop">intent</span>.<span className="prop">confidence</span>);
              {"    "}
              <span className="cm">{"// 0.9"}</span>
              {"\n"}
              {"}"}
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>parseNaturalLanguageIntent()</code>의 반환값은{" "}
              <code>null</code>일 수 있습니다. 어떤 패턴에도 매칭되지 않거나, 매칭된 도구가 사용
              가능 목록에 없으면 <code>null</code>을 반환합니다. 반드시 <code>null</code> 체크를
              수행하세요.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              에이전트 루프에서 네이티브/텍스트 파싱 전략이 도구 호출을 추출하지 못했을 때 마지막
              수단으로 자연어 의도 파싱을 시도합니다.
            </p>
            <CodeBlock>
              ...
              <span className="prop">content</span>, <span className="prop">toolCalls</span>);
              {"\n"}
              {"\n"}
              <span className="kw">if</span> (<span className="prop">calls</span>.
              ...
              <span className="kw">const</span> <span className="prop">intent</span> ={" "}
              <span className="fn">parseNaturalLanguageIntent</span>(
              <span className="prop">content</span>, <span className="prop">tools</span>);
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="prop">intent</span>) {"{"}
              {"\n"}
              ...
              {"\n"}
              {"      "}
              <span className="prop">name</span>: <span className="prop">intent</span>.
              <span className="prop">toolName</span>,{"\n"}
              {"      "}
              <span className="prop">arguments</span>: <span className="prop">intent</span>.
              <span className="prop">extractedArgs</span>,{"\n"}
              {"    "}
              {"}"});
              {"\n"}
              {"  "}
            ...
            <Callout type="tip" icon="*">
              ...
              사용자에게 확인을 요청하는 로직을 추가할 수 있습니다. 현재는 고정값 0.9이지만, 향후
              동적 계산으로 발전할 수 있습니다.
            </Callout>
            ...
            <DeepDive title="패턴 매칭 우선순위와 한계">
              <p className="mb-3">
                ...
                첫 번째로 일치하는 패턴이 사용되므로, 모호한 입력에서는 의도하지 않은 도구가 매칭될
                수 있습니다:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <code>&quot;search for files *.ts&quot;</code> &rarr; <code>grep_search</code>{" "}
                  (find가 아닌 search가 먼저 매칭)
                </li>
                <li>
                  <code>&quot;write a test&quot;</code> &rarr; <code>file_write</code>{" "}
                  (&quot;write&quot;가 매칭되지만 의도는 코드 작성)
                </li>
                <li>
                  <code>&quot;edit the config&quot;</code> &rarr; <code>file_edit</code> (&quot;the
                  config&quot;가 file_path로 추출)
                </li>
              </ul>
              <p className="mt-3 text-amber-600">
                이러한 한계 때문에 이 전략은 &quot;최후의 수단&quot;으로만 사용해야 합니다. 가능하면
                네이티브나 텍스트 파싱 전략을 우선 사용하세요.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">parseNaturalLanguageIntent()</code>의 내부 판단
              흐름입니다. 7가지 패턴을 순서대로 매칭하고, 도구 존재 여부를 확인합니다.
            </p>
  ...
  style NULL fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <CodeBlock>
              ...
              <span className="prop">p</span> <span className="kw">of</span>{" "}
              <span className="prop">INTENT_PATTERNS</span>) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">const</span> <span className="prop">match</span> ={" "}
              <span className="prop">trimmed</span>.<span className="fn">match</span>(
              <span className="prop">p</span>.<span className="prop">regex</span>);
              {"\n"}
              {"    "}
              ...
              <span className="prop">availableTools</span>.<span className="fn">some</span>({"\n"}
              {"      "}
              <span className="prop">t</span> =&gt; <span className="prop">t</span>.
              <span className="prop">name</span> === <span className="prop">p</span>.
              ...
              <span className="prop">p</span>.<span className="fn">argMap</span>(
              <span className="prop">match</span>);
              {"\n"}
              {"      "}
              <span className="kw">return</span> {"{"}
              {"\n"}
              {"        "}
              <span className="prop">toolName</span>: <span className="prop">p</span>.
              <span className="prop">tool</span>,{"\n"}
              {"        "}
              <span className="prop">extractedArgs</span>: <span className="prop">args</span>,{"\n"}
              {"        "}
              <span className="prop">requiredParams</span>: <span className="fn">Object</span>.
              <span className="fn">keys</span>(<span className="prop">args</span>),
              {"\n"}
              {"        "}
              <span className="prop">confidence</span>: <span className="num">0.9</span>,{"  "}
              <span className="cm">{"// [3] 고정 신뢰도"}</span>
              {"\n"}
              {"      "}
              {"}"};{"\n"}
              {"    "}
              ...
              <span className="cm">{"// [4] 매칭 실패"}</span>
              {"\n"}
              {"}"}
            </CodeBlock>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">[1]</strong> 정규식이 매칭되더라도 해당 도구가{" "}
                <code className="text-cyan-600">availableTools</code>에 존재하지 않으면 다음
                패턴으로 넘어갑니다. 권한 설정으로 비활성화된 도구는 자동 제외됩니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong> 각 패턴의{" "}
                <code className="text-cyan-600">argMap</code> 함수가 정규식 캡처 그룹에서 인자를
                추출합니다. 예: <code className="text-cyan-600">match[1]?.trim()</code>으로 첫 번째
                캡처 그룹의 값을 가져옵니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong> 신뢰도는 현재 0.9로 고정되어
                있습니다. 정규식 매칭 자체는 비교적 확실하지만, 의도 해석의 정확도가 100%는 아니기
                때문입니다.
              </p>
              <p>
                <strong className="text-gray-900">[4]</strong> 모든 패턴을 순회해도 매칭되지 않으면
                null을 반환합니다. 호출자는 반드시 null 체크를 수행해야 합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                <li>
                  <strong>패턴 미매칭:</strong> 7가지 패턴(read, search, write, run, list, edit,
                  find)의 동사로 시작하는 텍스트만 매칭됩니다. &quot;open file&quot;이나
                  &quot;execute command&quot; 등은 매칭되지 않습니다.
                </li>
                <li>
                  <strong>도구 미등록:</strong> 매칭된 도구가{" "}
                  <code className="text-cyan-600">availableTools</code>에 없으면 null을 반환합니다.
                  권한 설정을 확인하세요.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                패턴은 배열 순서대로 매칭됩니다. &quot;search for files&quot;는{" "}
                <code className="text-cyan-600">grep_search</code>로 매칭됩니다(
                <code className="text-cyan-600">find</code>가 아닌{" "}
                <code className="text-cyan-600">search</code>가 먼저 정의). 이는 현재 설계의
                한계입니다. 정확한 도구 호출이 필요하면 네이티브나 텍스트 파싱 전략을 사용하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">file_edit</code>처럼 old_string + new_string이
                필요한 도구는 완전히 지원되지 않습니다. 이런 경우 부족한 인자에 대해 도구 자체가
                에러를 반환합니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">regex</code>(정규식),{" "}
                <code className="text-cyan-600">tool</code>(도구 이름),
                <code className="text-cyan-600">argMap</code>(인자 추출 함수) 세 가지를 정의하면
                됩니다. 배열의 순서가 매칭 우선순위를 결정하므로, 더 구체적인 패턴을 앞에
                배치하세요.
              </p>
            </div>
          </section>
