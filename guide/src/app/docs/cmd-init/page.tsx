...
export default function CmdInitPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">/init 프로젝트 초기화</span>
            </h1>
  ...
  style AGENT fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
              ...
              params={[
                {
                  ...
                  type: "boolean",
                  required: true,
                  desc: "새로운 파일/디렉토리가 생성되었으면 true, 이미 존재하면 false",
                },
                {
                  name: "path",
                  type: "string",
            ...
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">initProject</span>(
              <span className="prop">cwd</span>: <span className="type">string</span>):{" "}
              <span className="type">Promise</span>&lt;<span className="type">InitResult</span>&gt;
            </CodeBlock>
            <ParamTable
              params={[
                {
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              /init 슬래시 명령어 정의입니다. 인자에 따라 일반 모드 또는 인터랙티브 모드로
              분기합니다.
            </p>
            <CodeBlock>
              ...
              {"                "}
              <span className="cm">{"// 일반 모드: 12단계 LLM 분석"}</span>
              {"\n"}
              <span className="str">/init -i</span>
              {"             "}
              <span className="cm">{"// 인터랙티브 모드: 4단계 대화형"}</span>
              {"\n"}
              <span className="str">/init --interactive</span>
              {"  "}
              <span className="cm">{"// 인터랙티브 모드 (풀네임)"}</span>
            </CodeBlock>
            ...
            <ParamTable
              params={[
                ...
                },
                {
                  ...
                  type: "모듈",
                  required: true,
                  desc: "12단계 코드베이스 분석 프롬프트 빌더 (7가지 분석 항목)",
                },
                {
                ...
                },
                {
                  ...
                  type: "모듈",
                  required: true,
                  desc: "4단계 대화형 초기화 플로우 (사용자 선택 → 탐색 → 질문 → 확인)",
                },
              ]}
            />
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              ...
              <li>
                CLI 모드는 LLM을 사용하지 않으므로{" "}
                <code className="text-cyan-600">template-generator.ts</code>의 정적 분석에
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <Callout type="tip" icon="*">
              <strong>팁:</strong> 대규모 프로젝트(50+ 소스 파일)에서는 인터랙티브 모드가
            ...
            <DeepDive title="생성되는 .dbcode/ 디렉토리 구조">
              ...
              <CodeBlock>
                ...
                {"    "}
                <span className="cm">{"// 모델: 기본값, 허용 도구 6개"}</span>
                {"\n"}
                {"  "}
                <span className="prop">rules/</span>
                {"\n"}
                {"    "}
                <span className="prop">.gitkeep</span>
                {"         "}
                <span className="cm">{"// 빈 디렉토리 유지"}</span>
              </CodeBlock>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              /init 명령어는 모드에 따라 다른 경로를 따릅니다. CLI 모드는 정적 템플릿을 직접
              생성하고, 세션 내 모드는 LLM에게 프롬프트를 주입합니다.
            </p>
  ...
  style INJECT fill:#dcfce7,stroke:#10b981,color:#065f46,stroke-width:2px`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              슬래시 명령어의 <code className="text-cyan-600">execute()</code> 함수는 3단계로
              작동합니다.
            </p>
            <CodeBlock>
              <span className="fn">execute</span>: <span className="kw">async</span> (
              <span className="prop">args</span>, <span className="prop">context</span>) =&gt; {"{"}
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">cwd</span> ={" "}
              <span className="prop">context</span>.<span className="prop">workingDirectory</span>;
              {"\n"}
              {"  "}
              <span className="kw">const</span> {"{"} <span className="prop">interactive</span>{" "}
              {"}"} = <span className="fn">parseInteractiveArgs</span>(
              ...
              <span className="kw">await</span> <span className="fn">ensureConfigDir</span>(
              <span className="prop">cwd</span>);
              {"\n"}
              {"  "}
              <span className="kw">await</span> <span className="fn">ensureGitignoreEntry</span>(
              <span className="prop">cwd</span>);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              <span className="prop">interactive</span>
              {"\n"}
              ...
              {"    "}
              <span className="prop">shouldInjectAsUserMessage</span>:{" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">shouldInjectAsUserMessage</strong> &mdash;
                프롬프트가 사용자 메시지로 에이전트 루프에 주입됩니다. LLM이 이 지시에 따라
                코드베이스를 분석합니다.
              </p>
              <p>
            ...
            <DeepDive title="template-generator의 15+ 프로젝트 감지 목록">
              ...
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>Node.js:</strong> package.json (이름, 스크립트, 모듈 타입, 워크스페이스)
                </li>
                <li>
                  <strong>TypeScript:</strong> tsconfig.json (strict, target, module, path aliases)
                </li>
                <li>
                  <strong>Rust:</strong> Cargo.toml (edition, workspace members, features)
                </li>
                <li>
                  <strong>Go:</strong> go.mod (모듈 경로, Go 버전)
                </li>
                <li>
                  <strong>Python:</strong> pyproject.toml (빌드 시스템: poetry/hatch/setuptools)
                </li>
                <li>
                  <strong>Java:</strong> pom.xml (Spring Boot, 모듈), build.gradle(.kts)
                </li>
                <li>
                  <strong>Ruby:</strong> Gemfile (Ruby/Rails 버전)
                </li>
                <li>
                  <strong>CI/CD:</strong> .github/workflows/, .gitlab-ci.yml, Jenkinsfile
                </li>
                <li>
                  <strong>Container:</strong> Dockerfile (베이스 이미지), docker-compose (서비스)
                </li>
                <li>
                  <strong>환경변수:</strong> .env.example (변수명, 값 제외)
                </li>
                <li>
                  <strong>모노레포:</strong> nx.json, turbo.json, lerna.json
                </li>
                <li>
                  <strong>빌드:</strong> Makefile (주요 타겟, 최대 10개)
                </li>
                <li>
                  <strong>테스트:</strong> vitest.config, jest.config, pytest.ini
                </li>
              </ul>
            </DeepDive>
          </section>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                ...
                주입하여 에이전트 루프가 작성하게 합니다. LLM이 분석을 완료하고{" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                인터랙티브 모드의 4단계 플로우는 LLM에게 &quot;각 단계에서 사용자 입력을
                기다리라&quot;고 지시하는 프롬프트입니다. LLM이 지시를 무시하고 건너뛸 수 있습니다.
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <strong>이미 존재하는 경우에만</strong> 항목을 추가합니다. 프로젝트 루트에
                .gitignore가 없으면 아무 작업도 수행하지 않습니다. <code>git init</code>으로
                .gitignore를 먼저 생성하세요.
              </p>
            </div>
          </section>
