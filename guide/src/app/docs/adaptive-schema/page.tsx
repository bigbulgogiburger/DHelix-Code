...
export default function AdaptiveSchemaPage() {
      ...
      <RevealOnScroll>
        <div style={{ marginBottom: "48px" }}>
          ...
          <p className="text-[16px] text-gray-600 leading-relaxed">
            모델 능력별 도구 스키마 축소 — LLM 성능 등급(CapabilityTier)에 따라 도구 스키마를 최적화합니다.
          </p>
        </div>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section style={{ marginBottom: "64px" }}>
          ...
          <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
            ...
            <p>
              LLM 모델마다 이해력과 토큰 처리 비용이 다릅니다. 선택적 매개변수가 많으면
              저성능 모델이 잘못된 값을 전달할 가능성이 높아지고, 토큰 비용도 증가합니다.
              이 모듈은 등급별로 스키마를 축소하여 이런 문제를 해결합니다.
            </p>
            <p>
  ...
  style TE fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
          ...
          <Callout type="info" icon="💡">
            <strong>비유:</strong> 레스토랑 메뉴판을 떠올리세요. 미식가(HIGH)에게는 전체 메뉴를 보여주고,
            일반 고객(MEDIUM)에게는 인기 메뉴 위주로 보여주며, 처음 방문한 고객(LOW)에게는
            대표 메뉴 몇 개와 주문 예시를 보여주는 것과 같습니다.
          </Callout>
        </section>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section style={{ marginBottom: "64px" }}>
          ...
          <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
            성능 등급에 맞게 적응된 도구 정보입니다. <code className="text-cyan-600">adaptToolSchema</code> 함수의 반환 타입입니다.
          </p>
          <ParamTable
            params={[
              { name: "name", type: "string", required: true, desc: "도구 이름" },
              { name: "description", type: "string", required: true, desc: "성능 등급에 맞게 축소된 설명" },
              { name: "parameters", type: "Record<string, unknown>", required: true, desc: "성능 등급에 맞게 필터링된 매개변수 JSON Schema" },
              { name: "examples", type: "readonly string[]", required: false, desc: "사용 예시 목록 (LOW 등급에서만 제공) — few-shot 프롬프팅에 사용" },
            ]}
          />
          ...
          <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
            모델 성능 등급에 따라 도구 스키마를 적응시키는 핵심 함수입니다.
            등급별로 설명 길이, 매개변수 범위, 예시 포함 여부가 달라집니다.
          </p>
          <CodeBlock>
            ...
            {"\n"}{"  "}<span className="prop">parameters</span>: <span className="type">Record</span>{"<"}<span className="type">string</span>, <span className="type">unknown</span>{">"}{","}
          ...
          <ParamTable
            params={[
              ...
              { name: "parameters", type: "Record<string, unknown>", required: true, desc: "원본 JSON Schema 매개변수 객체" },
              { name: "tier", type: "CapabilityTier", required: true, desc: '모델 성능 등급 ("high" | "medium" | "low")' },
              { name: "_workingDirectory", type: "string", required: true, desc: "작업 디렉토리 (향후 경로 기반 예시 생성에 사용 예정)" },
            ]}
          />
          ...
          <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
            ...
            <li>
              <code className="text-cyan-600">MEDIUM</code> 등급에서는 기본값(<code className="text-cyan-600">default</code>)이
              있는 선택적 매개변수가 제거됩니다. 기본값이 없는 선택적 매개변수는 중요할 수 있으므로 유지됩니다.
            </li>
            <li>
              <code className="text-cyan-600">LOW</code> 등급에서는 <code className="text-cyan-600">required</code> 배열에
              포함된 매개변수만 남습니다. <code className="text-cyan-600">required</code>가 비어 있으면 원본을 그대로 반환합니다.
            </li>
            <li>
              Few-shot 예시(<code className="text-cyan-600">TOOL_EXAMPLES</code>)는 6개 핵심 도구에만 정의되어 있습니다:
            ...
            <li>
              문장 구분은 마침표(<code className="text-cyan-600">.</code>) 뒤에 공백 또는 줄바꿈이 오는 경우에만 인식됩니다.
              URL이나 소수점 등은 문장 끝으로 인식되지 않습니다.
            </li>
          </ul>
        </section>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section style={{ marginBottom: "64px" }}>
          ...
          {/* 기본 사용법 */}
          <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>기본 사용법 &mdash; 등급별 스키마 적응</h3>
          <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
            모델의 <code className="text-cyan-600">CapabilityTier</code>를 확인한 후,
            각 도구 스키마를 해당 등급에 맞게 적응시킵니다.
          </p>
          <CodeBlock>
            <span className="cm">{"// 1. 모델의 성능 등급 확인"}</span>
            {"\n"}<span className="kw">const</span> <span className="prop">tier</span> = <span className="fn">getCapabilityTier</span>(<span className="prop">modelId</span>);
            {"\n"}
            {"\n"}<span className="cm">{"// 2. 각 도구의 스키마를 등급에 맞게 적응"}</span>
            {"\n"}<span className="kw">const</span> <span className="prop">adapted</span> = <span className="fn">adaptToolSchema</span>(
            ...
            {"\n"}<span className="fn">sendToLLM</span>({"{"} <span className="prop">tools</span>: [<span className="prop">adapted</span>] {"}"});
          </CodeBlock>

          <Callout type="warn" icon="!">
            <strong>주의:</strong> <code>adaptToolSchema</code>는 원본 스키마를 수정하지 않고
            새 객체를 반환합니다. 원본 도구 정의를 보존하면서 등급별로 다른 스키마를 생성할 수 있습니다.
          </Callout>
          ...
          <CodeBlock>
            ...
            {"\n"}<span className="kw">const</span> <span className="prop">high</span> = <span className="fn">adaptToolSchema</span>(<span className="str">&quot;file_read&quot;</span>, <span className="prop">desc</span>, <span className="prop">params</span>, <span className="str">&quot;high&quot;</span>, <span className="prop">cwd</span>);
            {"\n"}
            {"\n"}<span className="cm">{"// MEDIUM: 핵심 매개변수 + 2문장 설명"}</span>
            {"\n"}<span className="cm">{"// → 기본값 있는 선택적 매개변수 제거"}</span>
            {"\n"}<span className="kw">const</span> <span className="prop">med</span> = <span className="fn">adaptToolSchema</span>(<span className="str">&quot;file_read&quot;</span>, <span className="prop">desc</span>, <span className="prop">params</span>, <span className="str">&quot;medium&quot;</span>, <span className="prop">cwd</span>);
            {"\n"}
            {"\n"}<span className="cm">{"// LOW: 필수 매개변수만 + 1문장 + few-shot 예시"}</span>
            {"\n"}<span className="cm">{"// → examples 필드가 추가됨"}</span>
            {"\n"}<span className="kw">const</span> <span className="prop">low</span> = <span className="fn">adaptToolSchema</span>(<span className="str">&quot;file_read&quot;</span>, <span className="prop">desc</span>, <span className="prop">params</span>, <span className="str">&quot;low&quot;</span>, <span className="prop">cwd</span>);
            {"\n"}<span className="fn">console</span>.<span className="fn">log</span>(<span className="prop">low</span>.<span className="prop">examples</span>);
            {"\n"}<span className="cm">{"// → ['file_read({\"file_path\": \"/absolute/path/to/file.ts\"})']"}</span>
          </CodeBlock>
          ...
          <DeepDive title="truncateToSentences 동작 상세">
            <p className="mb-3">
              설명 텍스트를 N문장으로 자르는 내부 함수입니다. 문장 구분자는
              마침표(<code className="text-cyan-600">.</code>) 뒤에 공백이나 줄바꿈이 오는 경우입니다.
            </p>
            <ul className="list-disc list-inside space-y-1.5 text-gray-600">
              <li><code>&quot;First sentence. Second sentence. Third.&quot;</code> → 2문장 → <code>&quot;First sentence. Second sentence.&quot;</code></li>
              <li>URL 속 마침표(예: <code>&quot;api.example.com&quot;</code>)는 뒤에 공백이 없으므로 문장 끝으로 인식되지 않습니다.</li>
      ...
      <RevealOnScroll>
        <section style={{ marginBottom: "64px" }}>
          ...
          <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>등급별 스키마 축소 흐름</h3>
          <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
            <code className="text-cyan-600">adaptToolSchema</code>는 <code className="text-cyan-600">tier</code>에 따라
            서로 다른 필터 함수를 조합하여 스키마를 축소합니다.
          </p>
  ...
  style LOW_OUT fill:#fee2e2,stroke:#ef4444,color:#991b1b,stroke-width:2px`}
          ...
          <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
            ...
            <code className="text-cyan-600">filterToRequiredOnly</code>는
            JSON Schema의 <code className="text-cyan-600">properties</code>를 필터링하여
            등급에 맞는 매개변수만 남깁니다.
          </p>
          <CodeBlock>
            <span className="cm">{"// filterToCoreParams — MEDIUM 등급"}</span>
            {"\n"}<span className="cm">{"// 기본값(default)이 있는 비필수 매개변수를 제거"}</span>
            {"\n"}<span className="kw">for</span> (<span className="kw">const</span> [<span className="prop">key</span>, <span className="prop">value</span>] <span className="kw">of</span> <span className="fn">Object</span>.<span className="fn">entries</span>(<span className="prop">properties</span>)) {"{"}
            {"\n"}{"  "}<span className="kw">if</span> (<span className="prop">requiredSet</span>.<span className="fn">has</span>(<span className="prop">key</span>)) {"{"}
            {"\n"}{"    "}<span className="cm">{"// [1] 필수 매개변수 → 항상 포함"}</span>
            {"\n"}{"    "}<span className="prop">filtered</span>[<span className="prop">key</span>] = <span className="prop">value</span>;
            {"\n"}{"  "}<span className="kw">{"}"}</span> <span className="kw">else if</span> (<span className="prop">value</span> && !(<span className="str">&quot;default&quot;</span> <span className="kw">in</span> <span className="prop">value</span>)) {"{"}
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
            <p><strong className="text-gray-900">[1]</strong> <code className="text-cyan-600">required</code> 배열에 포함된 매개변수는 어떤 등급에서든 항상 유지됩니다.</p>
            <p><strong className="text-gray-900">[2]</strong> 기본값이 없는 선택적 매개변수는 의도적으로 값을 지정해야 할 수 있으므로 MEDIUM에서 유지합니다.</p>
            <p><strong className="text-gray-900">[3]</strong> 기본값이 있는 선택적 매개변수는 LLM이 생략하더라도 기본값이 자동 적용되므로 안전하게 제거할 수 있습니다.</p>
          </div>

          <DeepDive title="TOOL_EXAMPLES — few-shot 프롬프팅 상수">
            <p className="mb-3">
              LOW 등급에서만 사용되는 도구별 사용 예시입니다. 저성능 모델은
              예시가 있을 때 올바른 호출 형식을 훨씬 잘 따릅니다.
            </p>
            <CodeBlock>
              ...
              {"\n"}{"  "}],
              {"\n"}{"  "}<span className="cm">{"// ... file_write, file_edit, grep_search, glob_search"}</span>
              {"\n"}{"}"};
            </CodeBlock>
        ...
        <section style={{ marginBottom: "64px" }}>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            ...
            <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
              LOW 등급에서는 <code className="text-cyan-600">required</code> 매개변수만 스키마에 포함됩니다.
              특정 선택적 매개변수가 반드시 필요하다면, 해당 도구 정의에서 그 매개변수를
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
            ...
            <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
              <code className="text-cyan-600">truncateToSentences</code>는 마침표 기준으로 문장을 자릅니다.
              도구 설명의 첫 문장(MEDIUM) 또는 첫 문장(LOW)에 핵심 정보가 담기도록
              설명을 작성하는 것이 중요합니다.
            </p>
            <Callout type="tip" icon="*">
              도구 설명의 첫 문장은 &quot;이 도구는 무엇을 하는가&quot;를 명확히 담으세요.
              세부 사항은 두 번째 문장 이후에 적으면 됩니다.
            </Callout>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <p className="text-[13px] text-gray-600 leading-relaxed">
              <code className="text-cyan-600">TOOL_EXAMPLES</code> 상수에 도구 이름을 키로,
              예시 문자열 배열을 값으로 추가하세요. LOW 등급에서 자동으로 반환 객체의
