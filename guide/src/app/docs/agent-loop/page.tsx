...
const agentLoopConfigParams = [
  {
    ...
    type: "LLMProvider",
    required: true,
    desc: "LLM API 클라이언트 (OpenAI, Anthropic 등). chat() / stream() 메서드를 제공해야 합니다.",
  },
  {
    name: "model",
    type: "string",
    required: true,
    desc: "사용할 LLM 모델명 (예: 'gpt-4o', 'claude-sonnet-4-20250514'). dual-model 라우팅 시 기본 모델로 사용됩니다.",
  },
  {
    name: "toolRegistry",
    type: "ToolRegistry",
    required: true,
    desc: "등록된 도구 목록을 관리하는 레지스트리. Hot Tools + Deferred Loading 모드를 지원합니다.",
  },
  {
    name: "strategy",
    type: "ToolCallStrategy",
    required: true,
    desc: "도구 호출 전략 (네이티브 함수 호출 / 텍스트 파싱). 요청 준비와 응답 파싱을 담당합니다.",
  },
  {
    name: "events",
    type: "AppEventEmitter",
    required: true,
    desc: "이벤트 이미터 -- UI에 진행 상황(llm:start, tool:complete, agent:iteration 등)을 알리는 데 사용됩니다.",
  },
  {
    name: "maxIterations",
    type: "number",
    required: false,
    desc: "최대 반복 횟수. 기본값: AGENT_LOOP.maxIterations (constants.ts). 서킷 브레이커와 함께 무한 루프를 방지합니다.",
  },
  {
    name: "temperature",
    type: "number",
    required: false,
    desc: "LLM 응답의 창의성 수준 (0 = 결정적, 1 = 창의적). 기본값: 0.",
  },
  {
    name: "maxTokens",
    type: "number",
    required: false,
    desc: "LLM 응답의 최대 토큰 수. 기본값: 4096.",
  },
  {
    name: "signal",
    type: "AbortSignal",
    required: false,
    desc: "취소 신호. 사용자가 Esc를 누르면 전파되어 루프가 즉시 중단됩니다.",
  },
  {
    name: "workingDirectory",
    type: "string",
    required: false,
    desc: "작업 디렉토리 경로. 도구 실행 시 CWD로 사용됩니다.",
  },
  {
    name: "checkPermission",
    type: "(call: ExtractedToolCall) => Promise<PermissionResult>",
    required: false,
    desc: "도구 호출 전 권한 확인 콜백. 2회 이상 거절되면 해당 도구 사용 중단을 LLM에 지시합니다.",
  },
  {
    name: "maxRetries",
    type: "number",
    required: false,
    desc: "LLM 호출 재시도 횟수. 기본값: 2. transient 에러에만 적용됩니다.",
  },
  {
    name: "useStreaming",
    type: "boolean",
    required: false,
    desc: "스트리밍 모드 활성화. true이면 텍스트 델타를 실시간으로 UI에 전달합니다.",
  },
  {
    name: "maxContextTokens",
    type: "number",
    required: false,
    desc: "컨텍스트 윈도우 최대 토큰 수. 설정 시 auto-compaction이 활성화됩니다.",
  },
  {
    name: "maxToolResultChars",
    type: "number",
    required: false,
    desc: "개별 도구 결과의 최대 문자 수. 기본값: 12,000.",
  },
  {
    name: "maxToolResultTokens",
    type: "number",
    required: false,
    desc: "개별 도구 결과의 최대 토큰 수. 설정 시 문자 기반 대신 토큰 기반 잘라내기를 사용합니다.",
  },
  {
    name: "enableGuardrails",
    type: "boolean",
    required: false,
    desc: "보안 가드레일 활성화 여부. 기본값: true. false로 설정하면 입출력 가드레일을 건너뜁니다.",
  },
  {
    name: "checkpointManager",
    type: "CheckpointManager",
    required: false,
    desc: "자동 체크포인트 매니저. 파일 수정 전 백업 스냅샷을 생성합니다.",
  },
  {
    name: "sessionId",
    type: "string",
    required: false,
    desc: "세션 ID. 체크포인트 메타데이터와 컨텍스트 매니저에서 사용됩니다.",
  },
  {
    name: "thinking",
    type: "ThinkingConfig",
    required: false,
    desc: "Extended thinking 설정 (Claude 모델용). 내부 추론 토큰 예산을 제어합니다.",
  },
  {
    name: "dualModelRouter",
    type: "DualModelRouter",
    required: false,
    desc: "이중 모델 라우터. Architect/Editor 패턴으로 반복마다 모델을 자동 전환합니다.",
  },
  {
    name: "isSubagent",
    type: "boolean",
    required: false,
    desc: "서브에이전트 모드 여부. true이면 초기 반복에서 도구 호출이 없을 때 자동으로 nudge 메시지를 주입합니다.",
  },
];
...
const seeAlsoItems = [
  {
    ...
    desc: "3-Layer 토큰 관리 -- Agent Loop가 매 반복 시작 시 prepare()를 호출하여 compaction을 수행합니다.",
  },
  {
    name: "circuit-breaker.ts",
    slug: "circuit-breaker",
    relation: "sibling" as const,
    desc: "무한 루프 방지 -- Agent Loop가 매 반복 끝에 recordIteration()을 호출합니다.",
  },
  {
    name: "recovery-executor.ts",
    slug: "recovery-executor",
    relation: "sibling" as const,
    desc: "에러 유형별 복구 전략 -- LLM 호출 실패 시 findRecoveryStrategy()로 복구를 시도합니다.",
  },
  {
    name: "tools/executor.ts",
    slug: "tool-executor",
    relation: "child" as const,
    desc: "도구 실행 파이프라인 -- Agent Loop가 executeToolCall()을 호출하여 개별 도구를 실행합니다.",
  },
  {
    name: "permissions/manager.ts",
    slug: "permission-manager",
    relation: "child" as const,
    desc: "5단계 권한 결정 -- checkPermission 콜백을 통해 도구 실행 전 권한을 확인합니다.",
  },
  {
    name: "llm/client.ts",
    slug: "llm-client",
    relation: "child" as const,
    desc: "LLM API 클라이언트 -- Agent Loop가 chat()/stream()을 호출하여 LLM과 통신합니다.",
  },
  {
    name: "observation-masking.ts",
    slug: "observation-masking",
    relation: "sibling" as const,
    desc: "도구 출력 마스킹 -- 재생성 가능한 도구 출력을 마스킹하여 컨텍스트를 절약합니다.",
  },
];
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-[clamp(32px,5vw,52px)] font-black tracking-tight leading-[1.1] mt-4 mb-3">
              <span className="text-gray-900">Agent Loop</span>
            </h1>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] space-y-4 mb-8">
              <p>
                <strong className="text-cyan-600">ReAct (Reasoning + Acting)</strong>는 LLM이 단순히
                텍스트를 생성하는 것을 넘어,
                <span className="text-gray-900 font-semibold"> 사고(Reasoning)</span>와
                <span className="text-gray-900 font-semibold"> 행동(Acting)</span>을 번갈아 수행하는
                패턴입니다.
              </p>
              <p>
                일반적인 챗봇은 질문에 대해 텍스트로만 답변합니다. 하지만 ReAct 에이전트는 "파일을
                읽어야겠다"고 판단(Reasoning)하면 실제로{" "}
                <span className="font-mono text-cyan-600 text-[13px]">file_read</span> 도구를
                호출(Acting)하고, 그 결과를 바탕으로 다시 사고하여 다음 행동을 결정합니다.
              </p>
              <p>
                Agent Loop는 이 ReAct 사이클의{" "}
                <strong className="text-gray-900">메인 while 루프</strong>를 구현합니다. 매
                반복(iteration)마다 다음 단계를 거칩니다:
              </p>
              <ol className="list-decimal list-inside space-y-1 pl-2 text-gray-600">
                <li>
                  <strong className="text-gray-900">Context Prepare</strong> -- Observation Masking
                  + Auto-compaction으로 토큰 예산 관리
                </li>
                <li>
                  <strong className="text-gray-900">LLM Call</strong> -- 스트리밍/비스트리밍 모드로
                  LLM 호출 (재시도 로직 포함)
                </li>
                <li>
                  <strong className="text-gray-900">Output Parse</strong> -- 응답에서 도구 호출 추출
                  + 유효성 검증
                </li>
                <li>
                  <strong className="text-gray-900">Permission Check</strong> -- 보안 가드레일 +
                  사용자 권한 확인
                </li>
                <li>
                  <strong className="text-gray-900">Tool Execution</strong> -- 병렬/순차 그룹화 후
                  도구 실행
                </li>
                <li>
                  <strong className="text-gray-900">Circuit Check</strong> -- 무한 루프 감지 + 진행
                  상황 추적
                </li>
              </ol>
            </div>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 mb-4">
              루프의 동작을 제어하는 모든 옵션을 담고 있는 인터페이스입니다. LLM 클라이언트, 도구
              레지스트리, 이벤트 이미터 등{" "}
              <strong className="text-gray-900">5개의 필수 의존성</strong>과
            ...
            <p className="text-[14px] text-gray-600 mb-4">
              <span className="font-mono text-violet-600 text-[13px]">runAgentLoop()</span>의
              반환값입니다. 전체 대화 내역, 반복 횟수, 중단 여부, 토큰 사용량을 포함합니다.
            </p>
            <ParamTable
              params={[
                {
                  ...
                  required: true,
                  desc: "전체 대화 메시지 배열. 초기 메시지 + 루프 중 생성된 assistant/tool 메시지를 모두 포함합니다.",
                },
                {
                  name: "iterations",
                  type: "number",
                  required: true,
                  desc: "실행된 반복 횟수. 서킷 브레이커나 max-iterations에 의해 중단된 경우 마지막 반복 번호입니다.",
                },
                {
                  name: "aborted",
                  type: "boolean",
                  required: true,
                  desc: "사용자에 의해 중단(Esc)되었는지 여부. true이면 signal.aborted에 의한 조기 종료입니다.",
                },
                {
                  name: "usage",
                  type: "AggregatedUsage",
                  required: false,
                  desc: "토큰 사용량 통계. 전체 프롬프트/응답 토큰, 반복 횟수, 도구 호출 수, 재시도 횟수를 포함합니다.",
                },
              ]}
            />
            ...
            <p className="text-[14px] text-gray-600 mb-4">
              전체 루프 실행에 걸쳐 누적된 토큰 사용량과 실행 메트릭입니다. 내부적으로는{" "}
              <span className="font-mono text-violet-600 text-[13px]">UsageAggregator</span>{" "}
              클래스가 가변(mutable) 상태로 관리하지만, 외부에는{" "}
              <span className="font-mono text-[13px]">snapshot()</span>을 통해 불변 스냅샷만
              제공합니다.
            </p>
            <ParamTable
              params={[
            ...
            <p className="text-[14px] text-gray-600 mb-4">
              LLM 에러를 분류하여 재시도 전략을 결정합니다. 에러 메시지의 키워드를 분석하여 3가지
              유형으로 분류합니다:
            </p>
              ...
              <div className="space-y-3 text-[13px]">
                <div className="flex gap-3">
                  ...
                  <span className="text-gray-600">
                    일시적 에러 (timeout, ECONNRESET, 500, 502, 504, network). 지수 백오프로
                    재시도합니다.
                  </span>
                </div>
                <div className="flex gap-3">
                  <span className="font-mono text-amber-600 font-bold shrink-0">
                    {'"overload"'}
                  </span>
                  <span className="text-gray-600">
                    과부하 에러 (429, 503, rate limit, capacity). 클라이언트가 이미 Retry-After로
                    재시도했으므로 추가 재시도하지 않습니다.
                  </span>
                </div>
                <div className="flex gap-3">
                  <span className="font-mono text-red-600 font-bold shrink-0">{'"permanent"'}</span>
                  <span className="text-gray-600">
                    영구적 에러 (Request too large, too many tokens, 기타). 재시도해도 소용없으므로
                    즉시 실패합니다.
                  </span>
                </div>
              </div>
            ...
            <Callout type="danger" icon="🚨">
              <strong>주의: "Request too large"는 permanent입니다.</strong> -- 이 에러가 발생하면
              동일한 페이로드로 재시도해도 실패합니다.
              <span className="font-mono text-[12px]">maxContextTokens</span>를 설정하여
              auto-compaction을 활성화하거나,
              <span className="font-mono text-[12px]">maxToolResultChars</span>/
              <span className="font-mono text-[12px]">maxToolResultTokens</span>를 줄여야 합니다.
            </Callout>

            <Callout type="info" icon="📌">
              <strong>PermissionResult 인터페이스</strong> --
              <span className="font-mono text-[13px]">checkPermission</span> 콜백의 반환값입니다.
              <span className="font-mono text-cyan-600 text-[13px]">{`{ allowed: boolean, reason?: string }`}</span>{" "}
              형태이며,
              <span className="font-mono text-[13px]">allowed: false</span>일 때 거부 사유를 LLM에
              전달합니다. 같은 도구가 2회 이상 거절되면 "이 도구 사용을 중단하라"는 가이드를 LLM에
              주입합니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-[1.85] mb-4">
              ...
              사용자 입력을 받아{" "}
              <span className="font-mono text-violet-600 text-[13px]">runAgentLoop()</span>를
              호출합니다. 훅은 이벤트 이미터를 통해 UI 상태를 업데이트합니다.
            </p>
...
// result.usage -- 토큰 사용량`}</CodeBlock>
    ...
    AL->>LLM: 다음 반복으로 결과 전달`}
            ...
            <Callout type="warn" icon="⚠️">
              <strong>Pre-flight 검사는 순차적입니다.</strong> -- 권한 확인(checkPermission)은
              사용자에게 승인을 요청할 수 있으므로 순차적으로 실행됩니다. 하지만 실제 도구 실행은
              그룹 내에서 <span className="font-mono text-[13px]">Promise.allSettled()</span>로 병렬
              실행됩니다.
            </Callout>
              ...
              <div className="space-y-4 text-[13px]">
                <div>
                  ...
                  <p className="text-gray-600">
                    <span className="font-mono text-violet-600">findRecoveryStrategy(error)</span>로
                    에러에 맞는 복구 전략을 찾습니다. 복구 전략이 있으면{" "}
                    <span className="font-mono text-violet-600">executeRecovery()</span>를
                    실행합니다. 복구가 "retry"를 반환하면 compacted messages와 함께 반복을
                    재시작합니다.
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-gray-600">
                    <span className="font-mono text-violet-600">classifyLLMError(error)</span>로
                    에러를 분류합니다. "overload"와 "permanent"는 즉시 throw합니다. "transient"만
                    다음 단계로 진행합니다.
                  </p>
                </div>
                <div>
            ...
            <Callout type="danger" icon="🔥">
              <strong>Pitfall: overload 에러에 재시도하지 마세요.</strong> -- 429/503 에러는 OpenAI
              SDK가 이미 Retry-After 헤더를 기반으로 재시도합니다. Agent Loop가 추가로 재시도하면
              이중 재시도가 발생하여 rate limit을 더 악화시킵니다.
            </Callout>

            <DeepDive title="Tool Grouping 전략">
              <div className="space-y-4">
                <p>
                  LLM이 한 번의 응답에서 여러 도구를 호출할 수 있습니다. Agent Loop는 이를{" "}
                  <strong>병렬 실행 그룹</strong>으로 분류하여 최대한 동시에 실행합니다.
                </p>
                  ...
                  <div className="flex gap-2 items-start">
                    ...
                    <span>
                      <span className="font-mono text-[12px]">
                        glob_search, grep_search, file_read
                      </span>{" "}
                      -- 읽기 전용이므로 항상 병렬 실행 가능
                    </span>
                  </div>
                  <div className="flex gap-2 items-start">
                    <span className="text-amber-600 font-bold shrink-0">FILE_WRITE:</span>
                    <span>
                      <span className="font-mono text-[12px]">file_write, file_edit</span> -- 같은
                      파일 경로 대상이면 순차, 다른 파일이면 병렬
                    </span>
                  </div>
                  <div className="flex gap-2 items-start">
                    <span className="text-blue-600 font-bold shrink-0">BASH_EXEC:</span>
                    <span>
                      <span className="font-mono text-[12px]">bash_exec</span> -- 서로 독립적이므로
                      병렬 실행 가능
                    </span>
                  </div>
                </div>
...
// Group 1은 병렬 실행, Group 2는 Group 1 완료 후 실행`}</CodeBlock>
                ...
                <Callout type="tip" icon="✅">
                  ...
                  <span className="font-mono text-[12px]"> file_path, path, filePath</span> 3가지
                  파라미터명을 시도합니다. 커스텀 도구에서 다른 이름을 사용하면 충돌 감지가 되지
                  않을 수 있습니다.
                </Callout>
              </div>
            </DeepDive>
            ...
            <DeepDive title="Observation Masking">
              <div className="space-y-4">
                <p>
                  ...
                  <span className="font-mono text-cyan-600 text-[12px]">
                    {" "}
                    applyObservationMasking()
                  </span>
                  은 최근 N개(기본 5개)를 제외한 이전 도구 결과를 요약/마스킹하여 토큰을 절약합니다.
                </p>
                <p>
                ...
                <ol className="list-decimal list-inside space-y-1 pl-2">
                  ...
                  <li>
                    Context Manager prepare() (고비용, 비동기) -- 토큰 예산 초과 시 compaction
                  </li>
                </ol>
                <Callout type="info" icon="💡">
                  Masking은 <strong>비파괴적</strong>입니다. 원본 messages 배열을 수정하지 않고
                  새로운 배열을 반환합니다. 따라서 이전 반복의 도구 결과를 나중에 참조해야 할 때
                  원본이 보존됩니다.
                </Callout>
              </div>
            </DeepDive>
            ...
            <Callout type="warn" icon="⚠️">
              <strong>Pitfall: Deferred 도구 스키마 누락</strong> -- MCP 도구는 초기에 스키마를
              로드하지 않습니다(Deferred Loading).
              <span className="font-mono text-[12px]"> resolveDeferredFromHistory()</span>가 최근
              3개 assistant 메시지에서 사용된 MCP 도구의 스키마를 자동으로 해석합니다. 하지만 새로운
              MCP 도구를 처음 호출할 때는 Hot Tools 목록에만 의존합니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-[1.85] mb-6">
              Agent Loop의 각 반복(iteration)은 아래 상태 머신을 따릅니다. 종료 조건은 4가지입니다:
              도구 호출 없음(정상 완료), 최대 반복 도달, 사용자 중단(Esc), 서킷 브레이커 발동.
            </p>
  ...
  return { messages, iterations, aborted: false, usage: finalUsage };
            ...
            <p className="text-[14px] text-gray-600 leading-[1.85] mb-4">
              Agent Loop에는 3가지 <strong className="text-gray-900">HeadlessGuard</strong>가
              내장되어 있습니다. headless(UI 없는) 환경에서 LLM이 비정상 동작할 때 자동으로
              대응합니다:
            </p>
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              <div className="space-y-4 text-[13px]">
                <div>
                  <div className="font-bold text-amber-600 mb-1">
                    1. 빈 응답 감지 (Empty Response Guard)
                  </div>
                  <p className="text-gray-600">
                    content가 빈 문자열이고 도구 호출도 없을 때 최대{" "}
                    <span className="font-mono">2회</span> 자동 재시도합니다. "[System] Your
                    previous response was empty..." 메시지를 주입하여 LLM을 nudge합니다.
                  </p>
                </div>
                <div>
                  <div className="font-bold text-amber-600 mb-1">
                    2. 중복 도구 호출 감지 (Duplicate Tool Call Guard)
                  </div>
                  <p className="text-gray-600">
                    동일한 도구+파라미터 조합이 <span className="font-mono">3회 연속</span> 발생하면
                    루프를 탈출합니다. 도구 호출의 시그니처(이름+JSON 인자)를 해싱하여 비교합니다.
                  </p>
                </div>
                <div>
                  <div className="font-bold text-amber-600 mb-1">
                    3. Incomplete 응답 감지 (Incomplete Response Guard)
                  </div>
                  <p className="text-gray-600">
                    Responses API가 "incomplete" 상태를 반환하면 최대{" "}
                    <span className="font-mono">2회</span> 재시도합니다. Codex 모델이 early-stop하는
                    경우에 대응합니다.
                  </p>
                </div>
              </div>
            ...
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              Permission Denial 추적
            </h3>
            <p className="text-[14px] text-gray-600 leading-[1.85] mb-4">
              같은 도구가 반복적으로 권한이 거절되면 LLM이 계속 같은 도구를 시도하는 무한 루프에
              빠질 수 있습니다. Agent Loop는 도구별 거절 횟수를 추적하여{" "}
              <span className="font-mono text-red-600 text-[13px]">2회 이상</span> 거절되면 "이 도구
              사용을 중단하고 사용자에게 가이드를 요청하라"는 메시지를 주입합니다.
            </p>
...
denialCount >= MAX_DENIALS_BEFORE_STOP
  ...
  : \`Permission denied: \${permission.reason ?? "User rejected"}\`;`}</CodeBlock>
            ...
            <Callout type="tip" icon="✅">
              ...
              <span className="font-mono text-violet-600 text-[12px]">
                {" "}
                checkpointManager.createCheckpoint()
              </span>
              가 호출됩니다. 파일 수정이 잘못되면{" "}
              <span className="font-mono text-[12px]">/undo</span> 명령으로 복구할 수 있습니다.
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                <h4 className="font-bold text-[15px] text-red-600 mb-2">
                  Q1. Agent가 같은 작업을 무한 반복합니다
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> LLM이 같은 도구를 동일한
                  파라미터로 반복 호출하고 있습니다. 서킷 브레이커가 감지하지 못하는 미묘한 변화(예:
                  파일 내용이 매번 동일하게 변경)가 있을 수 있습니다.
                </p>
                ...
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  ...
                  <li>
                    HeadlessGuard의 중복 감지(MAX_DUPLICATE_TOOL_CALLS = 3)가 동작하는지 확인하세요
                  </li>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                <h4 className="font-bold text-[15px] text-red-600 mb-2">
                  Q2. 도구 실행 권한이 계속 거부됩니다
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> 권한 모드가 restrictive하게
                  설정되어 있거나, 사용자가 도구 실행을 거부하고 있습니다.
                </p>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <strong className="text-gray-900">해결:</strong>
                </p>
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  <li>
                    Shift+Tab을 눌러 권한 모드를 순회하세요 (5단계: deny-all → ask-all → ask-writes
                    → auto → trust-all)
                  </li>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                <h4 className="font-bold text-[15px] text-red-600 mb-2">
                  Q3. "Request too large" 에러가 발생합니다
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> 대화 히스토리 + 도구 결과의 총
                  토큰이 모델의 컨텍스트 윈도우를 초과했습니다. 이 에러는 permanent로 분류되어
                  재시도하지 않습니다.
                </p>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <strong className="text-gray-900">해결:</strong>
                ...
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  <li>
                    <span className="font-mono text-[12px]">maxContextTokens</span>를 설정하여
                    auto-compaction을 활성화하세요 (모델 컨텍스트의 ~80%)
                  </li>
                  <li>
                    <span className="font-mono text-[12px]">maxToolResultChars</span>를 줄여 개별
                    도구 결과 크기를 제한하세요
                  </li>
                  <li>
                    <span className="font-mono text-[12px]">/compact</span> 명령으로 수동
                    compaction을 실행하세요
                  </li>
                  <li>
                    새 세션(<span className="font-mono text-[12px]">/clear</span>)을 시작하세요
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                ...
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> 네트워크 문제이거나, LLM 서버의
                  응답이 너무 느립니다. 이 에러는 transient으로 분류되어 자동 재시도됩니다.
                </p>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <strong className="text-gray-900">해결:</strong>
                </p>
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  <li>
                    기본 타임아웃은 120초입니다 (
                    <span className="font-mono text-[12px]">config/defaults.ts</span>)
                  </li>
                  <li>로컬 LLM을 사용 중이면 GPU 메모리와 모델 크기를 확인하세요</li>
                  <li>
                    <span className="font-mono text-[12px]">maxRetries</span>를 늘려 재시도 횟수를
                    증가시키세요
                  </li>
                  <li>
                    스트리밍 모드(<span className="font-mono text-[12px]">useStreaming: true</span>
                    )를 사용하면 partial content를 살릴 수 있습니다
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                <h4 className="font-bold text-[15px] text-red-600 mb-2">
                  Q5. 서킷 브레이커가 너무 빨리 발동됩니다
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> CircuitBreaker가 연속된 반복에서
                  "진행 없음"(no progress)을 감지하여 루프를 중단했습니다. 파일 수정 없이 같은
                  에러가 반복되면 발동됩니다.
                </p>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <strong className="text-gray-900">해결:</strong>
                ...
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  <li>
                    <span className="font-mono text-[12px]">maxIterations</span>를 늘려 서킷
                    브레이커의 기준을 완화하세요
                  </li>
                  <li>도구 실행 에러가 있다면 근본 원인을 해결하세요 (에러 로그 확인)</li>
                  <li>
                    서킷 브레이커 상태는 <span className="font-mono text-[12px]">getStatus()</span>
                    로 확인할 수 있습니다 (reason 필드)
                  </li>
                  <li>
                    대화를 재시작(<span className="font-mono text-[12px]">/clear</span>)하면 서킷
                    브레이커가 리셋됩니다
                  </li>
                </ul>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-6">
                <h4 className="font-bold text-[15px] text-red-600 mb-2">
                  Q6. 도구 호출 인자가 불완전합니다 (JSON 파싱 실패)
                </h4>
                <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                  <strong className="text-gray-900">원인:</strong> 스트리밍 모드에서 스트림이 중간에
                  끊겨 도구 호출 인자가 불완전한 JSON이 되었습니다.
                  <span className="font-mono text-[12px]">filterValidToolCalls()</span>가 이를
                  감지하여 제거합니다.
                </p>
                <p className="text-[13px] text-gray-600 leading-relaxed">
                  <strong className="text-gray-900">해결:</strong>
                ...
                <ul className="list-disc list-inside text-[13px] text-gray-600 pl-2 space-y-1">
                  <li>
                    자동으로 "[System] Your tool calls had invalid JSON arguments" 피드백이
                    주입됩니다
                  </li>
                  ...
                  <li>
                    비스트리밍 모드(
                    <span className="font-mono text-[12px]">useStreaming: false</span>)를
                    시도해보세요
                  </li>
                </ul>
              </div>
