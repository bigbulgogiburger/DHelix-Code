...
export default function ToolFileEditPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">file_edit Tool</span>
            </h1>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">file_edit</code>는 파일의 특정 부분만 정밀하게
                수정하는 도구입니다. 전체 파일을 덮어쓰는{" "}
                <code className="text-cyan-600">file_write</code>와 달리, 정확한 문자열 매칭으로
                원하는 부분만 안전하게 교체합니다.
              </p>
              <p>
                ...
                <code className="text-cyan-600"> replace_all: true</code>를 명시적으로 설정해야
                합니다.
              </p>
              <p>
                편집 후에는 변경 위치 주변의 컨텍스트 라인(+-3줄)을 metadata에 포함하여 UI에서 diff
                미리보기를 보여줄 수 있습니다. 권한 수준은
                <code className="text-amber-600"> &quot;confirm&quot;</code>으로, 파일을 변경하므로
                사용자 확인이 필요합니다.
              </p>
            </div>
  ...
  style FS fill:#f1f5f9,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 문서의 &quot;찾기 및 바꾸기&quot;를 떠올리세요. 하지만 일반
              에디터와 달리, 찾으려는 텍스트가 문서에 딱 하나만 있을 때만 교체를 허용합니다. 여러
              곳에 있으면 &quot;어디를 바꿀지 모호합니다&quot;라고 알려주어 실수를 방지합니다.
            </Callout>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                { name: "path", type: "string", required: true, desc: "편집할 파일 경로" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "찾을 문자열 — 공백/들여쓰기 포함 정확히 일치해야 함",
                },
                {
                  name: "new_string",
                  type: "string",
                  required: true,
                  desc: "교체할 문자열 — old_string을 이 문자열로 바꿈",
                },
                {
                  name: "replace_all",
                  type: "boolean",
                  required: false,
                  desc: "모든 발생 위치를 교체할지 여부 (기본값: false)",
                },
              ]}
            />
            ...
            <ParamTable
              params={[
                ...
                { name: "linesRemoved", type: "number", required: true, desc: "삭제된 줄 수" },
                {
                  ...
                  type: "string[]",
                  required: true,
                  desc: "변경 위치 주변 +-3줄의 코드",
                },
                {
                  name: "contextStartLine",
                  type: "number",
                  required: true,
                  desc: "컨텍스트 시작 줄 번호 (1-based)",
                },
              ]}
            />
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              ...
              <li>
                ...
                <code className="text-cyan-600"> old_string</code>이 2개 이상 존재하면 에러를
                반환합니다. 더 많은 주변 컨텍스트를 포함하여 고유하게 만드세요.
              </li>
              <li>
                반드시 <code className="text-cyan-600">file_read</code>로 파일 내용을 먼저 확인한 후
                사용하세요. 읽지 않고 수정하면 들여쓰기나 공백이 맞지 않아 매칭 실패할 수 있습니다.
              </li>
              <li>
                편집 후 <code className="text-cyan-600">buildImportHint()</code>가 수정된 파일을
                import하는 다른 파일 목록을 안내합니다. 인터페이스 변경 시 관련 파일도 확인하세요.
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="kw">await</span> <span className="fn">fileRead</span>({"{"}{" "}
              <span className="prop">path</span>:{" "}
              <span className="str">&quot;src/config.ts&quot;</span> {"}"});
              {"\n"}
              {"\n"}
              <span className="cm">{"// 2. 고유한 문자열을 찾아 교체"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">result</span> ={" "}
              <span className="kw">await</span> <span className="fn">fileEdit</span>({"{"}
              {"\n"}
              {"  "}
              <span className="prop">path</span>:{" "}
              <span className="str">&quot;src/config.ts&quot;</span>,{"\n"}
              ...
              <span className="str">&quot;const MAX_RETRIES = 3;&quot;</span>,{"\n"}
              {"  "}
              <span className="prop">new_string</span>:{" "}
              <span className="str">&quot;const MAX_RETRIES = 5;&quot;</span>,{"\n"}
              {"}"});
              {"\n"}
              {"\n"}
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>old_string</code>은 반드시 파일에 존재하는{" "}
              <strong>정확한</strong> 문자열이어야 합니다. 공백이나 탭 하나라도 다르면 &quot;String
              not found&quot; 에러가 발생합니다.
            </Callout>
            ...
            <CodeBlock>
              ...
              {"  "}
              <span className="prop">path</span>:{" "}
              <span className="str">&quot;src/utils.ts&quot;</span>,{"\n"}
              {"  "}
              <span className="prop">old_string</span>:{" "}
              <span className="str">&quot;oldVarName&quot;</span>,{"\n"}
              {"  "}
              <span className="prop">new_string</span>:{" "}
              <span className="str">&quot;newVarName&quot;</span>,{"\n"}
              {"  "}
            ...
            <Callout type="tip" icon="*">
              <strong>팁:</strong> <code>replace_all</code>은 내부적으로{" "}
              <code>split(old).join(new)</code> 방식으로 모든 발생 위치를 한 번에 교체합니다.
              정규식이 아닌 정확한 문자열 매칭입니다.
            </Callout>

            <DeepDive title="고유성 검사의 동작 방식">
              <p className="mb-3">
                <code className="text-cyan-600">replace_all: false</code>(기본값)일 때, 내부적으로
                <code className="text-cyan-600"> indexOf()</code>를 두 번 호출하여 고유성을
                확인합니다:
              </p>
              <CodeBlock>
                <span className="cm">{"// 첫 번째 발견 위치"}</span>
                {"\n"}
                <span className="kw">const</span> <span className="prop">firstIdx</span> ={" "}
                <span className="prop">content</span>.<span className="fn">indexOf</span>(
                <span className="prop">old_string</span>);
                {"\n"}
                <span className="cm">{"// 두 번째 발견 위치 (firstIdx + 1부터 검색)"}</span>
                {"\n"}
                <span className="kw">const</span> <span className="prop">secondIdx</span> ={" "}
                <span className="prop">content</span>.<span className="fn">indexOf</span>(
                <span className="prop">old_string</span>, <span className="prop">firstIdx</span> +{" "}
              ...
              <p className="mt-3 text-gray-600">
                여러 곳에 존재할 때는 <strong>주변 컨텍스트를 더 포함</strong>하여{" "}
                <code className="text-cyan-600">old_string</code>을 고유하게 만들거나,{" "}
                <code className="text-cyan-600">replace_all: true</code>를 사용하세요.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style RESULT fill:#dcfce7,stroke:#10b981,color:#065f46`}
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">lineNumber</span> ={" "}
              <span className="prop">content</span>.<span className="fn">slice</span>(
              <span className="num">0</span>, <span className="prop">firstIdx</span>).
              ...
              <span className="prop">params</span>.<span className="prop">replace_all</span>
              {"\n"}
              {"  "}? <span className="prop">content</span>.<span className="fn">split</span>(
              <span className="prop">old_string</span>).<span className="fn">join</span>(
              <span className="prop">new_string</span>){"\n"}
              {"  "}: <span className="prop">content</span>.<span className="fn">replace</span>(
              <span className="prop">old_string</span>, <span className="prop">new_string</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// [3] 파일에 쓰기"}</span>
              {"\n"}
              <span className="kw">await</span> <span className="fn">writeFile</span>(
              <span className="prop">filePath</span>, <span className="prop">updated</span>,{" "}
              <span className="str">&quot;utf-8&quot;</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// [4] 변경 위치 주변 +-3줄 컨텍스트 추출"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">startLine</span> ={" "}
              <span className="fn">Math.max</span>(<span className="num">0</span>,{" "}
              <span className="prop">lineNumber</span> - <span className="num">1</span> -{" "}
              <span className="num">3</span>);
              {"\n"}
              <span className="kw">const</span> <span className="prop">endLine</span> ={" "}
              <span className="fn">Math.min</span>(<span className="prop">lines</span>.
              <span className="prop">length</span>, <span className="prop">lineNumber</span> -{" "}
              <span className="num">1</span> + <span className="prop">linesAdded</span> +{" "}
              <span className="num">3</span>);
            </CodeBlock>

            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">[1]</strong> 첫 번째 발견 위치 앞의 줄바꿈 개수를
                세어 줄 번호를 계산합니다 (1-based).
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong>{" "}
                <code className="text-cyan-600">replace_all</code>이면 <code>split().join()</code>{" "}
                방식으로 모든 위치를 교체하고, 아니면 <code>String.replace()</code>로 첫 번째만
                교체합니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong> 교체된 전체 내용을 UTF-8로 파일에
                씁니다.
              </p>
              <p>
                <strong className="text-gray-900">[4]</strong> 변경 위치 앞뒤 3줄을 추출하여 UI에서
                diff 미리보기를 보여줄 수 있도록 합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                <code className="text-cyan-600">old_string</code>이 파일 내용과 정확히 일치하지
                않습니다. 들여쓰기(탭 vs 스페이스), 줄바꿈 문자, 후행 공백을 확인하세요.
              </p>
              <Callout type="tip" icon="*">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                <code className="text-cyan-600">old_string</code>이 파일에 여러 번 존재합니다. 두
                가지 해결 방법이 있습니다:
              </p>
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                <li>
                  <strong>주변 코드를 포함:</strong>{" "}
                  <code className="text-cyan-600">old_string</code>에 앞뒤 줄을 더 포함하여 고유하게
                  만드세요.
                </li>
                <li>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">buildImportHint()</code>가 수정된 파일을 import하는
                다른 파일을 찾아 안내합니다. 인터페이스나 export를 변경했다면 안내된 파일들도 함께
                수정해야 할 수 있습니다. 이는 정보 제공 목적이며, 에러가 아닙니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
