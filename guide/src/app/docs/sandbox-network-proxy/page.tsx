...
export default function SandboxNetworkProxyPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">네트워크 프록시</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="leaf" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              네트워크 정책을 적용하는 HTTP 프록시 서버입니다. 샌드박스 프로세스의 네트워크 트래픽을
              검사하고, 허용된 호스트만 통과시킵니다.
              <code className="text-cyan-600 ml-1">sandboxed-network.ts</code>와 함께 정책 기반
              샌드박스 실행을 구성합니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">network-proxy.ts</code>는 로컬(127.0.0.1)에서 HTTP
                프록시 서버를 시작하여 샌드박스 프로세스의 네트워크 트래픽을 제어합니다. HTTP 요청은
                직접 전달(forwarding)하고, HTTPS 요청은 CONNECT 메서드를 통한 TCP 터널링으로
                처리합니다. TLS 내용은 검사하지 않습니다(투명 터널).
              </p>
              <p>
                <code className="text-cyan-600">sandboxed-network.ts</code>는 이 프록시와 기존
                샌드박스 (Seatbelt/Bubblewrap)를 통합하는 모듈입니다. 네트워크 정책이 설정되면
                프록시를 시작하고,
                <code className="text-cyan-600">HTTP_PROXY</code>/
                <code className="text-cyan-600">HTTPS_PROXY</code>
                환경 변수를 설정하여 샌드박스 프로세스가 프록시를 경유하도록 합니다. 실행 완료 후
                프록시는 <code className="text-cyan-600">finally</code>에서 반드시 정리됩니다.
              </p>
              <p>
                정책이 없거나 &quot;모두 허용&quot;인 경우, 프록시 오버헤드 없이 직접 샌드박스를
                실행합니다. 이를 통해 불필요한 성능 저하를 방지합니다.
              </p>
            </div>
  ...
  style BLOCKED fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 공항의 세관 검사소를 떠올리세요. 모든 나가는 짐(네트워크
              요청)이 검사소(프록시)를 통과해야 하고, 허용 목록에 있는 목적지만 통과시키며, 차단된
              목적지는 돌려보냅니다(403 Forbidden). 검사소가 없는 공항(정책 없음)에서는 바로
              탑승합니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
              ...
              <ParamTable
                params={[
                  ...
                  },
                  {
                    ...
                    required: false,
                    desc: "연결이 차단될 때 호출되는 콜백 (모니터링/로깅용)",
                  },
                ]}
              />
                ...
                params={[
                  {
                    ...
                    type: "number",
                    required: true,
                    desc: "프록시가 실제로 리스닝 중인 포트 번호",
                  },
                  {
                    name: "stop",
                    type: "() => Promise<void>",
                    required: true,
                    desc: "프록시를 종료하고 리소스를 정리하는 함수",
                  },
                ]}
              />
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
                ...
                params={[
                  {
                    ...
                    type: "-",
                    required: true,
                    desc: "SandboxConfig의 모든 프로퍼티 (command, args, projectDir 등)",
                  },
                  {
                    name: "networkPolicy",
                    type: "NetworkPolicy | undefined",
                    required: false,
                    desc: "적용할 네트워크 정책 (생략하면 기본 정책)",
                  },
                ]}
              />
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                네트워크 정책을 적용하여 샌드박스 안에서 명령을 실행합니다. 정책이 없거나 &quot;모두
                허용&quot;이면 프록시 없이 직접 실행합니다.
              </p>
              <CodeBlock>
                <span className="kw">async function</span>{" "}
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                프록시는 <code className="text-cyan-600">127.0.0.1</code>에서만 리스닝합니다. 외부
                네트워크에서 프록시에 직접 접근할 수 없습니다.
              </li>
              <li>
                HTTPS 요청은 CONNECT 터널링으로 처리됩니다. TLS 내용은 복호화하지 않으므로, 호스트
                이름으로만 정책을 적용합니다.
              </li>
              <li>
                포트 <code className="text-cyan-600">0</code>을 지정하면 OS가 사용 가능한 포트를
                자동 할당합니다.
                <code className="text-cyan-600">ProxyHandle.port</code>에서 실제 포트를 확인하세요.
              </li>
              <li>
                <code className="text-cyan-600">stop()</code> 호출 시
                <code className="text-cyan-600">closeAllConnections()</code>으로 남아있는 연결을
                강제 종료합니다.
              </li>
              <li>
                <code className="text-cyan-600">executeSandboxedWithNetwork()</code>은 차단된 호스트
                목록을 에러 컨텍스트에 포함시켜 디버깅을 돕습니다.
              </li>
              <li>
                <code className="text-cyan-600">HTTP_PROXY</code>와{" "}
                <code className="text-cyan-600">http_proxy</code>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              기본 사용법 &mdash; 네트워크 정책 적용 샌드박스 실행
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              가장 일반적인 패턴입니다.{" "}
              <code className="text-cyan-600">executeSandboxedWithNetwork()</code>가 프록시 시작,
              환경 변수 설정, 샌드박스 실행, 프록시 정리를 모두 자동으로 처리합니다.
            </p>
            <CodeBlock>
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>networkPolicy</code>를 생략하거나 기본 &quot;모두
              허용&quot; 정책이면 프록시를 시작하지 않고 직접 샌드박스를 실행합니다. 반드시 정책을
              설정해야 네트워크 제어가 적용됩니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">startNetworkProxy()</code>로 프록시를 직접 시작하고
              관리할 수 있습니다. 여러 프로세스가 동일한 프록시를 공유하거나, 차단 이벤트를
              모니터링할 때 유용합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// 차단 이벤트 모니터링"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">blockedHosts</span>:{" "}
              <span className="type">string</span>[] = [];
              {"\n"}
              {"\n"}
              <span className="kw">const</span> <span className="prop">proxy</span> ={" "}
              <span className="kw">await</span> <span className="fn">startNetworkProxy</span>({"{"}
              {"\n"}
              {"  "}
              <span className="prop">port</span>: <span className="num">0</span>,{" "}
              <span className="cm">{"// OS가 포트 자동 할당"}</span>
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"    "}
              <span className="prop">blockedHosts</span>.<span className="fn">push</span>(
              <span className="prop">host</span>);
              {"\n"}
              {"    "}
              ...
              <span className="str">`프록시 포트: ${"{"}</span>
              ...
              {"\n"}
              {"  "}
              <span className="kw">await</span> <span className="prop">proxy</span>.
            ...
            <DeepDive title="HTTP_PROXY 환경 변수 호환성">
              ...
              <CodeBlock>
                <span className="prop">HTTP_PROXY</span> ={" "}
                <span className="str">&quot;http://127.0.0.1:54321&quot;</span>{" "}
                <span className="cm">{"// 대문자 (표준)"}</span>
                {"\n"}
                <span className="prop">HTTPS_PROXY</span> ={" "}
                <span className="str">&quot;http://127.0.0.1:54321&quot;</span>{" "}
                <span className="cm">{"// 대문자 (표준)"}</span>
                {"\n"}
                <span className="prop">http_proxy</span> ={" "}
                <span className="str">&quot;http://127.0.0.1:54321&quot;</span>{" "}
                <span className="cm">{"// 소문자 (일부 도구)"}</span>
                {"\n"}
                <span className="prop">https_proxy</span> ={" "}
                <span className="str">&quot;http://127.0.0.1:54321&quot;</span>{" "}
                <span className="cm">{"// 소문자 (일부 도구)"}</span>
              ...
              <p className="mt-3 text-amber-600">
                ...
                <code className="text-cyan-600">Node.js</code>의 일부 HTTP 라이브러리는 대문자를
                사용합니다. 양쪽 모두 설정하여 최대 호환성을 확보합니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style BLOCK2 fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              HTTPS 연결은 CONNECT 메서드를 통해 TCP 터널을 수립합니다. 프록시는 TLS 내용을
              복호화하지 않고 양방향으로 데이터를 전달합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// CONNECT 처리 핵심 로직"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">serverSocket</span> ={" "}
              <span className="fn">connect</span>(<span className="prop">targetPort</span>,{" "}
              <span className="prop">targetHost</span>, () =&gt; {"{"}
              {"\n"}
              {"  "}
              <span className="cm">{"// 1. 클라이언트에게 터널 수립 성공 알림"}</span>
              {"\n"}
              {"  "}
              <span className="prop">clientSocket</span>.<span className="fn">write</span>(
              <span className="str">
              ...
              {"\n"}
              {"  "}
              <span className="prop">serverSocket</span>.<span className="fn">write</span>(
              <span className="prop">head</span>);
              {"\n"}
              {"  "}
              <span className="cm">{"// 3. 양방향 파이프 설정"}</span>
              {"\n"}
              {"  "}
              <span className="prop">serverSocket</span>.<span className="fn">pipe</span>(
              <span className="prop">clientSocket</span>);
              {"\n"}
              {"  "}
              <span className="prop">clientSocket</span>.<span className="fn">pipe</span>(
              <span className="prop">serverSocket</span>);
              {"\n"}
              {"}"});
            </CodeBlock>

            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              정책 통합 실행 플로우 (sandboxed-network.ts)
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">executeSandboxedWithNetwork()</code>의 전체 실행
              과정입니다.
            </p>
            <CodeBlock>
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (!<span className="prop">networkPolicy</span>){" "}
              <span className="kw">return</span> <span className="fn">executeSandboxed</span>(...);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="prop">isDefaultAllow</span>){" "}
              <span className="kw">return</span> <span className="fn">executeSandboxed</span>(...);
              {"\n"}
              {"\n"}
              {"  "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">[1-2] 최적화:</strong> 네트워크 제어가 불필요한
                경우 프록시를 시작하지 않아 성능 오버헤드를 제거합니다.
              </p>
              <p>
                <strong className="text-gray-900">[6] 디버깅:</strong> 차단된 호스트 목록(
                <code className="text-cyan-600">blockedHosts</code>)을 에러 컨텍스트에 포함시켜,
                어떤 도메인이 차단되었는지 확인할 수 있습니다.
              </p>
              <p>
                <strong className="text-gray-900">[7] 리소스 정리:</strong>{" "}
                <code className="text-cyan-600">finally</code>에서 프록시를 반드시 종료하여 포트
                누수를 방지합니다.
              </p>
            </div>
          </section>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                실행하는 도구가 <code className="text-cyan-600">HTTP_PROXY</code> 환경 변수를
                지원하는지 확인하세요. 일부 도구(예: <code className="text-cyan-600">go</code>{" "}
                바이너리)는 프록시 환경 변수를 무시합니다.
              </p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                샌드박스 내부의 명령이 차단된 도메인에 접근을 시도했습니다. 에러의{" "}
                <code className="text-cyan-600">blockedHosts</code> 필드에서 어떤 도메인이
                차단되었는지 확인하고, 필요한 도메인을
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">port: 0</code>을 사용하여 OS가 자동으로 사용 가능한
                포트를 할당하도록 하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">stop()</code>은{" "}
                <code className="text-cyan-600">closeAllConnections()</code>을 호출하여 남아있는
                연결을 강제 종료합니다. 그래도 포트가 해제되지 않으면, TIME_WAIT 상태의 소켓이 있을
                수 있습니다. 잠시 후 다시 시도하거나 다른 포트를 사용하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
