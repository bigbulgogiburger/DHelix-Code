...
export default function CmdReviewPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <p className="text-[16px] text-gray-600 leading-relaxed">
              코드 변경 사항을 수집하여 LLM에게 버그, 보안, 코드 품질 관점의 체계적인 코드 리뷰를
              요청하는 슬래시 명령어입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">/review</code>는 커밋 전에 코드 변경 사항을
                체계적으로 점검하는 명령어입니다.
                <code className="text-cyan-600">git diff</code>를 수집하여 LLM에게 4가지 관점(버그,
                보안, 코드 품질, 에러 처리)으로 리뷰를 요청하고, 각 이슈에 대해
                심각도(CRITICAL/HIGH/MEDIUM/LOW)와 수정 제안을 받습니다.
              </p>
              <p>
                스테이징된 변경과 스테이징되지 않은 변경을 모두 수집하거나,
                <code className="text-cyan-600">--staged</code> 플래그로 스테이징된 변경만 리뷰할 수
                있습니다. 특정 파일 경로를 인자로 전달하면 해당 파일의 변경만 리뷰합니다.
              </p>
              <p>
  ...
  style EMPTY fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <Callout type="info" icon="💡">
              <strong>리뷰 관점:</strong> LLM은 4가지 카테고리로 코드를 분석합니다 &mdash; (1) 버그:
              로직 에러, null 위험, 경계값 에러, 경쟁 조건 / (2) 보안: 인젝션 취약점, 노출된 비밀,
              안전하지 않은 작업 / (3) 코드 품질: 가독성, 네이밍, 중복, 에러 처리 누락 / (4) 에러
              처리: 처리되지 않은 예외, 누락된 엣지 케이스
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              diff 크기 제한 상수입니다. 이 값을 초과하는 diff는 잘라내어 LLM 컨텍스트 윈도우를
              보호합니다.
            </p>
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">MAX_DIFF_SIZE</span> ={" "}
              <span className="num">50</span> * <span className="num">1024</span>;{" "}
              <span className="cm">{"// 50KB"}</span>
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              <code className="text-cyan-600">/review</code> 슬래시 명령어의 정의 객체입니다. git
              diff를 수집하고 리뷰 프롬프트를 LLM에 주입합니다.
            </p>
            <ParamTable
              params={[
                { name: "name", type: '"review"', required: true, desc: "명령어 이름" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: '"Review current code changes for bugs, security, and quality"',
                },
                {
                  name: "usage",
                  type: "string",
                  required: true,
                  desc: '"/review [--staged] [file path]"',
                },
                {
                  name: "execute",
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                ...
                스테이징되지 않은 변경을 <strong>모두</strong> 수집합니다. 두 diff가 모두 있으면
                &quot;=== Staged Changes ===&quot;와 &quot;=== Unstaged Changes ===&quot;로
                구분합니다.
              </li>
              <li>
                <code className="text-cyan-600">MAX_DIFF_SIZE</code>는 <strong>바이트</strong>{" "}
                기준입니다.
                <code className="text-cyan-600">Buffer.byteLength()</code>로 측정하므로 UTF-8
                멀티바이트 문자가 포함된 diff는 글자 수보다 더 빠르게 제한에 도달합니다.
              </li>
              <li>
                diff가 잘리면 <code className="text-cyan-600">string.slice()</code>로 단순
                절단됩니다. diff 경계(hunk boundary)를 고려하지 않으므로 마지막 파일의 diff가
                불완전할 수 있습니다.
              </li>
              <li>
                파일 경로 인자를 전달할 때 공백이 포함된 경로는{" "}
                <code className="text-cyan-600">JSON.stringify()</code>로 감싸서 안전하게
                처리됩니다.
              </li>
            </ul>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">--staged</code> 플래그로 커밋 예정인 변경만 집중적으로
              리뷰할 수 있습니다.
            </p>
            <CodeBlock>
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="fn">/review</span>{" "}
              <span className="str">src/commands/review.ts</span>
              {"\n"}
              {"\n"}
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> diff가 50KB를 초과하면 자동으로 잘립니다. 대규모 리팩토링
              시에는 파일별로 나눠서 리뷰하는 것이 더 정확한 결과를 얻을 수 있습니다.
            </Callout>
            ...
            <DeepDive title="리뷰 결과 활용 팁">
              ...
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>커밋 전 점검:</strong> <code>/review --staged</code> &rarr; 이슈 수정
                  &rarr; <code>/commit</code>
                </li>
                <li>
                  <strong>PR 준비:</strong> 전체 변경을 리뷰한 후 CRITICAL/HIGH 이슈를 우선 수정
                </li>
                <li>
                  <strong>학습:</strong> MEDIUM/LOW 이슈의 수정 제안을 참고하여 코딩 습관 개선
                </li>
              </ul>
              <p className="mt-3 text-amber-600">
                CRITICAL 이슈가 있으면 <strong>반드시</strong> 수정 후 커밋하세요. 보안 취약점이나
                심각한 버그는 코드 리뷰로 사전에 차단하는 것이 가장 효과적입니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              인자에 따라 서로 다른 git diff 명령어 조합을 사용합니다. staged와 unstaged diff를 모두
              수집할 때는 구분자를 넣어 LLM이 구별할 수 있도록 합니다.
            </p>
  ...
  style PASS fill:#dcfce7,stroke:#10b981,color:#065f46`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              diff 수집과 병합의 핵심 로직입니다. staged와 unstaged 모두 존재할 때만 구분자를 넣어
              병합합니다.
            </p>
            <CodeBlock>
              ...
              {"\n"}
              {"  "}
              <span className="prop">diff</span> = <span className="fn">gitExec</span>(
              <span className="str">
                `git diff --cached${"{"}
                <span className="prop">pathArg</span>
                {"}"}`
              </span>
              ...
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">unstaged</span> ={" "}
              <span className="fn">gitExec</span>(
              <span className="str">
                `git diff${"{"}
                <span className="prop">pathArg</span>
                {"}"}`
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="prop">staged</span> &&{" "}
              ...
              {"\n"}
              {"    "}
              <span className="prop">diff</span> = <span className="prop">staged</span> ||{" "}
              <span className="prop">unstaged</span>;{"\n"}
              {"  "}
              {"}"}
              {"\n"}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">staged 우선:</strong>{" "}
                <code className="text-cyan-600">staged || unstaged</code>에서 staged가 먼저
                평가되므로, staged만 있으면 staged diff를 사용합니다.
              </p>
              <p>
                <strong className="text-gray-900">구분자 삽입:</strong> 두 diff가 모두 있을 때만
                &quot;=== Staged/Unstaged ===&quot; 구분자를 삽입하여 LLM이 구별할 수 있게 합니다.
              </p>
              <p>
                <strong className="text-gray-900">크기 보호:</strong>{" "}
                <code className="text-cyan-600">Buffer.byteLength()</code>로 바이트 크기를 측정하여
                50KB를 초과하면 <code className="text-cyan-600">string.slice()</code>로 잘라냅니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                스테이징된 변경도, 스테이징되지 않은 변경도 없는 상태입니다. 파일을 수정하거나{" "}
                <code className="text-cyan-600">git add</code>한 후 다시 시도하세요. 이미 커밋된
                변경은 <code className="text-cyan-600">/review</code>로 리뷰할 수 없습니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <CodeBlock>
                ...
                {"\n"}
                <span className="fn">/review</span>{" "}
                <span className="str">src/core/context-manager.ts</span>
              </CodeBlock>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                네, 가능합니다. <code className="text-cyan-600">/review --staged src/app.ts</code>
                처럼 조합하면 해당 파일의 스테이징된 변경만 리뷰합니다. 인자 순서는 자유이며,{" "}
                <code className="text-cyan-600">--staged</code>가 아닌 인자가 파일 경로로
                인식됩니다.
              </p>
            </div>
          </section>
