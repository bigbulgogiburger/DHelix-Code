...
export default function MemoryManagerPage() {
      ...
      <div className="center-narrow">
        ...
        <div style={{ marginBottom: "48px" }}>
          ...
          <p className="text-[16px] text-gray-600 max-w-[640px]">
            프로젝트 스코프 메모리를 관리하는 중앙 클래스입니다.
            하위 모듈(loader, writer, paths)을 <span className="text-cyan-600 font-semibold">하나의 인터페이스</span>로 통합하여
            프로젝트당 하나의 인스턴스로 CRUD 작업을 제공합니다.
          </p>
        </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              ...
              <strong className="text-gray-900"> MemoryManager</strong>는 이 시스템의 <span className="text-cyan-600 font-semibold">파사드(Facade)</span>로,
              <code className="text-cyan-600 text-xs"> loader</code>, <code className="text-cyan-600 text-xs">writer</code>,
              <code className="text-cyan-600 text-xs"> paths</code> 모듈의 기능을 하나의 클래스로 묶습니다.
              프로젝트 경로의 SHA-256 해시 앞 16자를 사용하여 격리된 저장소를 보장합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>저장 위치:</strong> <code className="text-cyan-600 text-xs">~/.dbcode/projects/{"{프로젝트해시}"}/memory/MEMORY.md</code>
              &mdash; 프로젝트해시는 <code className="text-cyan-600 text-xs">SHA-256(절대경로).slice(0, 16)</code>입니다.
            </Callout>
  ...
  style DISK fill:#f8fafc,stroke:#ef4444,color:#1e293b`}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4">
              ...
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                <div className="flex gap-3">
                  ...
                  <span><code className="text-cyan-600 text-xs">projectRoot</code>를 생성자에서 한 번만 설정하면 모든 메서드에서 재사용</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-violet-600 font-bold shrink-0 w-28">설정 중앙 관리</span>
                  <span><code className="text-cyan-600 text-xs">maxLoadLines</code>, <code className="text-cyan-600 text-xs">maxMemoryLines</code> 등을 config로 일괄 관리</span>
                </div>
                <div className="flex gap-3">
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                프로젝트 루트 경로를 받아 메모리 관리자 인스턴스를 생성합니다.
                설정은 기본값에 사용자 오버라이드를 스프레드로 병합합니다.
              </p>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                MEMORY.md의 처음 <code className="text-cyan-600 text-xs">maxLoadLines</code> 줄을 로드합니다.
              ...
              <div className="mt-3">
                ...
                <ParamTable
                  params={[
                    { name: "content", type: "string", required: true, desc: "메모리 내용 (maxLoadLines까지 잘림)" },
                    { name: "memoryFilePath", type: "string", required: true, desc: "MEMORY.md 절대 경로" },
                    { name: "exists", type: "boolean", required: true, desc: "파일 존재 여부" },
                    { name: "topicFiles", type: "string[]", required: true, desc: "overflow된 토픽 파일 목록" },
                  ]}
                />
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                새 항목을 추가합니다. 중복 감지 + 자동 overflow를 포함합니다.
                같은 내용이 이미 있으면 건너뛰고, <code className="text-cyan-600 text-xs">maxMemoryLines</code>를 초과하면
                오래된 섹션을 토픽 파일로 분리합니다.
              </p>
              <ParamTable
                params={[
                  {
              ...
              />
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">반환: <code className="text-violet-600 text-xs">{"{ written: boolean, overflowed: boolean }"}</code></h4>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                  <div className="flex gap-3">
                    <code className="text-cyan-600 font-bold shrink-0 w-40">saveMemory(content)</code>
                  ...
                  <div className="flex gap-3">
                    <code className="text-cyan-600 font-bold shrink-0 w-40">readTopicFile(topic)</code>
                    <span>특정 토픽 파일 내용 읽기 (없으면 null)</span>
                  </div>
                  <div className="flex gap-3">
                    <code className="text-cyan-600 font-bold shrink-0 w-40">writeTopicFile(t, c)</code>
              ...
              <Callout type="warn" icon="⚠️">
                ...
                MEMORY.md를 비우고 모든 토픽 파일을 삭제합니다. 프로덕션 코드에서는 사용자 확인 후 호출하세요.
              </Callout>
            </div>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>기본 사용 (세션 시작 시)</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              프로젝트 루트를 전달하여 인스턴스를 생성하고,{" "}
              <code className="text-cyan-600 text-xs">loadMemory()</code>로 이전 세션의 학습 내용을 로드합니다.
            </p>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>학습 내용 저장</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              AI가 새로운 것을 학습하면{" "}
              <code className="text-cyan-600 text-xs">appendMemory()</code>로 기록합니다.
              중복은 자동으로 건너뛰고, 줄 수 초과 시 overflow됩니다.
            </p>
            ...
            <CodeBlock>
              ...
              <span className="text-[#c9d1d9]">{": "}</span>
              <span className="text-[#a5d6ff]">{'"이 프로젝트는 ESM only이므로 .js 확장자 필수"'}</span>{"\n"}
              <span className="text-[#c9d1d9]">{"}"}</span>
              <span className="text-[#c9d1d9]">);</span>
            </CodeBlock>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>Facade 패턴</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              MemoryManager는 GoF의 <strong>Facade</strong> 패턴을 사용합니다.
              복잡한 하위 모듈(loader, writer, paths)의 함수를 래핑하여
  ...
  Saved --> Loaded : 다시 로드`}
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>설정 병합 전략</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              기본값(<code className="text-cyan-600 text-xs">DEFAULT_CONFIG</code>)에 사용자 설정을 스프레드로 얕은 병합합니다.
            ...
            <CodeBlock>
              <span className="text-[#8b949e]">{"// 기본 설정에 사용자 오버라이드를 스프레드 병합"}</span>{"\n"}
              <span className="text-[#ff7b72]">this</span>
              <span className="text-[#c9d1d9]">.config = {"{"} ...DEFAULT_CONFIG, ...config {"}"};</span>
            </CodeBlock>

            <Callout type="info" icon="📝">
              <strong>readonly 속성:</strong> <code className="text-cyan-600 text-xs">config</code>와{" "}
              <code className="text-cyan-600 text-xs">projectRoot</code>는 모두 <code className="text-cyan-600 text-xs">readonly</code>입니다.
              인스턴스 생성 후에는 변경할 수 없으며, 불변성을 보장합니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인 1:</strong> 아직 메모리가 저장된 적이 없습니다.
                    <code className="text-cyan-600 text-xs"> exists</code>가 <code className="text-cyan-600 text-xs">false</code>인지 확인하세요.
                  </p>
                  <p>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <strong className="text-gray-900">원인:</strong> 동일한 내용이 이미 MEMORY.md에 존재합니다.
                    중복 감지는 대소문자 무시 부분 문자열 매칭으로 수행됩니다.
                    내용을 수정하여 다시 시도하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <strong className="text-gray-900">원인:</strong> 같은 절대 경로를 사용하고 있거나, 심볼릭 링크로 인해
                    다른 경로가 같은 해시를 생성할 수 있습니다.
                    <code className="text-cyan-600 text-xs"> getProjectHash()</code>로 해시를 확인하세요.
                  </p>
                </div>
              </div>
            </div>
