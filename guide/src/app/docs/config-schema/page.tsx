...
export default function ConfigSchemaPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mb-3">
              <span className="text-gray-900">Config Schema</span>
            </h1>
            <p className="text-[16px] text-gray-600 max-w-[640px]">
              Zod를 사용한 런타임 설정 유효성 검증 모듈입니다. JSON 파일이나 환경변수에서 로드한
              값이 올바른 형식인지{" "}
              <span className="text-violet-600 font-semibold">런타임에 검증</span>하고, 잘못된 값에{" "}
              <span className="text-cyan-600 font-semibold">명확한 에러 메시지</span>를 제공합니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              TypeScript의 타입 검사는 컴파일 시점에만 동작합니다. JSON 파일이나 환경변수 같은{" "}
              <strong className="text-gray-900">외부 입력</strong>은 런타임에 어떤 값이 들어올지 알
              수 없으므로, <span className="text-violet-600 font-semibold">Zod 스키마</span>로
              형식과 범위를 검증합니다. 각 스키마의{" "}
              <code className="text-cyan-600 text-xs">.default()</code>는 누락된 필드에 자동으로
              기본값을 적용합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>Zod의 장점:</strong> 검증과 타입 추론이 동시에 이루어집니다.
              <code className="text-cyan-600 text-xs"> z.infer&lt;typeof schema&gt;</code>로
              스키마에서 TypeScript 타입을 자동 생성할 수 있습니다.
            </Callout>
  ...
  style DUAL fill:#f1f5f9,stroke:#3b82f6,color:#1e293b`}
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                LLM 제공자 연결 설정. 로컬 LLM(Ollama, LM Studio)과 외부 API(OpenAI, Anthropic) 모두
                지원합니다.
              </p>
              <ParamTable
                params={[
                  {
                    ...
                    type: "string (URL)",
                    required: false,
                    desc: 'API 엔드포인트 (기본: "https://api.openai.com/v1")',
                  },
                  {
                    name: "apiKey",
                    type: "string?",
                    required: false,
                    desc: "API 인증 키 (로컬 LLM은 불필요)",
                  },
                  {
                    name: "model",
                    type: "string",
                    required: false,
                    desc: '모델명 (기본: "gpt-5.1-codex-mini")',
                  },
                  {
                    name: "temperature",
                    type: "number (0-2)",
                    required: false,
                    desc: "응답 온도 (기본: 0.0 — 결정적)",
                  },
                  {
                    name: "maxTokens",
                    type: "number (+)",
                    required: false,
                    desc: "최대 생성 토큰 (기본: 32768)",
                  },
                  {
                    name: "contextWindow",
                    type: "number (+)",
                    required: false,
                    desc: "컨텍스트 윈도우 (기본: 1,000,000)",
                  },
                  {
                    name: "timeout",
                    type: "number (+)",
                    required: false,
                    desc: "API 타임아웃 ms (기본: 60,000)",
                  },
                ]}
              />
            </div>
            ...
            <div className="mb-8">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                  ...
                  <div className="flex gap-3">
                    <code className="text-red-600 font-bold shrink-0 w-36">
                      "bypassPermissions"
                    </code>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                보안/가드레일 설정. 프롬프트 인젝션 방지, 비밀키 유출 방지, 요청 제한 등을
                검증합니다.
              </p>
              <ParamTable
                params={[
                  ...
                  },
                  {
                    ...
                    type: "boolean",
                    required: false,
                    desc: "프롬프트 인젝션 차단 (기본: true)",
                  },
                  {
                    name: "outputFiltering",
                    type: "boolean",
                    required: false,
                    desc: "민감 정보 마스킹 (기본: true)",
                  },
                  {
                    name: "auditLogging",
                    type: "boolean",
                  ...
                  },
                  {
                    ...
                    type: "object",
                    required: false,
                    desc: "requestsPerMinute(60), tokensPerDay(1M)",
                  },
                ]}
              />
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                  <div className="flex gap-3">
                    ...
                    <span>
                      theme(auto/dark/light), markdown, syntaxHighlighting, spinner, statusBar
                    </span>
                  </div>
                  <div className="flex gap-3">
                    <code className="text-cyan-600 font-bold shrink-0 w-44">
              ...
              <Callout type="warn" icon="⚠️">
                ...
                <code className="text-cyan-600 text-xs"> process.env</code>가 아닌 하드코딩 값을
                사용해야 합니다. 환경변수 기반 오버라이드는{" "}
                <code className="text-cyan-600 text-xs">config/loader.ts</code>의{" "}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              병합된 설정 객체를 <code className="text-cyan-600 text-xs">configSchema.parse()</code>
              로 검증합니다. 실패하면 Zod 에러가 발생합니다.
            </p>
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="text-[#8b949e]">
                {"// 나머지 필드(baseUrl, timeout 등)에는 기본값이 적용됨"}
              </span>
            </CodeBlock>

            <DeepDive title="safeParse로 에러를 던지지 않고 검증하기">
              <p className="mb-3">
                <code className="text-cyan-600 text-xs">.safeParse()</code>를 사용하면 에러를 던지지
                않고 검증 결과를 객체로 받을 수 있습니다.
              </p>
              ...
              <CodeBlock>
                ...
                <span className="text-[#c9d1d9]">{"  "}</span>
                <span className="text-[#8b949e]">
                  {"// result.error: ZodError (어떤 필드가 잘못됐는지 상세 정보)"}
                </span>
                {"\n"}
                <span className="text-[#c9d1d9]">{"}"}</span>
              </CodeBlock>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              <code className="text-cyan-600 text-xs">z.object()</code>로 조합합니다. 각 하위
              스키마는 독립적으로 <code className="text-cyan-600 text-xs">.default({})</code>를
              가지므로 전체 객체에 빈 객체만 전달해도 모든 필드에 기본값이 채워집니다.
            </p>
  ...
  style UI fill:#f8fafc,stroke:#10b981,color:#10b981,stroke-width:2px`}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                <div className="flex gap-3">
                  ...
                  <span>
                    <code className="text-cyan-600 text-xs">baseUrl</code>에{" "}
                    <code className="text-cyan-600 text-xs">z.string().url()</code> 적용 &mdash;
                    잘못된 URL은 즉시 거부
                  </span>
                </div>
                <div className="flex gap-3">
                  ...
                  <span>
                    <code className="text-cyan-600 text-xs">temperature</code>에{" "}
                    <code className="text-cyan-600 text-xs">.min(0).max(2)</code> &mdash; 0~2 범위만
                    허용
                  </span>
                </div>
                <div className="flex gap-3">
                  ...
                  <span>
                    <code className="text-cyan-600 text-xs">maxTokens</code>,{" "}
                    <code className="text-cyan-600 text-xs">timeout</code> 등에{" "}
                    <code className="text-cyan-600 text-xs">.positive()</code> &mdash; 0 이하 거부
                  </span>
                </div>
                <div className="flex gap-3">
                  <span className="text-amber-600 font-bold shrink-0 w-28">열거형 검증</span>
                  <span>
                    <code className="text-cyan-600 text-xs">permissionMode</code>는 5가지,{" "}
                    <code className="text-cyan-600 text-xs">tone</code>은 6가지 값만 허용
                  </span>
                </div>
              </div>
            </div>
            ...
            <Callout type="info" icon="📝">
              ...
              <code className="text-cyan-600 text-xs"> "minimal"</code> &mdash; 6가지 응답 톤을
              지원하여 사용자 경험을 커스터마이징합니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> ZodError: Expected string, received
                  number
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    JSON에서 문자열이어야 할 필드에 숫자를 넣었습니다. 예:{" "}
                    <code className="text-red-600 text-xs">{'"model": 4'}</code> 대신{" "}
                    <code className="text-cyan-600 text-xs">{'"model": "gpt-4o"'}</code>를
                    사용하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <code className="text-cyan-600 text-xs">temperature</code>는 0~2 사이여야
                    합니다. 음수나 2 초과 값은 Zod가 거부합니다. 대부분의 코딩 작업에서는{" "}
                    <code className="text-cyan-600 text-xs">0.0</code>(결정적)이 권장됩니다.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    ...
                    <code className="text-cyan-600 text-xs"> "bypassPermissions"</code>. 대소문자에
                    주의하세요 &mdash; <code className="text-red-600 text-xs">"Plan"</code>은
                    유효하지 않습니다.
                  </p>
                </div>
              </div>
