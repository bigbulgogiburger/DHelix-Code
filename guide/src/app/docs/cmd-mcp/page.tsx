...
export default function CmdMcpPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">/mcp MCP 서버 관리</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="leaf" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              MCP(Model Context Protocol) 서버의 추가, 제거, 조회를 3-스코프 체계로 관리하는
              명령어입니다.
            </p>
          </div>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                ...
                <strong>project</strong>(팀 공유), <strong>local</strong>(개인). 우선순위는{" "}
                <code className="text-cyan-600">local &gt; project &gt; user</code> 순서이며, 같은
                이름의 서버가 여러 스코프에 있으면 높은 우선순위 스코프가 적용됩니다.
              </p>
              <p>
                서버 목록 조회 시 <code className="text-cyan-600">MCPManager</code>에서 실시간 연결
                상태와 등록된 도구 수도 함께 표시합니다.
              </p>
            </div>
  ...
  style MGR fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              ...
              <code>.env.local</code>(개인) &gt; <code>.env</code>(프로젝트) &gt; 시스템
              환경변수(전역) 순서로 같은 이름이면 가장 가까운 설정이 적용됩니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: false,
                  desc: '전송 방식 (기본값: "stdio")',
                },
                {
                  name: "command",
                  type: "string",
                  required: false,
                  desc: "실행할 명령어 (stdio 전송 시)",
                },
              ...
              params={[
                {
                  ...
                  type: "조회",
                  required: false,
                  desc: "모든 스코프의 MCP 서버 목록 (연결 상태, 도구 수 포함)",
                },
                {
                  name: "/mcp add <name> <cmd>",
                  type: "추가",
                  required: false,
                  desc: "stdio 기반 MCP 서버를 user 스코프에 추가 (기본)",
                },
                {
                  name: "/mcp add -s <scope> <name> <cmd>",
                  type: "추가",
                  required: false,
                  desc: "지정한 스코프에 MCP 서버 추가",
                },
                {
                  name: "/mcp remove <name>",
                  type: "제거",
                  required: false,
                  desc: "모든 스코프에서 MCP 서버 제거",
                },
                {
                  name: "/mcp remove -s <scope> <name>",
                  type: "제거",
                  required: false,
                  desc: "지정한 스코프에서만 MCP 서버 제거",
                },
              ]}
            />
            ...
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">getConfigPath</span>(
              <span className="prop">scope</span>, <span className="prop">workingDirectory</span>):{" "}
              <span className="type">string</span>
              {"\n"}
              <span className="kw">async function</span> <span className="fn">readScopeConfig</span>
              (<span className="prop">filePath</span>): <span className="type">Promise</span>&lt;
              <span className="type">ScopeConfigFile</span>&gt;
              {"\n"}
              <span className="kw">async function</span>{" "}
              <span className="fn">addServerToConfig</span>(<span className="prop">filePath</span>,{" "}
              <span className="prop">name</span>, <span className="prop">entry</span>):{" "}
              <span className="type">Promise</span>&lt;<span className="type">void</span>&gt;
              {"\n"}
              <span className="kw">async function</span>{" "}
              <span className="fn">removeServerFromConfig</span>(
              <span className="prop">filePath</span>, <span className="prop">name</span>):{" "}
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              ...
              <li>
                ...
                <strong>모든 스코프</strong>에서 해당 서버를 제거합니다. 특정 스코프만 제거하려면{" "}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">-s</code> 옵션으로 스코프를 지정하여 서버를 추가하거나
              제거합니다.
            </p>
            <CodeBlock>
            ...
            <DeepDive title="서버 목록의 연결 상태 표시">
              ...
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <code>*</code> &mdash; 연결됨 (connected). 도구 수도 함께 표시
                </li>
                <li>
                  <code>-</code> &mdash; 미연결 (not connected). 재시작 필요
                </li>
                <li>
                  <code>(overridden)</code> &mdash; 같은 이름이 높은 우선순위 스코프에 존재하여
                  무시됨
                </li>
              </ul>
              <p className="mt-3 text-gray-600">
                연결된 서버의 도구 수는{" "}
                <code className="text-cyan-600">MCPManager.getRegisteredTools()</code>에서
                가져옵니다. 서버가 제공하는 도구가 많을수록 LLM이 활용할 수 있는 기능이
                풍부해집니다.
              </p>
            </DeepDive>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">handleList()</code>는 local, project, user 순서로
              스캔하며 이미 표시된 서버 이름을 추적하여 중복을 처리합니다.
            </p>
  ...
  style OVER fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              서버 추가 핸들러의 핵심 로직입니다. <code className="text-cyan-600">-s</code> 옵션을
              파싱하고 설정 파일에 서버를 기록합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">handleAdd</span>(
              <span className="prop">parts</span>, <span className="prop">workingDirectory</span>){" "}
              {"{"}
              {"\n"}
              {"  "}
              <span className="kw">let</span> <span className="prop">scope</span>:{" "}
              <span className="type">Scope</span> = <span className="str">&quot;user&quot;</span>;
              {"  "}
              <span className="cm">{"// 기본 스코프"}</span>
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">if</span> (<span className="prop">remaining</span>[
              <span className="num">0</span>] === <span className="str">&quot;-s&quot;</span>) {"{"}
              {"\n"}
              {"    "}
              <span className="prop">scope</span> = <span className="prop">remaining</span>[
              <span className="num">1</span>];
              {"\n"}
              {"    "}
              <span className="prop">remaining</span> = <span className="prop">remaining</span>.
              <span className="fn">slice</span>(<span className="num">2</span>);
              {"\n"}
              {"  "}
              {"}"}
              {"\n"}
              ...
              {"    "}...(<span className="prop">cmdArgs</span>.<span className="prop">length</span>{" "}
              {">"} <span className="num">0</span> ? {"{"} <span className="prop">args</span>:{" "}
              <span className="prop">cmdArgs</span> {"}"} : {"{ }"}){"\n"}
              {"  "}
              {"}"};{"\n"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              {"  "}
              <span className="kw">await</span> <span className="fn">addServerToConfig</span>(
              <span className="prop">configPath</span>, <span className="prop">name</span>,{" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">readScopeConfig()</strong> &mdash; 설정 파일이
                없으면 빈 객체를 반환합니다. JSON 파싱 실패도 빈 객체로 안전하게 처리합니다.
              </p>
              <p>
                <strong className="text-gray-900">addServerToConfig()</strong> &mdash; 디렉토리가
                없으면 <code>mkdir recursive</code>로 자동 생성합니다. 기존 설정에 서버를 추가하고
                덮어씁니다.
              </p>
              <p>
                <strong className="text-gray-900">displayed Set</strong> &mdash; 이미 표시된 서버
                이름을 추적하여, 낮은 우선순위 스코프에서 같은 이름이 나오면
                &quot;overridden&quot;으로 표시합니다.
              </p>
            </div>
          </section>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                같은 이름의 서버가 여러 스코프에 존재하면 우선순위가 높은 스코프 (local &gt; project
                &gt; user)만 적용됩니다. 의도적이라면 그대로 두고, 불필요한 중복이면
                <code>/mcp remove -s &lt;scope&gt; &lt;name&gt;</code>으로 낮은 우선순위 스코프에서
                제거하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code>-s</code> 옵션에는 <code>local</code>, <code>project</code>, <code>user</code>
                만 사용할 수 있습니다. 대소문자를 확인하고, 오타가 없는지 확인하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
