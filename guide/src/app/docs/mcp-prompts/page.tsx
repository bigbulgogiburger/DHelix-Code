...
export default function MCPPromptsPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">MCPPromptManager &amp; MCPResourceManager</span>
            </h1>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">MCPPromptManager</code>는 MCP 서버가 제공하는 재사용
                가능한 프롬프트 템플릿을 관리합니다. 사용자가 슬래시 명령(
                <code className="text-cyan-600">/mcp__서버이름__프롬프트이름</code>)으로 프롬프트를
                실행하면, 서버에서 인자가 치환된 메시지를 받아 대화에 주입합니다.
              </p>
              <p>
                <code className="text-cyan-600">MCPResourceManager</code>(
                <code className="text-cyan-600">resources.ts</code>)는 MCP 서버의 외부 데이터(파일,
                DB 레코드, API 데이터 등)를 참조합니다. 사용자가{" "}
                <code className="text-cyan-600">@서버이름:리소스URI</code> 구문으로 멘션하면, 해당
                리소스의 콘텐츠를 가져와 대화 컨텍스트에 주입합니다.
              </p>
              <p>
                두 모듈 모두 네임스페이싱을 통해 여러 서버의 프롬프트/리소스가 충돌하지 않도록
                합니다. 리소스 매니저는 TTL(5분) 기반 캐시로 반복 읽기를 최적화하고, 캐시 히트/미스
                통계를 추적합니다.
              </p>
            </div>
  ...
  style CTX fill:#dcfce7,stroke:#10b981,color:#065f46`}
            ...
            <Callout type="info" icon="💡">
              ...
              양식(프롬프트 템플릿)에 빈칸(인자)을 채워 보내면, 서버가 완성된 편지(메시지)를
              돌려줍니다. 리소스 매니저는 &quot;도서관 사서&quot;입니다. @멘션으로 책(리소스)을
              요청하면, 캐시(최근 대출 목록)를 확인하고 없으면 서가(서버)에서 꺼내옵니다.
            </Callout>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                ...
                {
                  name: "messages",
                  type: "readonly MCPPromptMessage[]",
                  required: true,
                  desc: "서버에서 반환한 메시지 배열",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              프롬프트 기능에 필요한 MCPClient 인터페이스의 부분 집합입니다. 테스트 시 목(mock)
              객체를 쉽게 만들 수 있도록 분리되어 있습니다.
            </p>
            <ParamTable
              params={[
                {
                  name: "listPrompts()",
                  type: "Promise<readonly MCPPrompt[]>",
                  required: true,
                  desc: "사용 가능한 프롬프트 목록 조회",
                },
                {
                  name: "getPrompt(name, args)",
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              MCP 프롬프트의 발견, 인자 파싱, 검증, 실행, 슬래시 명령 생성을 담당하는 메인
              클래스입니다.
            </p>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              MCP 클라이언트에서 프롬프트를 발견하고{" "}
              <code className="text-cyan-600">mcp__서버이름__프롬프트이름</code> 키로 저장합니다.
            </p>
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">discoverPrompts</span>({"\n"}
              {"  "}
              <span className="prop">client</span>:{" "}
              <span className="type">PromptCapableClient</span>,{"\n"}
              {"  "}
              ...
              <span className="type">MCPPrompt</span>[]&gt;
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              <code className="text-cyan-600">key=&apos;single quoted&apos;</code> 형식을
              지원합니다.
            </p>
            <CodeBlock>
              ...
              {"  "}
              <span className="prop">promptArgs</span>?: <span className="kw">readonly</span>{" "}
              <span className="type">MCPPromptArgument</span>[]
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "파싱할 인자 문자열 (예: 'name=\"John\" age=30')",
                },
                {
                  name: "promptArgs",
                  type: "readonly MCPPromptArgument[]",
                  required: false,
                  desc: "프롬프트 인자 정의 (위치 인자 할당용)",
                },
              ]}
            />
            ...
            <CodeBlock>
              <span className="fn">validateArgs</span>(<span className="prop">args</span>:{" "}
              <span className="type">Record</span>&lt;<span className="type">string</span>,{" "}
              <span className="type">string</span>&gt;, <span className="prop">promptDef</span>:{" "}
              <span className="type">MCPPrompt</span>): {"{"}
              {"\n"}
              {"  "}
              <span className="kw">readonly</span> <span className="prop">valid</span>:{" "}
              <span className="type">boolean</span>;{"\n"}
              {"  "}
              <span className="kw">readonly</span> <span className="prop">missing</span>:{" "}
            ...
            <CodeBlock>
              ...
              {"\n"}): <span className="type">Promise</span>&lt;<span className="kw">readonly</span>{" "}
              <span className="type">MCPPromptMessage</span>[]&gt;
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              발견된 모든 프롬프트에 대해 슬래시 명령을 생성합니다. 각 명령의{" "}
              <code className="text-cyan-600">execute()</code>는 인자 파싱, 검증, 프롬프트 실행을
              자동으로 수행합니다.
            </p>
            <CodeBlock>
              <span className="fn">generateSlashCommands</span>({"\n"}
              {"  "}
              <span className="prop">getClient</span>: (<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>) =&gt;{" "}
              <span className="type">PromptCapableClient</span> |{" "}
              <span className="type">undefined</span>
              {"\n"}): <span className="kw">readonly</span> {"{"} <span className="prop">name</span>
              ; <span className="prop">description</span>; <span className="prop">usage</span>;{" "}
              <span className="prop">execute</span> {"}"}[]
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                네임스페이싱 형식은{" "}
                <code className="text-cyan-600">mcp__서버이름__프롬프트이름</code>입니다. 서버
                이름에 언더스코어가 있으면 파싱이 모호해질 수 있으니 주의하세요.
              </li>
              <li>
                <code className="text-cyan-600">parsePromptArgs()</code>에서{" "}
                <code className="text-cyan-600">key=value</code>
                패턴이 없고 단일 문자열만 있으면, 첫 번째 인자 정의의 이름에 전체 문자열을
                할당합니다.
              </li>
              <li>
                슬래시 명령 실행 결과의{" "}
                <code className="text-cyan-600">shouldInjectAsUserMessage: true</code>는 결과를
                사용자 메시지로 대화에 주입하라는 신호입니다. UI 레이어가 이 플래그를 처리해야
                합니다.
              </li>
            </ul>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              사용자 텍스트에서 추출한 <code className="text-cyan-600">@server:uri</code>{" "}
              멘션입니다.
            </p>
            <ParamTable
              params={[
            ...
            {/* Constructor */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">constructor(ttlMs?)</h4>
            <CodeBlock>
              <span className="kw">constructor</span>(<span className="prop">ttlMs</span>?:{" "}
            ...
            <CodeBlock>
              <span className="fn">parseResourceMentions</span>(<span className="prop">text</span>:{" "}
              <span className="type">string</span>): <span className="kw">readonly</span>{" "}
              <span className="type">ResourceMention</span>[]
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              서버에서 리소스를 읽어 반환합니다. TTL 기반 캐시를 사용하여 유효한 캐시가 있으면 서버
              호출 없이 즉시 반환합니다.
            </p>
            <CodeBlock>
            ...
            <CodeBlock>
              ...
              {"\n"}): <span className="type">Promise</span>&lt;<span className="kw">readonly</span>{" "}
              <span className="type">ResolvedResource</span>[]&gt;
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="fn">formatResourcesForContext</span>(
              <span className="prop">resources</span>: <span className="kw">readonly</span>{" "}
              <span className="type">ResolvedResource</span>[]):{" "}
              <span className="type">string</span>
              {"\n"}
              {"\n"}
            ...
            <CodeBlock>
              ...
              {"       "}
              <span className="cm">{"// 전체 초기화 (히트/미스 카운터 포함)"}</span>
              {"\n"}
              <span className="fn">clearExpiredCache</span>(): <span className="type">void</span>{" "}
              <span className="cm">{"// 만료된 항목만 제거"}</span>
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                @멘션 패턴: <code className="text-cyan-600">@server:protocol://path</code> 또는{" "}
                <code className="text-cyan-600">@server:name</code>. 서버 이름은{" "}
                <code className="text-cyan-600">\w[\w-]*</code> 패턴(영숫자, 하이픈)이어야 합니다.
              </li>
              <li>
                ...
                멘션이 실패해도 성공한 멘션의 결과는 반환됩니다. 실패는 조용히 건너뜁니다.
              </li>
              <li>
                캐시 키는 <code className="text-cyan-600">serverName::uri</code> 형태입니다. TTL
                만료 후에도 캐시 항목이 자동으로 삭제되지 않습니다.
                <code className="text-cyan-600">clearExpiredCache()</code>를 주기적으로 호출해야
                메모리가 절약됩니다.
              </li>
              <li>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="kw">new</span> <span className="fn">MCPPromptManager</span>();
              {"\n"}
              {"\n"}
              <span className="cm">{"// 1. 서버에서 프롬프트 발견"}</span>
              {"\n"}
              <span className="kw">await</span> <span className="prop">manager</span>.
              <span className="fn">discoverPrompts</span>(<span className="prop">client</span>,{" "}
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">commands</span> ={" "}
              <span className="prop">manager</span>.
              <span className="fn">generateSlashCommands</span>({"\n"}
              {"  "}
              ...
              {"\n"}
              <span className="cm">
                {'// 3. 사용자가 /mcp__github__create_pr description="Fix bug" 실행'}
              </span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">result</span> ={" "}
              <span className="kw">await</span> <span className="prop">commands</span>[
              <span className="num">0</span>].<span className="fn">execute</span>(
              <span className="str">&apos;description=&quot;Fix bug&quot;&apos;</span>);
              {"\n"}
              <span className="cm">
                {"// → { output: '...', success: true, shouldInjectAsUserMessage: true }"}
              </span>
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>generateSlashCommands()</code>에 전달하는{" "}
              <code>getClient</code> 함수가 서버 이름으로 유효한 클라이언트를 반환하지 않으면, 실행
              시 &quot;MCP server is not connected&quot; 에러가 반환됩니다.
            </Callout>

            {/* 리소스 기본 사용법 */}
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              리소스 &mdash; @멘션으로 리소스 참조
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              사용자 텍스트에서 @멘션을 파싱하고 리소스 콘텐츠를 가져오는 흐름입니다.
            </p>
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">resourceManager</span> ={" "}
              <span className="kw">new</span> <span className="fn">MCPResourceManager</span>();
              {"\n"}
              {"\n"}
              <span className="cm">{"// 1. 사용자 텍스트에서 @멘션 파싱"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">text</span> ={" "}
              <span className="str">&quot;이 파일 분석해줘 @myserver:file:///data.txt&quot;</span>;
              {"\n"}
              ...
              {"\n"}
              <span className="cm">
                {"// → [{ serverName: 'myserver', uri: 'file:///data.txt' }]"}
              </span>
              {"\n"}
              {"\n"}
              ...
              <span className="kw">await</span> <span className="prop">resourceManager</span>.
              <span className="fn">resolveResourceMentions</span>(<span className="prop">text</span>
              , <span className="prop">clients</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// 3. 대화 컨텍스트에 주입할 형태로 포맷"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">formatted</span> ={" "}
              <span className="prop">resourceManager</span>.
              <span className="fn">formatResourcesForContext</span>(
              <span className="prop">resources</span>);
              {"\n"}
              <span className="cm">
                {'// → \'<resource server="myserver" uri="file:///data.txt">내용...</resource>\''}
              </span>
            </CodeBlock>

            <Callout type="tip" icon="*">
              <strong>팁:</strong> 동일한 리소스를 여러 번 참조해도 캐시 덕분에 서버 호출은 한 번만
              발생합니다.
            ...
            <DeepDive title="인자 파싱 상세 동작">
              <p className="mb-3">
                <code className="text-cyan-600">parsePromptArgs()</code>는 세 가지 형식을
                지원합니다:
              </p>
              <CodeBlock>
                ...
                {"\n"}
                <span className="fn">parsePromptArgs</span>(
                <span className="str">&apos;name=John age=30&apos;</span>){"\n"}
                ...
                {"\n"}
                <span className="fn">parsePromptArgs</span>(
                <span className="str">&apos;desc=&quot;Fix the bug&quot;&apos;</span>){"\n"}
                ...
                {"\n"}
                <span className="fn">parsePromptArgs</span>(
                <span className="str">&apos;Hello World&apos;</span>, [{"{"}name:{" "}
              ...
              <p className="mt-3 text-amber-600">
                위치 인자는 key=value 패턴이 전혀 없을 때만 동작합니다. 하나라도 key=value가 있으면
                나머지 텍스트는 무시됩니다.
              </p>
            </DeepDive>
          </section>
  ...
  style ERR3 fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">readResource</span>(
              <span className="prop">client</span>, <span className="prop">serverName</span>,{" "}
              <span className="prop">uri</span>): <span className="type">Promise</span>&lt;
              <span className="type">string</span>&gt; {"{"}
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">cacheKey</span> ={" "}
              <span className="str">`${"{"}</span>
              <span className="prop">serverName</span>
              <span className="str">
                {"}"}::${"{"}
              </span>
              <span className="prop">uri</span>
              <span className="str">{"}"}`</span>;{"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">cached</span> ={" "}
              <span className="kw">this</span>.<span className="prop">cache</span>.
              <span className="fn">get</span>(<span className="prop">cacheKey</span>);
              {"\n"}
              {"\n"}
              {"  "}
              ...
              <span className="kw">this</span>.<span className="fn">isExpired</span>(
              <span className="prop">cached</span>)) {"{"}
              {"\n"}
              {"    "}
              ...
              {"  "}
              <span className="kw">this</span>.<span className="prop">cache</span>.
              <span className="fn">set</span>(<span className="prop">cacheKey</span>, {"{"}{" "}
              <span className="prop">content</span>, <span className="prop">timestamp</span>:{" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">프롬프트 네임스페이싱:</strong>{" "}
                <code>mcp__serverName__promptName</code> 형태로 Map에 저장하여, 여러 서버의 같은
                이름 프롬프트가 충돌하지 않습니다.
              </p>
              <p>
                <strong className="text-gray-900">리소스 캐시 키:</strong>{" "}
                <code>serverName::uri</code> 형태로, 같은 URI라도 서버가 다르면 별도로 캐시됩니다.
              </p>
              <p>
                <strong className="text-gray-900">부분 실패 허용:</strong>{" "}
                <code>resolveResourceMentions()</code>는 <code>Promise.allSettled()</code>를
                사용하여, 하나의 멘션이 실패해도 다른 멘션의 해석을 차단하지 않습니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                프롬프트에 필수 인자(<code className="text-cyan-600">required: true</code>)가
                누락되었습니다. 슬래시 명령의 usage 힌트(<code>&lt;name&gt;</code>은 필수,
                <code>[name]</code>은 선택)를 확인하고, 필수 인자를 모두 전달하세요.
              </p>
              <CodeBlock>
                <span className="cm">
                  {"// usage: /mcp__github__create_pr <description> [draft]"}
                </span>
                {"\n"}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                @멘션 형식이 올바른지 확인하세요. 서버 이름은 영숫자와 하이픈만 허용됩니다. 공백이나
                특수 문자가 포함되면 파싱되지 않습니다. 올바른 예:{" "}
                <code className="text-cyan-600">@my-server:file:///path</code>, 잘못된 예:{" "}
                <code className="text-red-600">@my server:file:///path</code>
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">clearCache()</code>로 캐시를 초기화하거나, 생성자에
                더 짧은 TTL을 전달하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">resolveResourceMentions()</code>는
                <code className="text-cyan-600">Promise.allSettled()</code>를 사용하므로, 알 수 없는
                서버나 존재하지 않는 리소스의 멘션은 조용히 건너뜁니다. 클라이언트 Map에 해당 서버가
                등록되어 있는지, 리소스 URI가 올바른지 확인하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
