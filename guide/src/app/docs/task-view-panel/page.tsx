...
export default function TaskViewPanelPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">TaskViewPanel</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="cli" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              멀티 에이전트 작업의 진행 상황을 테이블 형태로 보여주는 토글 가능한 오버레이
              패널입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">TaskViewPanel</code>은 Ctrl+T로 표시/숨김을 토글할
                수 있는 오버레이 패널입니다. 각 작업을 우선순위(★), 상태(아이콘), 제목, 담당자 열로
                구성된 테이블 형태로 보여줍니다.
              </p>
              <p>
                이 파일에는 표시용 타입 정의, 상수(아이콘/색상/우선순위/정렬 가중치), 헬퍼
                함수(정렬/통계/아이콘 조회), 서브 컴포넌트(
                <code className="text-cyan-600">TaskRow</code>,{" "}
                <code className="text-cyan-600">TaskSummaryBar</code>), 그리고 메인 패널 컴포넌트가
                모두 포함되어 있습니다.
              </p>
              <p>
                작업은 기본적으로 우선순위 순으로 정렬됩니다:{" "}
                <code className="text-red-600">critical</code>(★★★★) →
                <code className="text-yellow-600">high</code>(★★★) →{" "}
                <code className="text-blue-600">medium</code>(★★) →
  ...
  style APP fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              표시용으로 간소화된 작업 데이터입니다. 코어의 Task와 달리 UI 표시에 필요한 필드만
              포함합니다.
            </p>
            <ParamTable
              params={[
                { name: "id", type: "string", required: true, desc: "작업 고유 식별자" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "작업 제목 (40자 초과 시 말줄임 처리)",
                },
                {
                  name: "status",
                  type: "string",
                  required: true,
                  desc: "작업 상태 (pending, in_progress, completed, failed, blocked, cancelled)",
                },
                {
                  name: "priority",
                  type: "string",
                ...
                },
                {
                  ...
                  type: "string",
                  required: false,
                  desc: "담당 에이전트 이름 (시안색으로 표시)",
                },
                {
                  name: "dependsOn",
                  type: "readonly string[]",
                  required: false,
                  desc: "의존하는 작업 ID 배열 (개수만 표시)",
                },
            ...
            <ParamTable
              params={[
                {
                  name: "sortBy",
                  type: '"priority" | "status" | "created"',
                  required: true,
                  desc: "정렬 기준 — 우선순위, 상태, 또는 생성 순서",
                },
                {
                  name: "filterStatus",
                  type: "string",
                  required: false,
                  desc: "특정 상태만 표시할 필터 (미사용 시 전체 표시)",
                },
                {
                  name: "showCompleted",
                  type: "boolean",
                  required: true,
                  desc: "완료된 작업도 표시할지 여부",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              집계된 작업 통계입니다. <code className="text-cyan-600">TaskSummaryBar</code>에
              전달됩니다.
            </p>
            <ParamTable
              params={[
            ...
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">sortTasks(tasks, sortBy)</h4>
            ...
            <ParamTable
              params={[
                ...
                },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: '"priority" → 우선순위순, "status" → 상태순, "created" → 원래 순서',
                },
              ]}
            />

            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">getTaskStats(tasks)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              작업 목록에서 상태별 집계 통계를 계산하여{" "}
              <code className="text-cyan-600">TaskStats</code> 객체를 반환합니다.
            </p>
            <CodeBlock>
              <span className="kw">export function</span> <span className="fn">getTaskStats</span>(
              <span className="prop">tasks</span>:{" "}
              <span className="type">readonly TaskDisplayItem</span>[]):{" "}
              <span className="type">TaskStats</span>
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              제목을 최대 길이로 자르고, 초과 시 말줄임(…)을 추가합니다. 기본 최대 길이는
              40자입니다.
            </p>
            <CodeBlock>
              <span className="kw">export function</span> <span className="fn">truncateTitle</span>(
              <span className="prop">title</span>: <span className="type">string</span>,{" "}
              <span className="prop">maxLength</span>?: <span className="type">number</span>):{" "}
            ...
            <ParamTable
              params={[
                ...
                },
                {
                  ...
                  type: "string",
                  required: false,
                  desc: "헤더에 표시할 팀/프로젝트 이름",
                },
                {
                  name: "onClose",
                  type: "() => void",
                  required: false,
                  desc: "패널 닫기 요청 시 호출되는 콜백",
                },
              ]}
            />
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                이 컴포넌트는 키보드 입력을 직접 처리하지 않습니다. Ctrl+T 토글은 부모 컴포넌트(
                <code className="text-cyan-600">App.tsx</code>)에서 관리합니다.
              </li>
              <li>
                <code className="text-cyan-600">visible</code>이{" "}
                <code className="text-cyan-600">false</code>이면
                <code className="text-cyan-600">null</code>을 반환하여 DOM에 아무것도 렌더링하지
                않습니다.
              </li>
              <li>
                작업 정렬은 항상 <code className="text-cyan-600">&quot;priority&quot;</code>{" "}
                기준으로 고정되어 있습니다. 현재{" "}
                <code className="text-cyan-600">TaskViewOptions</code>의{" "}
                <code className="text-cyan-600">sortBy</code>를 동적으로 변경하는 UI는 구현되어 있지
                않습니다.
              </li>
              <li>
                <code className="text-cyan-600">TaskRow</code>와{" "}
                <code className="text-cyan-600">TaskSummaryBar</code>는
                <code className="text-cyan-600">React.memo</code>로 감싸져 있어 불필요한 리렌더링을
                방지합니다.
              </li>
            </ul>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              <span className="kw">import</span> {"{"} <span className="type">TaskViewPanel</span>{" "}
              {"}"} <span className="kw">from</span>{" "}
              <span className="str">&quot;./TaskViewPanel.js&quot;</span>;{"\n"}
              {"\n"}
              <span className="cm">{"// 부모 컴포넌트에서 토글 상태 관리"}</span>
              {"\n"}
              <span className="kw">const</span> [<span className="prop">visible</span>,{" "}
              <span className="prop">setVisible</span>] = <span className="fn">useState</span>(
              ...
              <span className="prop">ctrl</span> && <span className="prop">_input</span> ==={" "}
              <span className="str">&quot;t&quot;</span>) {"{"}
              {"\n"}
              {"    "}
              <span className="fn">setVisible</span>(<span className="prop">v</span> =&gt; !
              ...
              {"\n"}
              {"  "}
              <span className="prop">teamName</span>=
              <span className="str">&quot;Backend Team&quot;</span>
              {"\n"}/&gt;
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>TaskDisplayItem</code>의 <code>status</code> 필드는
              코어의 <code>TaskStatus</code> 타입이 아니라 <code>string</code>입니다. 정의되지 않은
              상태값을 전달하면 아이콘이 &quot;?&quot;로, 색상이 &quot;gray&quot;로 표시됩니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">sortTasks</code>,{" "}
              <code className="text-cyan-600">getTaskStats</code>,
              <code className="text-cyan-600">getStatusIcon</code> 등은 모두 export되어 다른
              컴포넌트에서도 사용할 수 있습니다.
            </p>
            <CodeBlock>
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">sorted</span> ={" "}
              <span className="fn">sortTasks</span>(<span className="prop">tasks</span>,{" "}
              ...
              <span className="fn">getTaskStats</span>(<span className="prop">tasks</span>);
              {"\n"}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="str">`${"{"}</span>
              <span className="prop">stats</span>.<span className="prop">completed</span>
              <span className="str">
                {"}"}/${"{"}
              </span>
              <span className="prop">stats</span>.<span className="prop">total</span>
              <span className="str">{"}"} 완료`</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// 아이콘/별 직접 조회"}</span>
              {"\n"}
              <span className="fn">getStatusIcon</span>(
              <span className="str">&quot;completed&quot;</span>);{" "}
              <span className="cm">{"// → ●"}</span>
              {"\n"}
              <span className="fn">getPriorityStars</span>(
              <span className="str">&quot;high&quot;</span>);{"     "}
              <span className="cm">{"// → ★★★"}</span>
            </CodeBlock>

            <Callout type="tip" icon="*">
              <strong>팁:</strong> <code>sortTasks</code>는 원본 배열을 변경하지 않고 새 배열을
              반환합니다.
            ...
            <DeepDive title="우선순위/상태 정렬 가중치 체계">
              <p className="mb-3">
                정렬은 각 값에 할당된 숫자 가중치를 비교하여 수행됩니다. 숫자가 작을수록 목록 앞에
                배치됩니다.
              </p>
  ...
  style FOOTER fill:#f1f5f9,stroke:#64748b,color:#1e293b`}
            ...
            <CodeBlock>
              <span className="cm">
                {"// [1] 우선순위 기준으로 정렬 (tasks 변경 시에만 재계산)"}
              </span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">sorted</span> ={" "}
              <span className="fn">useMemo</span>(() =&gt; <span className="fn">sortTasks</span>(
              <span className="prop">tasks</span>, <span className="str">&quot;priority&quot;</span>
              ), [<span className="prop">tasks</span>]);
              {"\n"}
              {"\n"}
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">stats</span> ={" "}
              <span className="fn">useMemo</span>(() =&gt; <span className="fn">getTaskStats</span>(
              <span className="prop">tasks</span>), [<span className="prop">tasks</span>]);
              {"\n"}
              {"\n"}
              <span className="cm">{"// [3] visible이 false면 즉시 null 반환"}</span>
              {"\n"}
              <span className="kw">if</span> (!<span className="prop">visible</span>){" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                ...
                <code className="text-cyan-600">sortTasks</code>는 내부에서 배열을 복사(
                <code>[...tasks]</code>)한 뒤 정렬하므로 원본을 변경하지 않습니다.{" "}
                <code className="text-cyan-600">useMemo</code> 덕분에 tasks 참조가 같으면 재정렬하지
                않습니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong> 통계 객체도 메모이제이션됩니다.{" "}
                <code className="text-cyan-600">TaskSummaryBar</code>가{" "}
                <code className="text-cyan-600">React.memo</code>로 감싸져 있어 stats가 같으면
                리렌더링하지 않습니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong>{" "}
                <code className="text-cyan-600">useMemo</code> 호출 후에{" "}
                <code className="text-cyan-600">visible</code> 체크를 합니다. Hook 규칙상 조건부
                렌더링 전에 모든 Hook을 호출해야 하기 때문입니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                이 컴포넌트 자체는 키보드 입력을 처리하지 않습니다. 부모 컴포넌트(
                <code className="text-cyan-600">App.tsx</code>)에서
                <code className="text-cyan-600">visible</code> 상태를 토글하는 로직이 올바르게
                연결되어 있는지 확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">truncateTitle</code>이 기본적으로 40자 제한을
                적용합니다. 이는 <code className="text-cyan-600">MAX_TITLE_LENGTH</code> 상수로
                제어되며, 모듈 내부에 하드코딩되어 있어 외부에서 변경할 수 없습니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">STATUS_ICONS</code>에 정의되지 않은 상태값을
                사용하고 있습니다. 유효한 상태는 <code className="text-cyan-600">pending</code>,
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">running</code>,{" "}
                <code className="text-cyan-600">blocked</code>,
                <code className="text-cyan-600">failed</code>,{" "}
                <code className="text-cyan-600">cancelled</code>가 0이면 해당 항목은 생략됩니다.{" "}
                <code className="text-cyan-600">pending</code>은 요약 바에 표시되지 않는 항목입니다
                (완료 비율만 표시).
              </p>
            </div>
          </section>
