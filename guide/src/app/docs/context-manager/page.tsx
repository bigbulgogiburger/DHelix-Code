    ...
    style OUTPUT fill:#1a3a2a,stroke:#10b981,color:#f1f5f9`}
        ...
        <RevealOnScroll>
          <section className="mb-14">
            ...
            <ParamTable
              params={[
                { name: "maxContextTokens", type: "number", required: false, desc: "컨텍스트 윈도우 최대 토큰 수. 기본값: TOKEN_DEFAULTS.maxContextWindow" },
                { name: "compactionThreshold", type: "number", required: false, desc: "자동 압축 트리거 비율 (0-1). 기본값: 0.835 (high-tier)" },
                { name: "preserveRecentTurns", type: "number", required: false, desc: "압축 시 보존할 최근 턴 수. 기본값: 5" },
                { name: "responseReserveRatio", type: "number", required: false, desc: "LLM 응답용 예약 비율. 기본값: AGENT_LOOP.responseReserveRatio" },
                { name: "workingDirectory", type: "string", required: false, desc: "DBCODE.md 재로딩을 위한 작업 디렉토리. 기본값: process.cwd()" },
                { name: "client", type: "LLMProvider", required: false, desc: "LLM 요약에 사용할 클라이언트. 없으면 로컬 추출 fallback" },
                { name: "summaryModel", type: "string", required: false, desc: "요약에 사용할 모델 ID. 기본값: 메인 모델과 동일" },
                { name: "sessionId", type: "string", required: false, desc: "콜드 스토리지 디렉토리 스코핑용 세션 ID. 기본값: 'default'" },
                { name: "onPreCompact", type: "() => void", required: false, desc: "압축 시작 전 이벤트 콜백 (UI 알림 등)" },
                { name: "coldStorageTtlMs", type: "number", required: false, desc: "콜드 스토리지 파일 만료 시간 (ms). 기본값: 24시간" },
                { name: "rehydrationStrategy", type: "RehydrationStrategy", required: false, desc: "리하이드레이션 파일 선택 전략. 기본값: 'recency'" },
              ]}
            />
            ...
            <ParamTable
              params={[
                ...
                { name: "maxTokens", type: "number", required: true, desc: "사용 가능한 최대 토큰 수 (응답 예약분 제외)" },
                { name: "usageRatio", type: "number", required: true, desc: "사용률 (totalTokens / maxTokens, 0~1)" },
                { name: "messageCount", type: "number", required: true, desc: "메시지 배열의 길이" },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "hash", type: "string", required: true, desc: "콘텐츠의 SHA-256 해시 (앞 16자)" },
                { name: "path", type: "string", required: true, desc: "콜드 스토리지 파일의 절대 경로" },
                { name: "originalTokens", type: "number", required: true, desc: "원본 콘텐츠의 토큰 수" },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "originalTokens", type: "number", required: true, desc: "압축 전 전체 토큰 수" },
                { name: "compactedTokens", type: "number", required: true, desc: "압축 후 전체 토큰 수" },
                { name: "removedMessages", type: "number", required: true, desc: "제거된 메시지 수" },
                { name: "summary", type: "string", required: true, desc: "구조화된 대화 요약 텍스트" },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "compactionCount", type: "number", required: true, desc: "수행된 총 압축 횟수" },
                { name: "totalTokensSaved", type: "number", required: true, desc: "절약된 총 토큰 수 (micro + auto)" },
                { name: "coldStorageEntries", type: "number", required: true, desc: "현재 콜드 스토리지에 저장된 항목 수" },
                { name: "coldStorageSizeBytes", type: "number", required: true, desc: "콜드 스토리지 추정 크기 (bytes)" },
                { name: "averageCompressionRatio", type: "number", required: true, desc: "평균 압축 비율 (0~1, 낮을수록 좋음)" },
                { name: "lastCompactionAt", type: "string | null", required: true, desc: "마지막 압축 시각 (ISO 8601)" },
              ]}
            />
...
// = maxContextTokens * (1 - responseReserveRatio)`}
            ...
            <div className="bg-bg-card border border-border rounded-xl p-5 mb-4">
              ...
              <ParamTable
                params={[
                  { name: "messages", type: "readonly ChatMessage[]", required: true, desc: "사용량을 측정할 대화 메시지 배열" },
                ]}
              />
            </div>
  ...
  readonly result: CompactionResult;
              ...
              <ParamTable
                params={[
                  { name: "messages", type: "readonly ChatMessage[]", required: true, desc: "압축할 대화 메시지 배열" },
                  { name: "focusTopic", type: "string", required: false, desc: "특정 주제에 집중하여 요약 (예: /compact 'API 설계')" },
                ]}
              />
            </div>
  ...
  preserveRecentTurns: number;  // high=5, medium=4, low=3
            ...
            <Callout type="info" icon="&#x1F4A1;">
              <strong>Caveat 5: GC는 best-effort입니다.</strong> 콜드 스토리지 가비지 컬렉션은 백그라운드에서
              실행되며, 실패해도 에러를 던지지 않습니다(swallow). 이는 GC 실패가 주요 워크플로우를
              방해하지 않도록 하기 위한 설계입니다.
            </Callout>
          </section>
        </RevealOnScroll>
...
const ctx3 = new ContextManager({
  rehydrationStrategy: "mixed",
            ...
            <Callout type="warn" icon="&#x26A0;&#xFE0F;">
              삭제되거나 이동된 파일은 리하이드레이션 시 조용히 건너뜁니다(try-catch).
              오류 로그가 남지 않으므로, 파일이 재읽기되지 않았다면 파일 존재 여부를 먼저 확인하세요.
            </Callout>
    ...
    style L3_SELECT fill:#1e293b,stroke:#06b6d4,color:#f1f5f9`}
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden my-4">
              <table className="w-full border-collapse text-[13px]">
                ...
                <tbody>
                  {[
                    { name: "coldRefs", type: "Map<string, ColdStorageRef>", desc: "해시 → 콜드 스토리지 참조. 리하이드레이션 및 GC에 사용" },
                    { name: "recentFiles", type: "string[]", desc: "최근 접근 파일 경로 (최대 10개). recency 리하이드레이션에 사용" },
                    { name: "fileAccessFrequency", type: "Map<string, number>", desc: "파일별 접근 횟수. frequency 리하이드레이션에 사용" },
                    { name: "coldRefAccessCount", type: "Map<string, number>", desc: "콜드 참조 재접근 횟수. Hot Tail 우선순위에 영향" },
                    { name: "compactionCount", type: "number", desc: "총 압축 횟수. 경계 마커(boundary marker)에 표시" },
                    { name: "totalTokensSaved", type: "number", desc: "micro + auto 압축으로 절약된 총 토큰 수" },
                    { name: "totalCompressionRatios", type: "number[]", desc: "각 압축의 비율(compacted/original). 평균 계산용" },
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden my-4">
              <table className="w-full border-collapse text-[13px]">
                ...
                <tbody>
                  {[
                    { score: "100", cond: "에러 결과 (Error:, STDERR:, error:)", reason: "디버깅 컨텍스트는 항상 보존해야 함" },
                    { score: "80", cond: "쓰기 도구 (file_edit, file_write)", reason: "파일 변경(mutation) 맥락은 중요" },
  ...
  return result;
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden my-4">
              <table className="w-full border-collapse text-[13px]">
                ...
                <tbody>
                  {[
                    { cond: "usage > 80% OR coldStorage > 100개", interval: "1", meaning: "매 압축마다 GC (높은 압박)" },
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden my-4">
              <table className="w-full border-collapse text-[13px]">
                ...
                <tbody>
                  {[
                    { name: "COLD_STORAGE_ELIGIBLE_TOOLS", val: "file_read, bash_exec, grep_search, glob_search", desc: "콜드 스토리지 대상 도구 (대용량 읽기 전용)" },
                    { name: "COLD_STORAGE_MIN_TOKENS", val: "200", desc: "이 이하의 도구 출력은 인라인 유지" },
