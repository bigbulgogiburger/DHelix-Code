...
export default function CmdBugPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">/bug 버그 리포트</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="leaf" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              시스템 진단 정보를 자동 수집하여 GitHub 이슈 형태의 버그 리포트를 생성하는 슬래시
              명령어입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                이 명령어는 두 개의 내부 헬퍼 함수(
                <code className="text-cyan-600">buildGitHubIssueUrl</code>,
                <code className="text-cyan-600">formatBugReport</code>)로 구성되어 있으며, 외부
                의존성 없이 순수하게 동작합니다.
              </p>
            </div>
  ...
  style OUT fill:#dcfce7,stroke:#10b981,color:#065f46`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 고객센터에 전화했을 때 상담원이 자동으로 고객 정보(이름, 계정,
              기기)를 조회해 두는 것처럼, <code>/bug</code>는 버그 리포트에 필요한 환경 정보를
              자동으로 채워서 사용자가 설명에만 집중할 수 있게 해줍니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">buildGitHubIssueUrl</span>(
              <span className="prop">title</span>: <span className="type">string</span>,{" "}
              <span className="prop">body</span>: <span className="type">string</span>):{" "}
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "버그 제목 (80자로 잘림, '[Bug]' 접두사 자동 추가)",
                },
                { name: "body", type: "string", required: true, desc: "버그 리포트 마크다운 본문" },
              ]}
            />
            <p className="text-[13px] text-gray-600 mt-2">
              <strong>반환값:</strong> <code className="text-cyan-600">string</code> &mdash; GitHub
              이슈 생성 URL (예:{" "}
              ...
              params={[
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "사용자가 입력한 버그 설명 텍스트",
                },
                {
                  name: "context.model",
                  type: "string",
                  required: true,
                  desc: "현재 사용 중인 LLM 모델명",
                },
                {
                  name: "context.sessionId",
                  type: "string | undefined",
                  required: false,
                  desc: "현재 세션 ID (없으면 'N/A'로 표시)",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              <code className="text-cyan-600">/bug</code> 슬래시 명령어의 등록 객체입니다. 명령어
              레지스트리에 등록되어 사용자 입력을 처리합니다.
            </p>
            <ParamTable
              params={[
                { name: "name", type: '"bug"', required: true, desc: "명령어 이름" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: '"Generate a bug report with system diagnostics"',
                },
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                설명 없이 <code className="text-cyan-600">/bug</code>만 입력하면 사용법 안내가
                표시됩니다. 빈 문자열은 리포트를 생성하지 않습니다.
              </li>
              <li>
                제목은 <strong>80자</strong>로 잘립니다. 긴 설명은 본문에 전체가 포함되지만, GitHub
                이슈 제목에는 앞 80자만 표시됩니다.
              </li>
              <li>
                생성된 URL은 터미널에 출력될 뿐, 브라우저를 자동으로 열지는 않습니다. 사용자가 직접
                URL을 클릭하거나 복사해야 합니다.
              </li>
              <li>
                <code className="text-cyan-600">APP_NAME</code>과{" "}
                <code className="text-cyan-600">VERSION</code>은
                <code className="text-cyan-600">constants.ts</code>에서 가져옵니다. 버전을
                업데이트하면 리포트에 자동 반영됩니다.
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="str">
                &quot;Tool output is truncated when response exceeds 4096 tokens&quot;
              </span>
              {"\n"}
              {"\n"}
              ...
              {"\n"}
              <span className="cm">
                {"// Open in browser: https://github.com/.../issues/new?title=..."}
              </span>
              {"\n"}
              <span className="cm">{"// "}</span>
              {"\n"}
            ...
            <Callout type="warn" icon="!">
              ...
              민감한 정보가 포함되지는 않지만, 세션 ID가 노출될 수 있으므로 공개 이슈로 등록할 때
              확인하세요.
            </Callout>
            ...
            <DeepDive title="리포트에 포함되는 환경 정보 상세">
              <p className="mb-3">
                <code className="text-cyan-600">formatBugReport()</code>가 자동으로 수집하는
                정보입니다:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>APP_NAME + VERSION</strong> &mdash; constants.ts에서 가져온 앱 이름과 버전
                </li>
                <li>
                  <strong>Platform</strong> &mdash; <code>process.platform</code> +{" "}
                  <code>process.arch</code> (예: darwin arm64)
                </li>
                <li>
                  <strong>Node.js</strong> &mdash; <code>process.version</code> (예: v20.11.0)
                </li>
                <li>
                  <strong>Model</strong> &mdash; 현재 세션의 LLM 모델명 (context에서 전달)
                </li>
                <li>
                  <strong>Session</strong> &mdash; 현재 세션 ID (없으면 &quot;N/A&quot;)
                </li>
                <li>
                  <strong>Timestamp</strong> &mdash; ISO 8601 형식의 현재 시각
                </li>
              </ul>
            </DeepDive>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">execute()</code> 함수의 분기 로직입니다. 설명이
              비어있으면 사용법을 반환하고, 있으면 리포트를 생성합니다.
            </p>
  ...
  style OUTPUT fill:#dcfce7,stroke:#10b981,color:#065f46`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              ...
              <code className="text-cyan-600">URLSearchParams</code>를 사용하여 안전하게
              인코딩합니다.
            </p>
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">buildGitHubIssueUrl</span>(
              ...
              {"  "}
              <span className="cm">
                {"// [1] URL 파라미터 구성 — 제목 80자 제한 + [Bug] 접두사"}
              </span>
              {"\n"}
              {"  "}
              <span className="kw">const</span> <span className="prop">params</span> ={" "}
              <span className="kw">new</span> <span className="fn">URLSearchParams</span>({"{"}
              {"\n"}
              {"    "}
              <span className="prop">title</span>: <span className="str">`[Bug] ${"{"}</span>
              <span className="prop">title</span>.<span className="fn">slice</span>(
              <span className="num">0</span>, <span className="num">80</span>)
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                ...
                <code className="text-cyan-600">URLSearchParams</code>가 자동으로 특수문자를
                인코딩합니다. 제목은 80자로 잘려 GitHub UI에서 적절히 표시됩니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong> GitHub의{" "}
                <code className="text-cyan-600">/issues/new</code> 엔드포인트는 쿼리 파라미터로
                제목, 본문, 라벨을 받아 미리 채워진 이슈 생성 폼을 보여줍니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                터미널 에뮬레이터에 따라 URL 클릭이 지원되지 않을 수 있습니다. URL을 복사하여
                브라우저에 직접 붙여넣으세요. 또한 URL이 너무 길면 (본문이 매우 긴 경우) 일부
                브라우저에서 잘릴 수 있습니다.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">context.model</code>이 올바르게 전달되지 않은
                경우입니다. 명령어 레지스트리가 context를 올바르게 주입하고 있는지 확인하세요.
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">package.json</code>의 version 필드와 동기화해야
                합니다.
              </p>
            </div>
          </section>
