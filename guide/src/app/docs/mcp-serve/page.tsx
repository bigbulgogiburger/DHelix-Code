...
export default function MCPServePage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">MCPServer</span>
            </h1>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                ...
                <code className="text-cyan-600">initialize</code>,{" "}
                <code className="text-cyan-600">tools/list</code>,
                <code className="text-cyan-600">tools/call</code>,{" "}
                <code className="text-cyan-600">ping</code>
                메서드를 지원합니다. 보안을 위해 기본적으로 &quot;safe&quot; 권한 레벨의 도구만
                노출합니다.
              </p>
              <p>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                ...
                },
                {
                  ...
                  type: "string",
                  required: false,
                  desc: "서버 버전 (기본: 상수에서 가져옴)",
                },
                {
                  name: "toolRegistry",
                  type: "ToolRegistry",
                  required: true,
                  desc: "노출할 도구가 등록된 레지스트리",
                },
                {
                  name: "exposedTools",
                  type: "readonly string[]",
                  required: false,
                  desc: "노출할 도구 화이트리스트 (기본: 모든 safe 도구)",
                },
                {
                  name: "workingDirectory",
                  type: "string",
                  required: false,
                  desc: "도구 실행 시 작업 디렉토리 (기본: process.cwd())",
                },
                {
                  name: "stdin",
                  type: "NodeJS.ReadableStream",
                  required: false,
                  desc: "stdin 오버라이드 (테스트용)",
                },
                {
                  name: "stdout",
                  type: "NodeJS.WritableStream",
                  required: false,
                  desc: "stdout 오버라이드 (테스트용)",
                },
              ]}
            />
            ...
            <CodeBlock>
              ...
              <span className="type">MCPServeConfig</span>)
            </CodeBlock>

            {/* start / stop */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">start() / stop()</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              stdin에서 JSON-RPC 메시지 수신을 시작/중지합니다.
            </p>
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">start</span>():{" "}
              <span className="type">Promise</span>&lt;<span className="type">void</span>&gt;
              {"\n"}
              <span className="kw">async</span> <span className="fn">stop</span>():{" "}
              <span className="type">Promise</span>&lt;<span className="type">void</span>&gt;
            </CodeBlock>

            {/* handleMessage */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">handleMessage(message)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              단일 JSON-RPC 메시지를 처리합니다. stdin/stdout 없이 직접 테스트할 수 있도록
              공개됩니다.
            </p>
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">handleMessage</span>(
            ...
            <p className="text-[13px] text-gray-600 mb-3">서버 상태를 확인합니다.</p>
            <CodeBlock>
              ...
              {"\n"}
              <span className="cm">
                // isInitialized: 클라이언트의 &quot;initialize&quot; 핸드셰이크 완료 여부
              </span>
            </CodeBlock>

            {/* getExposedToolNames */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">getExposedToolNames()</h4>
            <p className="text-[13px] text-gray-600 mb-3">노출된 도구 이름 집합을 반환합니다.</p>
            <CodeBlock>
              <span className="fn">getExposedToolNames</span>():{" "}
              <span className="type">ReadonlySet</span>&lt;<span className="type">string</span>&gt;
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                <code className="text-cyan-600">initialize</code> 메서드를 먼저 호출하지 않으면 다른
                모든 메서드가 <code className="text-red-600">INVALID_REQUEST</code> 에러를
                반환합니다.
              </li>
              <li>
              ...
              <li>
                도구 실행 기본 타임아웃은 30초입니다. 도구 정의의{" "}
                <code className="text-cyan-600">timeoutMs</code>로 개별 타임아웃을 설정할 수
                있습니다.
              </li>
              <li>
                도구 실행 중 에러가 발생하면 JSON-RPC 에러 응답이 아닌 성공 응답 +{" "}
                <code className="text-cyan-600">isError: true</code>로 반환됩니다. (MCP 스펙 준수)
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="kw">new</span> <span className="fn">MCPServer</span>({"{"}
              {"\n"}
              {"  "}
              <span className="prop">toolRegistry</span>: <span className="prop">myRegistry</span>,
              {"\n"}
              {"  "}
              <span className="prop">exposedTools</span>: [
              <span className="str">&quot;Read&quot;</span>,{" "}
              <span className="str">&quot;Grep&quot;</span>,{" "}
              <span className="str">&quot;Glob&quot;</span>],
              {"\n"}
              {"}"});
              {"\n"}
              {"\n"}
              ...
              {"\n"}
              <span className="kw">await</span> <span className="prop">server</span>.
              <span className="fn">start</span>();
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> MCP 서버는 stdin/stdout을 점유합니다. 서버 모드에서는 일반적인
              CLI 상호작용이 불가능합니다. 서버를 중지하려면 stdin을 닫거나 <code>stop()</code>을
              호출하세요.
            </Callout>
            ...
            <CodeBlock>
              ...
              {"\n"}
              {"{"}
              <span className="str">&quot;jsonrpc&quot;</span>:{" "}
              <span className="str">&quot;2.0&quot;</span>,{" "}
              <span className="str">&quot;id&quot;</span>: <span className="num">1</span>,{" "}
              ...
              {"\n"}
              {"  "}
              <span className="str">&quot;protocolVersion&quot;</span>:{" "}
              ...
              <span className="str">&quot;jsonrpc&quot;</span>:{" "}
              <span className="str">&quot;2.0&quot;</span>,{" "}
              <span className="str">&quot;id&quot;</span>: <span className="num">2</span>,{" "}
              <span className="str">&quot;method&quot;</span>:{" "}
              <span className="str">&quot;tools/list&quot;</span>
              {"}"}
              {"\n"}
              {"\n"}
              ...
              <span className="str">&quot;jsonrpc&quot;</span>:{" "}
              <span className="str">&quot;2.0&quot;</span>,{" "}
              <span className="str">&quot;id&quot;</span>: <span className="num">3</span>,{" "}
              <span className="str">&quot;method&quot;</span>:{" "}
              <span className="str">&quot;tools/call&quot;</span>,{"\n"}
              {"  "}
              <span className="str">&quot;params&quot;</span>: {"{"}{" "}
              <span className="str">&quot;name&quot;</span>:{" "}
              <span className="str">&quot;Grep&quot;</span>,{" "}
              <span className="str">&quot;arguments&quot;</span>: {"{"}{" "}
              <span className="str">&quot;pattern&quot;</span>:{" "}
              <span className="str">&quot;TODO&quot;</span> {"}"} {"}"}
              {"\n"}
              {"}"}
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">handleMessage()</code>를 직접 호출하여 stdin/stdout
              없이 서버를 테스트할 수 있습니다.
            </p>
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">server</span> ={" "}
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">initResp</span> ={" "}
              <span className="kw">await</span> <span className="prop">server</span>.
              <span className="fn">handleMessage</span>({"\n"}
              {"  "}
              <span className="fn">JSON</span>.<span className="fn">stringify</span>({"{"}{" "}
              <span className="prop">jsonrpc</span>: <span className="str">&quot;2.0&quot;</span>,{" "}
              <span className="prop">id</span>: <span className="num">1</span>,{" "}
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">listResp</span> ={" "}
              <span className="kw">await</span> <span className="prop">server</span>.
              <span className="fn">handleMessage</span>({"\n"}
              {"  "}
              <span className="fn">JSON</span>.<span className="fn">stringify</span>({"{"}{" "}
              <span className="prop">jsonrpc</span>: <span className="str">&quot;2.0&quot;</span>,{" "}
              <span className="prop">id</span>: <span className="num">2</span>,{" "}
              <span className="prop">method</span>:{" "}
              <span className="str">&quot;tools/list&quot;</span> {"}"}){"\n"});
            </CodeBlock>
            ...
            <Callout type="tip" icon="*">
              ...
              <code>handleMessage()</code>는 알림에 대해 <code>null</code>을 반환하며, 응답을 보내지
              않습니다.
            </Callout>

            <DeepDive title="도구 노출 결정 로직">
              <p className="mb-3">노출할 도구는 두 가지 방식으로 결정됩니다:</p>
              <CodeBlock>
              ...
              <p className="mt-3 text-gray-600">
                보안 민감한 환경에서는 <code className="text-cyan-600">exposedTools</code>를
                명시적으로 지정하여 필요한 도구만 노출하는 것을 권장합니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style ERR4 fill:#fee2e2,stroke:#ef4444,color:#991b1b`}
            ...
            <CodeBlock>
              ...
              <span className="prop">toolDef</span>.<span className="prop">parameterSchema</span>.
              <span className="fn">safeParse</span>(<span className="prop">toolArgs</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// [2] AbortController로 타임아웃 제어"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">abortController</span> ={" "}
              ...
              <span className="prop">abortController</span>.<span className="fn">abort</span>(),{" "}
              <span className="prop">timeout</span>);
              {"\n"}
              {"\n"}
              <span className="cm">{"// [3] 도구 실행"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">result</span> ={" "}
              <span className="kw">await</span> <span className="prop">toolDef</span>.
              <span className="fn">execute</span>(<span className="prop">parseResult</span>.
              <span className="prop">data</span>, <span className="prop">context</span>);
              {"\n"}
              {"\n"}
              ...
              <span className="prop">text</span>: <span className="prop">result</span>.
              <span className="prop">output</span> {"}"}], <span className="prop">isError</span>:{" "}
              <span className="prop">result</span>.<span className="prop">isError</span> {"}"};
            </CodeBlock>

            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">[1]</strong> Zod 스키마로 인자를 엄격하게
                검증합니다. 실패 시 <code className="text-cyan-600">INVALID_PARAMS</code> 에러를
                반환합니다.
              </p>
              <p>
                <strong className="text-gray-900">[2]</strong>{" "}
                <code className="text-cyan-600">AbortController</code>로 타임아웃을 관리합니다. 기본
                30초 후 자동 중단됩니다.
              </p>
              <p>
                <strong className="text-gray-900">[3]</strong> 도구를 실행합니다. 에러가 발생해도
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">initialize</code> 메서드를 먼저 호출해야 합니다. MCP
                프로토콜에서 핸드셰이크는 필수입니다. 클라이언트가 연결 후 가장 먼저
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                <li>
                  <code className="text-cyan-600">exposedTools</code>를 지정하지 않았다면,{" "}
                  <code className="text-cyan-600">permissionLevel: &quot;safe&quot;</code>인 도구만
                  노출됩니다. 원하는 도구가 &quot;unsafe&quot; 레벨이면{" "}
                  <code className="text-cyan-600">exposedTools</code>에 명시적으로 추가하세요.
                </li>
                <li>
                  <code className="text-cyan-600">exposedTools</code>를 지정했다면, 도구 이름이
                  정확한지 확인하세요. (대소문자 구분)
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                기본 타임아웃은 30초입니다. 오래 걸리는 도구(대규모 파일 검색 등)에서는 도구 정의의{" "}
                <code className="text-cyan-600">timeoutMs</code>를 늘려야 합니다.
                <code className="text-cyan-600">AbortController</code>가 타임아웃 시 도구 실행을
                취소합니다.
              </p>
            </div>
          </section>
