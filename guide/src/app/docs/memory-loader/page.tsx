...
export default function MemoryLoaderPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mb-3">
              <span className="text-gray-900">Memory Loader</span>
            </h1>
            <p className="text-[16px] text-gray-600 max-w-[640px]">
              프로젝트 메모리 파일을 디스크에서 읽어오는 모듈입니다. 세션 시작 시 MEMORY.md를 읽어
              시스템 프롬프트에 주입하여, AI가 이전 세션에서 학습한 내용을{" "}
              <span className="text-cyan-600 font-semibold">기억</span>하도록 합니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              메모리 로더는 MEMORY.md를 <code className="text-cyan-600 text-xs">maxLines</code>
              까지만 읽어
              <span className="text-violet-600 font-semibold"> 컨텍스트 윈도우를 절약</span>합니다.
              파일이 없으면 에러를 던지지 않고 빈 결과를{" "}
              <strong className="text-gray-900">우아하게(gracefully)</strong> 반환합니다. overflow된
              토픽 파일에 대한 개별 접근도 지원합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>컨텍스트 절약:</strong> MEMORY.md가 1000줄이어도 기본값 200줄까지만
              로드합니다. 나머지는 토픽 파일로 분리되어 필요할 때{" "}
              <code className="text-cyan-600 text-xs">loadTopicMemory()</code>로 접근합니다.
            </Callout>
  ...
  style ERROR fill:#f1f5f9,stroke:#ef4444,color:#1e293b`}
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                MEMORY.md의 처음 <code className="text-cyan-600 text-xs">maxLines</code> 줄을 읽어
                반환합니다. 파일이 없으면 빈 결과를 반환하고, 읽기 실패 시에만 에러를 던집니다.
              </p>
                ...
                <ParamTable
                  params={[
                    ...
                    },
                    {
                      ...
                      type: "boolean",
                      required: true,
                      desc: "파일 존재 여부 (없으면 false)",
                    },
                    {
                      name: "topicFiles",
                      type: "readonly string[]",
                      required: true,
                      desc: "overflow된 토픽 파일명 목록 (정렬됨)",
                    },
                  ]}
                />
              </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                특정 토픽 파일의 내용을 읽습니다. 토픽 이름은 자동으로 안전한 파일명으로
                정규화됩니다.
              </p>
              <ParamTable
                params={[
              ...
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">
                  반환:{" "}
                  <code className="text-violet-600 text-xs">Promise&lt;string | null&gt;</code>{" "}
                  &mdash; 파일 없으면 null
                </h4>
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                메모리 디렉토리의 모든 토픽 파일 목록을 반환합니다. MEMORY.md를 제외한{" "}
                <code className="text-cyan-600 text-xs">.md</code> 파일들을 정렬하여 반환합니다.
              </p>
              <ParamTable
                params={[
                  {
              ...
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">
                  반환:{" "}
                  <code className="text-violet-600 text-xs">{"Promise<readonly string[]>"}</code>{" "}
                  &mdash; 파일명만 (경로 제외)
                </h4>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                토픽 이름을 안전한 파일명으로 정규화합니다. 소문자 변환, 특수문자 치환, 연속 하이픈
                제거를 수행합니다.
              </p>
              <ParamTable
                params={[
                  {
                    ...
                    type: "string",
                    required: true,
                    desc: '토픽 이름 (예: "User Preferences!")',
                  },
                ]}
              />
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">
                  반환: <code className="text-violet-600 text-xs">string</code> &mdash; 예:{" "}
                  <code className="text-cyan-600 text-xs">"user-preferences.md"</code>
                </h4>
              </div>

              <Callout type="warn" icon="⚠️">
                빈 문자열이나 특수문자만으로 구성된 토픽 이름은{" "}
                <code className="text-red-600 text-xs">"untitled.md"</code>로 정규화됩니다. 의미
                있는 토픽 이름을 사용하세요.
              </Callout>
            </div>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              <code className="text-cyan-600 text-xs">false</code>이면 첫 번째 세션입니다.
            </p>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">normalizeTopicFileName()</code>은 다음
              파이프라인으로 안전한 파일명을 생성합니다:
            </p>
  ...
  style EXT fill:#f8fafc,stroke:#10b981,color:#10b981,stroke-width:2px`}
            ...
            <p className="text-[13px] text-gray-600 mb-3">로더는 두 가지 에러 전략을 사용합니다:</p>
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                ...
                <div className="flex gap-3">
                  ...
                  <span>
                    파일 존재하지만 읽기 실패 &rarr;{" "}
                    <code className="text-red-600 text-xs">MemoryLoadError</code> 던짐
                  </span>
                </div>
              </div>
            </div>
            ...
            <Callout type="info" icon="📝">
              <strong>listTopicFiles 방어:</strong>{" "}
              <code className="text-cyan-600 text-xs">readdir</code> 실패 시에도 에러를 던지지 않고
              빈 배열을 반환합니다. 토픽 파일 목록은 보조 정보이므로 실패가 전체 로딩을 방해하지
              않도록 설계되었습니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> MemoryLoadError: Failed to load project
                  memory
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인:</strong> MEMORY.md 파일이 존재하지만
                    읽기 권한이 없거나 인코딩 문제가 있습니다. 파일이 UTF-8로 인코딩되어 있는지
                    확인하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    ...
                    <code className="text-cyan-600 text-xs">maxLines</code> (기본 200)까지만
                    로드하여 컨텍스트 윈도우를 절약합니다. 나머지는 토픽 파일로 분리되어 있으며
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    ...
                    실제 파일명을 확인하세요. 예:{" "}
                    <code className="text-cyan-600 text-xs">"User Preferences"</code> &rarr;
                    <code className="text-cyan-600 text-xs"> "user-preferences.md"</code>
                  </p>
                </div>
              </div>
