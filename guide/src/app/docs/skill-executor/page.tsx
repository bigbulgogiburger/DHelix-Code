...
export default function SkillExecutorPage() {
      ...
      <div className="center-narrow">
        ...
        <div style={{ marginBottom: "48px" }}>
          ...
          <p className="text-[16px] text-gray-600 max-w-[640px]">
            스킬 실행 엔진 &mdash; SKILL.md의 본문을 파싱하여 <span className="text-cyan-600 font-semibold">변수 치환</span>과
            <span className="text-violet-600 font-semibold"> 동적 컨텍스트 주입</span>을 수행한 뒤 최종 프롬프트를 생성합니다.
          </p>
        </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              Skill Executor는 로드된 스킬 정의(SkillDefinition)를 받아 <strong className="text-gray-900">3단계 파이프라인</strong>으로 처리합니다.
              먼저 <code className="text-cyan-600 text-xs">$ARGUMENTS</code>, <code className="text-cyan-600 text-xs">$0</code> 등의 변수를 실제 값으로 치환하고,
              <code className="text-cyan-600 text-xs">{" `!command`"}</code> 구문을 셸 명령 실행 결과로 교체한 뒤,
              최종 프롬프트와 실행 옵션을 반환합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>핵심 원칙:</strong> 변수 치환은 동기, 동적 컨텍스트 주입은 비동기입니다.
              셸 명령 실패 시에도 프로세스를 중단하지 않고 에러 메시지를 본문에 삽입합니다.
            </Callout>
  ...
  style OUTPUT fill:#f8fafc,stroke:#06b6d4,color:#06b6d4,stroke-width:2px`}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4">
              ...
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                <div className="flex gap-3">
                  <span className="text-amber-600 font-bold shrink-0 w-36"><code className="text-xs">$ARGUMENTS</code></span>
                  <span>전체 인자 문자열 (예: <code className="text-cyan-600 text-xs">/commit fix auth bug</code> &rarr; <code className="text-cyan-600 text-xs">fix auth bug</code>)</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-violet-600 font-bold shrink-0 w-36"><code className="text-xs">$ARGUMENTS[N]</code></span>
                  <span>N번째 인자 접근 (0부터 시작)</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-blue-600 font-bold shrink-0 w-36"><code className="text-xs">$0, $1, $2...</code></span>
                  <span>위치별 인자 &mdash; <code className="text-cyan-600 text-xs">$ARGUMENTS[N]</code>과 동일</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-emerald-600 font-bold shrink-0 w-36"><code className="text-xs">{"${DBCODE_*}"}</code></span>
                  <span>세션 ID, 스킬 디렉토리, 프로젝트 디렉토리 환경 변수</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-red-600 font-bold shrink-0 w-36"><code className="text-xs">{"`!command`"}</code></span>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                ...
                에러 코드는 <code className="text-red-600 text-xs">SKILL_EXECUTION_ERROR</code>입니다.
                본문이 비어있는 스킬 실행 시 이 에러가 발생합니다.
              </p>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                스킬을 실행하여 최종 프롬프트를 생성하는 메인 함수입니다.
                변수 치환 &rarr; 동적 컨텍스트 주입 &rarr; 결과 조립의 3단계를 수행합니다.
              </p>
              ...
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">반환 타입: <code className="text-violet-600 text-xs">{"Promise<SkillExecutionResult>"}</code></h4>
                <ParamTable
                  params={[
                    {
                      name: "prompt",
              ...
              <Callout type="warn" icon="⚠️">
                스킬 본문이 비어있으면 <code className="text-red-600 text-xs">SkillExecutionError</code>를 던집니다.
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                스킬 본문의 변수 플레이스홀더를 SkillContext의 실제 값으로 치환합니다.
                정규식 기반으로 동기 처리됩니다.
              </p>
              <ParamTable
                params={[
            ...
            <div className="mb-4">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                <code className="text-cyan-600 text-xs">{"`!command`"}</code> 백틱 구문을 찾아 셸 명령을 실행하고,
                결과로 교체합니다. 명령은 순차 실행됩니다(순서 의존성 가능).
              </p>
              <ParamTable
                params={[
                  {
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>기본 스킬 실행</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">executeSkill()</code>에 로드된 스킬과 런타임 컨텍스트를 전달하면
              변수가 치환된 최종 프롬프트를 받을 수 있습니다.
            </p>
            ...
            <CodeBlock>
              ...
              <span className="text-[#8b949e]">{"// result.model   → undefined (세션 기본 모델 사용)"}</span>
            </CodeBlock>

            <DeepDive title="동적 컨텍스트 주입 예시">
              <p className="mb-3">
                스킬 본문에 <code className="text-cyan-600 text-xs">{"`!command`"}</code> 구문을 넣으면
                실행 시점에 셸 명령 결과가 자동으로 삽입됩니다.
              </p>

              <CodeBlock>
                <span className="text-[#8b949e]">{"// 스킬 본문 (변환 전)"}</span>{"\n"}
                <span className="text-[#c9d1d9]">{"현재 브랜치: `!git branch --show-current`"}</span>{"\n"}
              ...
              <Callout type="warn" icon="⚠️">
                셸 명령 타임아웃은 <strong>10초</strong>입니다.
                명령 실패 시 <code className="text-cyan-600 text-xs">[Command failed: ...]</code> 메시지가 본문에 삽입되며,
                프로세스 전체가 중단되지는 않습니다.
              </Callout>
            </DeepDive>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>변수 치환 순서</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">interpolateVariables()</code>는 정규식을 순서대로 적용합니다.
              <code className="text-cyan-600 text-xs"> $ARGUMENTS</code>를 먼저 치환하여 <code className="text-cyan-600 text-xs">$ARGUMENTS[N]</code>과 충돌을 방지합니다.
            </p>
  ...
  style DONE fill:#f8fafc,stroke:#06b6d4,color:#06b6d4,stroke-width:2px`}
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>동적 컨텍스트의 순차 실행</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">{"`!command`"}</code> 구문은 병렬이 아닌 <strong className="text-gray-900">순차적으로</strong> 실행됩니다.
              이전 명령의 결과가 다음 명령에 영향을 줄 수 있기 때문입니다(순서 의존성).
            </p>
            ...
            <Callout type="info" icon="📝">
              ...
              타임아웃 <strong>10초</strong>(<code className="text-cyan-600 text-xs">COMMAND_TIMEOUT_MS</code>)가 설정되어 있습니다.
              실패 시 에러를 던지지 않고 <code className="text-cyan-600 text-xs">[Command failed: ...]</code> 문자열을 반환합니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> 스킬 실행 시 "[Command failed: ...]" 메시지가 프롬프트에 포함돼요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인:</strong> <code className="text-cyan-600 text-xs">{"`!command`"}</code> 구문의 셸 명령이 실패했습니다.
                    명령어가 올바른지, 해당 프로그램이 설치되어 있는지 확인하세요.
                  </p>
                  <p>
                    <strong className="text-gray-900">해결:</strong> 명령을 터미널에서 직접 실행하여 정상 동작하는지 확인하세요.
                    타임아웃(10초) 초과 시에도 실패합니다.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <code className="text-cyan-600 text-xs">/commit</code>처럼 인자 없이 명령을 호출하면
                    <code className="text-cyan-600 text-xs"> $ARGUMENTS</code>는 빈 문자열로 치환됩니다.
                    인자가 필수인 스킬은 프론트매터에 <code className="text-cyan-600 text-xs">argumentHint</code>를 설정하여
                    사용법을 안내하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> SkillExecutionError: Skill has no body content
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    프론트매터만 있고 본문이 없는 스킬 파일입니다.
                    두 번째 <code className="text-cyan-600 text-xs">---</code> 이후에 프롬프트 텍스트를 작성하세요.
                  </p>
                </div>
              </div>
            </div>
