...
export default function SkillCommandBridgePage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mb-3">
              <span className="text-gray-900">Skill Command Bridge</span>
            </h1>
            <p className="text-[16px] text-gray-600 max-w-[640px]">
              슬래시 명령(/command)과 스킬 시스템을 연결하는 브릿지 모듈입니다. 사용자 호출 가능한
              스킬을 <span className="text-cyan-600 font-semibold">SlashCommand</span>로 변환하고,
              <span className="text-violet-600 font-semibold"> inline/fork</span> 분기 처리를
              수행합니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              사용자가 <code className="text-cyan-600 text-xs">/commit</code>이나{" "}
              <code className="text-cyan-600 text-xs">/review-pr</code>처럼 스킬 기반 명령어를
              입력하면, 이 브릿지 모듈이 <strong className="text-gray-900">SkillManager</strong>를
              통해 해당 스킬을 찾아 실행하고, 결과를{" "}
              <span className="text-cyan-600 font-semibold">CommandResult</span>로 변환합니다.
              <strong className="text-gray-900"> inline</strong> 스킬은 프롬프트를 사용자 메시지로
              주입하고,
              <strong className="text-gray-900"> fork</strong> 스킬은 서브에이전트를 생성합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>핵심 원칙:</strong> 스킬의 프론트매터에서{" "}
              <code className="text-cyan-600 text-xs">userInvocable: true</code>인 것만 슬래시
              명령어로 등록됩니다. <code className="text-cyan-600 text-xs">[skill]</code> 접두사로
              일반 명령어와 구분됩니다.
            </Callout>
  ...
  style SUBAGENT fill:#f8fafc,stroke:#06b6d4,color:#06b6d4,stroke-width:2px`}
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                SkillManager에서 사용자 호출 가능한(userInvocable) 스킬을 가져와 각각을{" "}
                <code className="text-cyan-600 text-xs">SlashCommand</code> 인터페이스에 맞게
                래핑합니다. 반환된 명령어 배열은{" "}
                <code className="text-cyan-600 text-xs">CommandRegistry</code>에 등록됩니다.
              </p>
              ...
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">
                  반환 타입:{" "}
                  <code className="text-violet-600 text-xs">{"readonly SlashCommand[]"}</code>
                </h4>
                <ParamTable
                  params={[
                    {
              ...
              <Callout type="warn" icon="⚠️">
                <code className="text-cyan-600 text-xs">userInvocable: false</code>인 스킬은 슬래시
                명령어로 등록되지 않습니다. LLM 전용 스킬(시스템 내부용)은 이 플래그를 false로
                설정하세요.
              </Callout>
            </div>
            ...
            <div className="mb-4">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                각 SlashCommand의 실행 핸들러입니다. SkillManager를 통해 스킬을 실행하고, 결과에
                따라 <strong className="text-gray-900">inline</strong>(프롬프트 주입) 또는{" "}
                <strong className="text-gray-900">fork</strong>(서브에이전트 생성)를 처리합니다.
              </p>
              <ParamTable
                params={[
                  {
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            >
              CommandRegistry에 스킬 명령어 등록
            </h3>
            <p className="text-[13px] text-gray-600 mb-3">
              앱 초기화 시 <code className="text-cyan-600 text-xs">createSkillCommands()</code>를
              호출하여 스킬 기반 슬래시 명령어를 레지스트리에 일괄 등록합니다.
            </p>
            ...
            <CodeBlock>
              ...
              <span className="text-[#ff7b72]">const</span>{" "}
              <span className="text-[#79c0ff]">cmd</span> <span className="text-[#ff7b72]">of</span>{" "}
              <span className="text-[#79c0ff]">skillCommands</span>
              <span className="text-[#c9d1d9]">) {"{"}</span>
              {"\n"}
            ...
            <DeepDive title="fork 스킬의 이벤트 기반 실행">
              <p className="mb-3">
                fork 스킬은 <code className="text-cyan-600 text-xs">skill:fork</code> 이벤트를
                발행하여 에이전트 루프가 서브에이전트를 생성하도록 합니다. 이벤트 페이로드에는
                프롬프트, 모델, 에이전트 유형, 허용 도구가 포함됩니다.
              </p>
              ...
              <Callout type="warn" icon="⚠️">
                ...
                <code className="text-cyan-600 text-xs">
                  {"Skill 'name' launched as general subagent."}
                </code>{" "}
                형태입니다. 실제 실행은 에이전트 루프의 이벤트 핸들러가 담당하며, 별도 컨텍스트에서
                진행됩니다.
              </Callout>
            </DeepDive>
          </section>
  ...
  style INJECT fill:#f8fafc,stroke:#10b981,color:#10b981,stroke-width:2px`}
            ...
            <h3
              ...
              style={{ marginTop: "32px", marginBottom: "16px" }}
            >
              shouldInjectAsUserMessage 동작
            </h3>
            <p className="text-[13px] text-gray-600 mb-3">
              inline 스킬에서{" "}
              <code className="text-cyan-600 text-xs">shouldInjectAsUserMessage: true</code>로
              반환하면, 에이전트 루프의 <code className="text-cyan-600 text-xs">handleSubmit</code>
              이 이 출력을 화면에 표시하지 않고 LLM에 사용자 메시지로 직접 전달합니다. 이를 통해
              스킬이 생성한 프롬프트가 자연스럽게 대화에 주입됩니다.
            </p>

            <Callout type="info" icon="📝">
              <code className="text-cyan-600 text-xs">[skill]</code> 접두사는{" "}
              <code className="text-cyan-600 text-xs">/help</code> 명령어에서 스킬 기반 명령어와
              내장 명령어를 구분하기 위해 추가됩니다. 예:{" "}
              <code className="text-cyan-600 text-xs">[skill] Git 커밋 메시지 생성</code>
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> 스킬이 /help에 나타나지만 실행하면
                  "failed to execute" 에러가 나요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인 1:</strong> 스킬 본문(body)이
                    비어있습니다. 프론트매터 이후에 프롬프트 텍스트가 있는지 확인하세요.
                  </p>
                  <p>
                    <strong className="text-gray-900">원인 2:</strong> SkillManager.execute()가
                    내부적으로 실패했습니다. 동적 컨텍스트(
                    <code className="text-cyan-600 text-xs">{"`!command`"}</code>)에서 치명적 에러가
                    발생했을 수 있습니다.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> fork 스킬을 실행했는데 아무 일도 안
                  일어나요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <code className="text-cyan-600 text-xs">skill:fork</code> 이벤트를 수신하는
                    리스너가 등록되어 있는지 확인하세요. 에이전트 루프가 이 이벤트를 핸들링하여
                    서브에이전트를 생성합니다. 리스너가 없으면 이벤트는 발행되지만 아무 동작도
                    일어나지 않습니다.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    프론트매터에 <code className="text-cyan-600 text-xs">userInvocable: false</code>
                    가 설정되어 있지 않은지 확인하세요. 기본값은{" "}
                    <code className="text-cyan-600 text-xs">true</code>이지만, 명시적으로 false로
                    설정하면
                    <code className="text-cyan-600 text-xs"> createSkillCommands()</code>에서
                    제외됩니다.
                  </p>
                </div>
              </div>
