...
export default function MCPManagedConfigPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">MCPManagedConfig</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="infra" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              기업/조직 환경에서 관리자가 MCP 서버 설정에 대한 정책을 강제하는 관리형 설정
              모듈입니다.
            </p>
          </div>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              ...
              <p>
                ...
                <code className="text-cyan-600">readOnly: true</code>로 설정한 서버는 사용자가
                수정할 수 없고,
                <code className="text-cyan-600">enforceAllowlist: true</code>이면 관리자가 허용한
                도구만 사용 가능합니다.
              </p>
              <p>
                설정 파일이 없으면 모든 정책이 허용적(permissive)으로 동작하므로, 개인 환경에서는 이
                모듈의 존재를 의식할 필요가 없습니다. Zod 스키마를 사용하여 설정 파일의 유효성을
                런타임에 검증합니다.
              </p>
            </div>
  ...
  style TF fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 회사에서 IT 관리자가 직원 컴퓨터에 소프트웨어 설치 정책을
              적용하는 것과 같습니다. 관리자가 &quot;승인된 소프트웨어만 설치 가능&quot;이라는
              정책을 설정하면, 직원은 목록에 없는 소프트웨어를 설치할 수 없습니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                ...
                { name: "env", type: "Record<string, string>", required: false, desc: "환경 변수" },
                {
                  ...
                  type: "string[]",
                  required: false,
                  desc: "허용할 도구 목록 — 이 목록의 도구만 사용 가능",
                },
                {
                  name: "blockedTools",
                  type: "string[]",
                  required: false,
                  desc: "차단할 도구 목록 — 이 목록의 도구는 사용 불가",
                },
                {
                  name: "enforceAllowlist",
                  type: "boolean",
                  required: false,
                  desc: "true면 사용자가 추가 도구를 허용할 수 없음",
                },
                {
                  name: "readOnly",
                  type: "boolean",
                  required: false,
                  desc: "true면 사용자가 이 서버 설정을 수정할 수 없음",
                },
                {
            ...
            <ParamTable
              params={[
                {
                  name: "allowUserServers",
                  type: "boolean",
                  required: true,
                  desc: "사용자가 자체 서버를 추가할 수 있는지 (기본: true)",
                },
                {
                  name: "requireApproval",
                  type: "boolean",
                  required: true,
                  desc: "새 서버 추가 시 관리자 승인 필요 여부 (기본: false)",
                },
                {
                  name: "maxServers",
                  type: "number",
                ...
                {
                  name: "blockedTransports",
                  type: '("stdio" | "http" | "sse")[]',
                  required: true,
                  desc: "차단할 트랜스포트 타입 목록 (기본: 빈 배열)",
                },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "valid", type: "boolean", required: true, desc: "검증 통과 여부" },
                {
                  ...
                  required: true,
                  desc: "검증 실패 사유 목록 (차단 사유)",
                },
                {
                  name: "warnings",
                  type: "readonly string[]",
                  required: true,
                  desc: "경고 목록 (차단하지는 않지만 주의 필요)",
                },
              ]}
            />
            ...
            <CodeBlock>
              <span className="fn">detectTransportType</span>(<span className="prop">config</span>:{" "}
              <span className="type">RawServerConfigInput</span>):{" "}
              <span className="str">&quot;stdio&quot;</span> |{" "}
              <span className="str">&quot;http&quot;</span> |{" "}
              <span className="str">&quot;sse&quot;</span>
            </CodeBlock>
            <div className="text-[13px] text-gray-600 mt-2 space-y-1">
              <p>
                &bull; <code className="text-cyan-600">transport</code>가 명시되어 있으면 해당 값
                사용
              </p>
              <p>
                &bull; <code className="text-cyan-600">url</code>이 있으면 &rarr;{" "}
                <code className="text-emerald-600">&quot;http&quot;</code>
              </p>
              <p>
                &bull; <code className="text-cyan-600">command</code>가 있으면 &rarr;{" "}
                <code className="text-emerald-600">&quot;stdio&quot;</code>
              </p>
              <p>
                &bull; 기본값 &rarr; <code className="text-emerald-600">&quot;stdio&quot;</code>
              </p>
            </div>
            ...
            <CodeBlock>
              <span className="kw">constructor</span>(<span className="prop">configPath</span>?:{" "}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              디스크에서 관리자 설정을 로드하고 Zod 스키마로 검증합니다. 파일이 없으면 기본 허용적
              설정을 반환합니다.
            </p>
            <CodeBlock>
              <span className="kw">async</span> <span className="fn">load</span>():{" "}
              <span className="type">Promise</span>&lt;<span className="type">ManagedConfig</span>
              &gt;
            </CodeBlock>

            {/* getManagedServers */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">getManagedServers()</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              관리자가 사전 등록한 서버 목록을 반환합니다.
            </p>
            <CodeBlock>
              <span className="fn">getManagedServers</span>():{" "}
              <span className="type">ReadonlyMap</span>&lt;<span className="type">string</span>,{" "}
              <span className="type">ManagedServerConfig</span>&gt;
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="fn">mergeWithUserConfigs</span>(
              <span className="prop">userConfigs</span>: <span className="type">Record</span>&lt;
            ...
            <CodeBlock>
              <span className="fn">validateServerConfig</span>(<span className="prop">name</span>:{" "}
              <span className="type">string</span>, <span className="prop">config</span>:{" "}
              <span className="type">MCPServerConfig</span>):{" "}
              <span className="type">ManagedConfigValidationResult</span>
            </CodeBlock>
            ...
            <CodeBlock>
              ...
              <span className="prop">userAllowlist</span>?:{" "}
              <span className="type">readonly string</span>[],{" "}
              <span className="prop">userDenylist</span>?:{" "}
              <span className="type">readonly string</span>[]): {"{"}{" "}
              <span className="prop">allowlist</span>?:{" "}
              <span className="type">readonly string</span>[];{" "}
              <span className="prop">denylist</span>?: <span className="type">readonly string</span>
              [] {"}"}
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="fn">canAddServer</span>(
              <span className="prop">currentServerCount</span>: <span className="type">number</span>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              ...
              <li>
                설정 파일이 없으면 모든 정책이 허용적(permissive)으로 동작합니다. 이것은 의도된
                동작입니다.
              </li>
              <li>
                <code className="text-cyan-600">enforceAllowlist</code>가 true이면 사용자 허용
                목록은 완전히 무시되고, 관리자 허용 목록만 적용됩니다.
              </li>
              <li>
                허용 목록은 교집합(intersection), 차단 목록은 합집합(union)으로 계산됩니다. 즉, 차단
                목록은 양쪽 모두의 차단 항목을 합칩니다.
              </li>
            </ul>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-lg font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>
              기본 사용법 &mdash; 관리자 설정 로드 및 병합
            </h3>
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              가장 일반적인 패턴입니다. 관리자 설정을 로드한 뒤 사용자 설정과 병합하여 최종 서버
              목록을 생성합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// 1. 인스턴스 생성 (기본 경로 사용)"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">managedConfig</span> ={" "}
              <span className="kw">new</span> <span className="fn">MCPManagedConfig</span>();
              {"\n"}
              {"\n"}
              <span className="cm">{"// 2. 디스크에서 설정 로드"}</span>
              {"\n"}
              <span className="kw">await</span> <span className="prop">managedConfig</span>.
              <span className="fn">load</span>();
              {"\n"}
              {"\n"}
              <span className="cm">{"// 3. 사용자 설정과 병합"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">userConfigs</span> = {"{"}{" "}
              ...
              <span className="prop">managedConfig</span>.
              <span className="fn">mergeWithUserConfigs</span>(
              <span className="prop">userConfigs</span>);
              {"\n"}
              {"\n"}
              <span className="cm">
                {"// merged에는 관리자 서버 + 허용된 사용자 서버가 포함됨"}
              </span>
            </CodeBlock>

            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>load()</code>를 호출하지 않고{" "}
              <code>mergeWithUserConfigs()</code>나<code>getManagedServers()</code>를 호출하면
              에러가 발생합니다. 반드시 <code>load()</code>를 먼저 호출하세요.
            </Callout>
            ...
            <CodeBlock>
              ...
              <span className="prop">managedConfig</span>.
              <span className="fn">validateServerConfig</span>(
              <span className="str">&quot;new-server&quot;</span>, {"{"}
              {"\n"}
              {"  "}
              ...
              <span className="str">&quot;https://mcp.example.com&quot;</span>,{"\n"}
              {"}"});
              {"\n"}
              {"\n"}
              <span className="kw">if</span> (!<span className="prop">result</span>.
              <span className="prop">valid</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">error</span>(
              <span className="str">&quot;차단:&quot;</span>, <span className="prop">result</span>.
              <span className="prop">errors</span>);
              {"\n"}
              {"}"}
              {"\n"}
              <span className="kw">if</span> (<span className="prop">result</span>.
              <span className="prop">warnings</span>.<span className="prop">length</span> {">"}{" "}
              <span className="num">0</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">warn</span>(
              <span className="str">&quot;경고:&quot;</span>, <span className="prop">result</span>.
              <span className="prop">warnings</span>);
              {"\n"}
              {"}"}
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="kw">const</span> <span className="prop">filter</span> ={" "}
              <span className="prop">managedConfig</span>.
              <span className="fn">getEffectiveToolFilter</span>({"\n"}
              {"  "}
              <span className="str">&quot;my-server&quot;</span>,{"\n"}
              ...
              {"\n"}
              {"  "}[<span className="str">&quot;read&quot;</span>,{" "}
              <span className="str">&quot;write&quot;</span>,{" "}
              <span className="str">&quot;search&quot;</span>],
              {"\n"}
              {"  "}
            ...
            <Callout type="tip" icon="*">
              <strong>팁:</strong> <code>enforceAllowlist: true</code>이면 사용자 허용 목록은 완전히
              무시됩니다. 관리자가 지정한 도구만 사용 가능하므로, 보안이 중요한 환경에서 권장됩니다.
            </Callout>
            ...
            <DeepDive title="설정 파일(managed-mcp.json) 예시">
              <CodeBlock>
                ...
                {"\n"}
                {"      "}
                <span className="str">&quot;transport&quot;</span>:{" "}
                <span className="str">&quot;http&quot;</span>,{"\n"}
                {"      "}
                <span className="str">&quot;url&quot;</span>:{" "}
                <span className="str">&quot;https://mcp.company.com&quot;</span>,{"\n"}
                {"      "}
                <span className="str">&quot;readOnly&quot;</span>: <span className="kw">true</span>,
                {"\n"}
                {"      "}
                <span className="str">&quot;allowedTools&quot;</span>: [
                <span className="str">&quot;search&quot;</span>,{" "}
                <span className="str">&quot;read&quot;</span>],
                {"\n"}
                {"      "}
                <span className="str">&quot;enforceAllowlist&quot;</span>:{" "}
                ...
                {"\n"}
                {"    "}
                <span className="str">&quot;allowUserServers&quot;</span>:{" "}
                <span className="kw">true</span>,{"\n"}
                {"    "}
                <span className="str">&quot;maxServers&quot;</span>: <span className="num">10</span>
                ,{"\n"}
                {"    "}
                <span className="str">&quot;blockedTransports&quot;</span>: [
                <span className="str">&quot;sse&quot;</span>]{"\n"}
                {"  "}
                {"}"}
                {"\n"}
              ...
              <p className="mt-3 text-gray-600">
                이 예시에서: <code className="text-cyan-600">company-tools</code> 서버는 읽기
                전용이며 search와 read 도구만 허용됩니다. 사용자는 최대 10개 서버를 추가할 수
                있지만, SSE 트랜스포트는 차단됩니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
  ...
  style CHK fill:#fef3c7,stroke:#f59e0b,color:#1e293b`}
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="kw">if</span> (<span className="prop">managed</span>.
              <span className="prop">enforceAllowlist</span> &&{" "}
              <span className="prop">managedAllowlist</span>) {"{"}
              {"\n"}
              {"  "}
              ...
              {"\n"}
              <span className="kw">if</span> (<span className="prop">managedAllowlist</span> &&{" "}
              <span className="prop">userAllowlist</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="kw">return</span> <span className="prop">userAllowlist</span>.
              <span className="fn">filter</span>(<span className="prop">t</span> ={">"}{" "}
              <span className="prop">managedSet</span>.<span className="fn">has</span>(
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">교집합 (Intersection)</strong> &mdash; 관리자와
                사용자 모두 허용한 도구만 통과합니다. 가장 보수적인 결과를 생성합니다.
              </p>
              <p>
                <strong className="text-gray-900">합집합 (Union)</strong> &mdash; 차단 목록은 양쪽의
                차단 항목을 모두 합칩니다. 즉, 어느 쪽에서든 차단하면 차단됩니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                관리자가 <code className="text-cyan-600">allowUserServers: false</code>로
                설정했습니다. 사용자 정의 서버를 추가할 수 없으며, 관리자가 사전 등록한 서버만 사용
                가능합니다. IT 관리자에게 서버 추가를 요청하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
                ...
                <li>
                  ...
                  <code className="text-cyan-600">MCPManagedConfigError</code>가 발생하며, 기본
                  허용적 설정으로 대체되지 않습니다.
                </li>
              </ul>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">allowedTools</code>를 설정했을 수 있습니다. 이 경우
                관리자가 명시적으로 허용한 도구만 사용 가능합니다.
                <code className="text-cyan-600">getEffectiveToolFilter()</code>로 최종 필터를
                확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">canAddServer()</code>로 추가 가능 여부를 사전에
                확인하세요. 불필요한 서버를 제거하거나, 관리자에게 제한 상향을 요청하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
