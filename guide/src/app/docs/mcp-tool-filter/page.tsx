...
export default function MCPToolFilterPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">MCPToolFilter</span>
            </h1>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                ...
                에이전트에 노출할 도구를 제어합니다. 서버별로 허용 목록(allowlist)과 차단
                목록(denylist)을 설정하여, 필요한 도구만 선택적으로 활성화할 수 있습니다.
              </p>
              <p>
                필터 규칙은 명확한 우선순위를 가집니다: (1) allowlist만 설정하면 목록의 도구만 통과,
                (2) denylist만 설정하면 목록의 도구만 차단, (3) 둘 다 설정하면 allowlist 먼저 적용
                후 denylist 적용, (4) 둘 다 미설정이면 모든 도구가 통과합니다.
              </p>
              <p>
                allowlist와 denylist에 같은 도구 이름이 있으면 모순이므로 에러를 던집니다. 이 유효성
                검사가 설정 실수를 사전에 방지합니다.
              </p>
            </div>
  ...
  style BRIDGE fill:#e0e7ff,stroke:#3b82f6,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> MCPToolFilter는 클럽의 &quot;출입 관리&quot;와 같습니다. VIP
              목록(allowlist)에 있는 사람만 입장시키거나, 블랙리스트(denylist)에 있는 사람을
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3 leading-relaxed">
              필터 설정 오류 시 발생하는 에러 클래스입니다. 에러 코드{" "}
              <code className="text-cyan-600">&quot;MCP_TOOL_FILTER_ERROR&quot;</code>로 식별됩니다.
            </p>
              ...
              params={[
                {
                  ...
                  required: false,
                  desc: "허용할 도구 이름 목록 (이 목록에 있는 도구만 통과)",
                },
                {
                  name: "denylist",
                  type: "readonly string[]",
                  required: false,
                  desc: "차단할 도구 이름 목록 (이 목록에 있는 도구는 제외)",
                },
              ]}
            />
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              서버의 필터 설정을 등록합니다. allowlist와 denylist에 중복 항목이 있으면 에러를
              던집니다.
            </p>
            <CodeBlock>
              <span className="fn">setFilter</span>(<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>, <span className="prop">config</span>:{" "}
              <span className="type">MCPToolFilterConfig</span>): <span className="type">void</span>
            </CodeBlock>
            <ParamTable
              params={[
            ...
            <CodeBlock>
              <span className="fn">filterTools</span>(<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>, <span className="prop">tools</span>:{" "}
              <span className="type">readonly MCPToolDefinition[]</span>):{" "}
              <span className="type">readonly MCPToolDefinition[]</span>
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="fn">isToolAllowed</span>(<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>, <span className="prop">toolName</span>:{" "}
              <span className="type">string</span>): <span className="type">boolean</span>
            </CodeBlock>

            {/* loadFromConfig */}
            <h4 className="text-sm font-bold text-gray-900 mb-2 mt-6">loadFromConfig(config)</h4>
            <p className="text-[13px] text-gray-600 mb-3">
              MCP 서버 설정에서 필터를 일괄 로드합니다.
              <code className="text-cyan-600">allowedTools</code>와{" "}
              <code className="text-cyan-600">blockedTools</code> 필드를 읽어 필터로 등록합니다.
            </p>
            <CodeBlock>
              <span className="fn">loadFromConfig</span>(<span className="prop">config</span>:{" "}
              <span className="type">
                Record{"<"}string, {"{"} allowedTools?: string[]; blockedTools?: string[] {"}"}
                {">"}
              </span>
            ...
            <CodeBlock>
              <span className="fn">getFilter</span>(<span className="prop">serverName</span>:{" "}
              <span className="type">string</span>):{" "}
              <span className="type">MCPToolFilterConfig | undefined</span>
              {"\n"}
              <span className="fn">removeFilter</span>(<span className="prop">serverName</span>:{" "}
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                ...
                <code className="text-cyan-600">MCPToolFilterError</code>가 발생합니다. 허용하면서
                동시에 차단하는 것은 모순이므로 사전에 차단합니다.
              </li>
              <li>
                <code className="text-cyan-600">filterTools()</code>는 원본 배열을 변경하지 않습니다
                (불변성 유지). 항상 새 배열을 반환합니다.
              </li>
              <li>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              <span className="kw">new</span> <span className="fn">MCPToolFilter</span>();
              {"\n"}
              {"\n"}
              <span className="cm">{"// allowlist: read와 search만 허용"}</span>
              {"\n"}
              <span className="prop">filter</span>.<span className="fn">setFilter</span>(
              ...
              <span className="str">&quot;drop_database&quot;</span>],
              {"\n"}
              {"}"});
              {"\n"}
              {"\n"}
              ...
              <span className="prop">filter</span>.<span className="fn">filterTools</span>(
              <span className="str">&quot;my-server&quot;</span>,{" "}
            ...
            <CodeBlock>
              ...
              {"\n"}
              {"    "}
              <span className="prop">allowedTools</span>: [
              <span className="str">&quot;read&quot;</span>,{" "}
              <span className="str">&quot;write&quot;</span>],
              {"\n"}
              {"    "}
              <span className="prop">blockedTools</span>: [
              <span className="str">&quot;delete&quot;</span>],
              {"\n"}
              {"  "}
              {"}"},{"\n"}
              ...
              {"    "}
              <span className="prop">blockedTools</span>: [
              <span className="str">&quot;dangerous_tool&quot;</span>],
              {"\n"}
              {"  "}
              {"}"},{"\n"}
              {"}"});
            ...
            <CodeBlock>
              <span className="kw">if</span> (<span className="prop">filter</span>.
              <span className="fn">isToolAllowed</span>(
              <span className="str">&quot;my-server&quot;</span>,{" "}
              <span className="str">&quot;delete&quot;</span>)) {"{"}
              {"\n"}
              {"  "}
  ...
  style HAS_FILTER fill:#f1f5f9,stroke:#64748b,color:#1e293b`}
            ...
            <CodeBlock>
              ...
              <span className="kw">this</span>.<span className="prop">filters</span>.
              <span className="fn">get</span>(<span className="prop">serverName</span>);
              {"\n"}
              {"  "}
              <span className="kw">if</span> (!<span className="prop">config</span>){" "}
              <span className="kw">return</span> <span className="prop">tools</span>;{" "}
              <span className="cm">{"// 필터 없으면 전체 통과"}</span>
              {"\n"}
              {"\n"}
              {"  "}
              <span className="kw">let</span> <span className="prop">filtered</span> = [...
              <span className="prop">tools</span>];{" "}
              <span className="cm">{"// 불변성: 복사본 생성"}</span>
              {"\n"}
              {"\n"}
              {"  "}
              ...
              {"  "}
              <span className="kw">if</span> (<span className="prop">config</span>.
              <span className="prop">allowlist</span>?.length) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">const</span> <span className="prop">allowSet</span> ={" "}
              <span className="kw">new</span> <span className="fn">Set</span>(
              <span className="prop">config</span>.<span className="prop">allowlist</span>);
              {"\n"}
              {"    "}
              <span className="prop">filtered</span> = <span className="prop">filtered</span>.
              <span className="fn">filter</span>(<span className="prop">t</span> =&gt;{" "}
              <span className="prop">allowSet</span>.<span className="fn">has</span>(
              <span className="prop">t</span>.<span className="prop">name</span>));
              {"\n"}
              {"  "}
              {"}"}
              {"\n"}
              ...
              {"  "}
              <span className="kw">if</span> (<span className="prop">config</span>.
              <span className="prop">denylist</span>?.length) {"{"}
              {"\n"}
              {"    "}
              <span className="kw">const</span> <span className="prop">denySet</span> ={" "}
              <span className="kw">new</span> <span className="fn">Set</span>(
              <span className="prop">config</span>.<span className="prop">denylist</span>);
              {"\n"}
              {"    "}
              <span className="prop">filtered</span> = <span className="prop">filtered</span>.
              <span className="fn">filter</span>(<span className="prop">t</span> =&gt; !
              <span className="prop">denySet</span>.<span className="fn">has</span>(
              <span className="prop">t</span>.<span className="prop">name</span>));
              {"\n"}
              {"  "}
              {"}"}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4 text-[13px] text-gray-600 space-y-2.5">
              <p>
                <strong className="text-gray-900">Set 변환:</strong> 배열을 Set으로 변환하여{" "}
                <code>has()</code> 조회를 O(1)로 최적화합니다.
              </p>
              <p>
                <strong className="text-gray-900">불변성:</strong> 원본 배열을 스프레드 연산자(
                <code>[...tools]</code>)로 복사합니다.
              </p>
              <p>
                <strong className="text-gray-900">2단계 파이프라인:</strong> allowlist 적용 결과에
                denylist를 다시 적용하므로, 두 필터가 모두 적용됩니다.
              </p>
            </div>
            ...
            <DeepDive title="validateConfig() — 교집합 검사 상세">
              ...
              <CodeBlock>
                <span className="kw">private</span> <span className="fn">validateConfig</span>(
                <span className="prop">serverName</span>: <span className="type">string</span>,{" "}
                <span className="prop">config</span>:{" "}
                <span className="type">MCPToolFilterConfig</span>) {"{"}
                {"\n"}
                {"  "}
                <span className="kw">if</span> (<span className="prop">config</span>.
                <span className="prop">allowlist</span> && <span className="prop">config</span>.
                <span className="prop">denylist</span>) {"{"}
                {"\n"}
                {"    "}
                <span className="kw">const</span> <span className="prop">overlap</span> ={" "}
                <span className="prop">config</span>.<span className="prop">allowlist</span>.
                <span className="fn">filter</span>({"\n"}
                {"      "}
                <span className="prop">name</span> =&gt; <span className="prop">config</span>.
                <span className="prop">denylist</span>!.<span className="fn">includes</span>(
                <span className="prop">name</span>){"\n"}
                {"    "});
                {"\n"}
                {"    "}
                <span className="kw">if</span> (<span className="prop">overlap</span>.
                <span className="prop">length</span> {">"} <span className="num">0</span>) {"{"}
                {"\n"}
                {"      "}
                <span className="kw">throw new</span> <span className="fn">MCPToolFilterError</span>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">blockedTools</code> 모두에 포함되어 있습니다. 에러의{" "}
                <code className="text-cyan-600">context.overlapping</code>에 중복된 도구 이름이
                표시됩니다. 하나의 목록에서만 사용하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                allowlist의 도구 이름이 MCP 서버가 제공하는 도구 이름과 정확히 일치하는지
                확인하세요. 대소문자가 다르면 매칭되지 않습니다.
              </p>
              <Callout type="tip" icon="*">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                이것은 정상 동작입니다. 필터가 설정되지 않은 서버의 도구는 기본적으로 모두
                허용됩니다. 도구를 제한하고 싶다면 allowlist 또는 denylist를 명시적으로 설정하세요.
              </p>
            </div>
          </section>
        </RevealOnScroll>
