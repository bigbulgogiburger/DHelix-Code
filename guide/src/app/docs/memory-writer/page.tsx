...
export default function MemoryWriterPage() {
      ...
      <div className="center-narrow">
        ...
        <div style={{ marginBottom: "48px" }}>
          ...
          <p className="text-[16px] text-gray-600 max-w-[640px]">
            ...
            <span className="text-emerald-600 font-semibold">자동 overflow</span>를 통해 데이터 안정성을 보장합니다.
          </p>
        </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              메모리 라이터는 MEMORY.md에 새 항목을 추가하거나 전체를 덮어쓰는 쓰기 작업을 담당합니다.
              모든 파일 쓰기는 <strong className="text-gray-900">원자적(atomic)</strong>으로 수행되어,
              프로세스가 중간에 죽어도 기존 파일이 손상되지 않습니다.
              MEMORY.md가 <code className="text-cyan-600 text-xs">maxLines</code>를 초과하면
              오래된 섹션을 <span className="text-violet-600 font-semibold">토픽 파일</span>로 자동 분리합니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>원자적 쓰기 원리:</strong> 임시 파일(<code className="text-cyan-600 text-xs">{"{file}.tmp.{timestamp}"}</code>)에 먼저 쓰고,
              <code className="text-cyan-600 text-xs"> rename()</code>으로 교체합니다.
              같은 파일시스템에서 rename은 원자적 작업이므로 부분 쓰기가 불가능합니다.
            </Callout>
  ...
  style DONE2 fill:#f8fafc,stroke:#10b981,color:#10b981,stroke-width:2px`}
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <ParamTable
                params={[
                  { name: "projectRoot", type: "string", required: true, desc: "프로젝트 루트 절대 경로" },
                  { name: "entry", type: "MemoryEntry", required: true, desc: "추가할 항목 { topic: string, content: string }" },
                  { name: "maxLines", type: "number", required: false, desc: "MEMORY.md 최대 줄 수 (기본: 200). 초과 시 overflow" },
                ]}
              />
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">반환: <code className="text-violet-600 text-xs">{"Promise<{ written: boolean, overflowed: boolean }>"}</code></h4>
              </div>
            </div>
            ...
            <div className="mb-8">
              ...
              <ParamTable
                params={[
                  { name: "projectRoot", type: "string", required: true, desc: "프로젝트 루트 경로" },
                  { name: "content", type: "string", required: true, desc: "저장할 전체 마크다운 내용" },
                ]}
              />
            </div>
            ...
            <div className="mb-8">
              ...
              <ParamTable
                params={[
                  { name: "projectRoot", type: "string", required: true, desc: "프로젝트 루트 경로" },
                  { name: "topic", type: "string", required: true, desc: "토픽 이름 (파일명으로 정규화됨)" },
                  { name: "content", type: "string", required: true, desc: "저장할 마크다운 내용" },
                ]}
              />
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">반환: <code className="text-violet-600 text-xs">{"Promise<string>"}</code> &mdash; 정규화된 파일명 (예: <code className="text-cyan-600 text-xs">"debugging.md"</code>)</h4>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                ...
                토픽 파일 삭제는 병렬로 수행되며, 개별 실패는 무시됩니다 (best-effort).
              </p>
              <ParamTable
                params={[
                  { name: "projectRoot", type: "string", required: true, desc: "프로젝트 루트 경로" },
              ...
              <Callout type="warn" icon="⚠️">
                ...
                모든 <code className="text-red-600 text-xs">.md</code> 토픽 파일(MEMORY.md 제외)을 삭제합니다.
              </Callout>
            </div>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>직접 호출 (MemoryManager 없이)</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              일반적으로는 <code className="text-cyan-600 text-xs">MemoryManager</code>를 통해 사용하지만,
              직접 호출도 가능합니다.
            </p>
            ...
            <CodeBlock>
              ...
              <span className="text-[#c9d1d9]">;</span>{"\n\n"}
              <span className="text-[#8b949e]">{"// 메모리에 항목 추가 (maxLines: 100으로 제한)"}</span>{"\n"}
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 mb-3">
                <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                  <div className="flex gap-3">
                    ...
                    <span>MEMORY.md 내용을 <code className="text-cyan-600 text-xs">## 섹션</code> 단위로 분리</span>
                  </div>
                  <div className="flex gap-3">
                  ...
                  <div className="flex gap-3">
                    ...
                    <span>나머지 섹션은 개별 토픽 파일로 저장 (예: <code className="text-cyan-600 text-xs">debugging.md</code>)</span>
                  </div>
                  <div className="flex gap-3">
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>원자적 파일 쓰기</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">atomicWrite()</code>는 데이터 무결성을 보장하는 핵심 함수입니다.
              직접 파일을 쓰는 대신, 임시 파일에 먼저 쓰고 <code className="text-cyan-600 text-xs">rename()</code>으로 교체합니다.
            </p>
  ...
  style ERROR fill:#f1f5f9,stroke:#ef4444,color:#1e293b`}
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>중복 감지 알고리즘</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">isDuplicate()</code>는 대소문자를 무시하고
              새 내용이 기존 내용의 부분 문자열인지 확인합니다.
              빈 내용이면 항상 비중복으로 판단합니다.
            </p>

            <CodeBlock>
              <span className="text-[#8b949e]">{"// 중복 감지: 대소문자 무시 + 부분 문자열 포함 검사"}</span>{"\n"}
              <span className="text-[#ff7b72]">const</span>{" "}
            ...
            <h3 className="text-[15px] font-bold" style={{ marginTop: "32px", marginBottom: "16px" }}>섹션 삽입 전략</h3>
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">appendToSection()</code>은 다음 3가지 경우를 처리합니다:
            </p>
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                <div className="flex gap-3">
                  <span className="text-cyan-600 font-bold shrink-0 w-32">파일이 비어있음</span>
                  <span><code className="text-cyan-600 text-xs"># Project Memory</code> 헤더와 함께 새로 생성</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-violet-600 font-bold shrink-0 w-32">섹션이 존재</span>
                  <span>해당 <code className="text-cyan-600 text-xs">##</code> 섹션 끝(다음 ## 직전)에 항목 추가</span>
                </div>
                <div className="flex gap-3">
                  <span className="text-emerald-600 font-bold shrink-0 w-32">섹션이 없음</span>
                  <span>파일 끝에 새 <code className="text-cyan-600 text-xs">## 섹션</code> 생성 후 항목 추가</span>
                </div>
              </div>
            </div>

            <Callout type="info" icon="📝">
              <strong>토픽 이름 정규화:</strong> <code className="text-cyan-600 text-xs">capitalize()</code>로 첫 글자를 대문자로 변환하여
              <code className="text-cyan-600 text-xs"> ## Debugging</code> 같은 일관된 섹션 헤더를 생성합니다.
            </Callout>
          </section>
        </RevealOnScroll>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> MemoryWriteError: Failed to write memory file atomically
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인 1:</strong> 디스크 공간 부족. 임시 파일 쓰기가 실패합니다.
                  </p>
                  <p>
                    <strong className="text-gray-900">원인 2:</strong> 파일 시스템 권한 문제.
                    <code className="text-cyan-600 text-xs"> ~/.dbcode/projects/</code> 디렉토리의 쓰기 권한을 확인하세요.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    ...
                    <code className="text-cyan-600 text-xs"> *.tmp.*</code> 파일을 수동으로 삭제해도 안전합니다.
                  </p>
                </div>
              </div>
            </div>
