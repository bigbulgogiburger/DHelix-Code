...
export default function ConfigLoaderPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mb-3">
              <span className="text-gray-900">Config Loader</span>
            </h1>
            <p className="text-[16px] text-gray-600 max-w-[640px]">
              5단계 계층에서 설정을 병합하여 최종 설정을 생성하는 모듈입니다. CLI 플래그부터
              기본값까지, 우선순위에 따라{" "}
              <span className="text-cyan-600 font-semibold">deepMerge</span>로 합쳐집니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[14px] text-gray-600 leading-relaxed mb-4">
              ...
              구조 덕분에 기본값 위에 사용자 설정을 얹고, 프로젝트별로 덮어쓰고, 환경변수나 CLI
              플래그로 일회성 오버라이드를 할 수 있습니다.
            </p>

            <Callout type="tip" icon="💡">
              <strong>핵심 원칙:</strong> 높은 레벨이 낮은 레벨을 덮어씁니다. CLI 플래그가 가장
              높고, <code className="text-cyan-600 text-xs">defaults.ts</code>가 가장 낮습니다.
            </Callout>
  ...
  style FINAL fill:#f8fafc,stroke:#06b6d4,color:#06b6d4,stroke-width:2px`}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mt-4">
              ...
              <div className="flex flex-col gap-2.5 text-[13px] text-gray-600">
                <div className="flex gap-3">
                  ...
                  <span>
                    아무 설정 없이도 앱이 동작하도록 보장하는{" "}
                    <strong className="text-gray-900">안전망</strong>
                  </span>
                </div>
                <div className="flex gap-3">
                ...
                <div className="flex gap-3">
                  ...
                  <span>
                    CLI 플래그 &mdash; <code className="text-cyan-600 text-xs">--model gpt-4o</code>
                    처럼 일회성 덮어쓰기
                  </span>
                </div>
              </div>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="mb-8">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                5단계 계층 설정 로더의 메인 함수입니다. 모든 설정 소스를 병합하고 Zod 스키마로
                검증하여
              ...
              <div className="mt-3">
                <h4 className="text-[13px] font-bold text-gray-900 mb-2">
                  반환 타입:{" "}
                  <code className="text-violet-600 text-xs">Promise&lt;ResolvedConfig&gt;</code>
                </h4>
                <ParamTable
                  params={[
                    {
              ...
              <Callout type="warn" icon="⚠️">
                Zod 검증에 실패하면 <code className="text-red-600 text-xs">ConfigError</code>를
                던집니다. 에러 객체의 <code className="text-cyan-600 text-xs">errors</code> 필드에
                어떤 필드가 잘못됐는지 상세 정보가 담겨 있습니다.
              </Callout>
            </div>
            ...
            <div className="mb-8">
              ...
              <Callout type="info" icon="🔑">
                <code className="text-cyan-600 text-xs">OPENAI_API_KEY</code>만 설정되고 Base URL이
                없으면, 자동으로{" "}
            ...
            <div className="mb-4">
              ...
              <p className="text-[13px] text-gray-600 mb-3">
                두 객체를 재귀적으로 깊은 병합하는 유틸리티. 양쪽 모두 객체(배열 제외)이면 재귀
                병합하고, 그 외에는 source 값이 target 값을 덮어씁니다. 원본은 변경하지 않습니다
                (불변성 보장).
              </p>
              <ParamTable
                params={[
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              앱이 시작될 때 <code className="text-cyan-600 text-xs">loadConfig()</code>를
              호출합니다. CLI에서 받은 플래그를{" "}
              <code className="text-cyan-600 text-xs">cliOverrides</code>로 전달하면 가장 높은
              우선순위로 적용됩니다.
            </p>
            ...
            <CodeBlock>
              ...
              <span className="text-[#8b949e]">
                {'// config.llm.model === "gpt-4o"  (CLI 플래그가 최우선)'}
              </span>
              {"\n"}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">sources</code> Map을 사용하면 특정 설정이
              어디서 왔는지 확인할 수 있어, &ldquo;왜 이 값이 적용됐지?&rdquo; 문제를 쉽게 디버깅할
              수 있습니다.
            </p>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              설정 병합의 핵심은 <code className="text-cyan-600 text-xs">deepMerge</code>{" "}
              함수입니다. 단순 <code className="text-cyan-600 text-xs">Object.assign</code>과 달리,
  ...
  style RETURN fill:#f8fafc,stroke:#10b981,color:#10b981,stroke-width:2px`}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              ...
              <code className="text-cyan-600 text-xs"> timeout</code>은 원본 값이 유지되는 것을
              확인할 수 있습니다.
            </p>
            ...
            <Callout type="info" icon="📝">
              <strong>배열은 재귀 병합하지 않습니다.</strong> 배열은 source 값으로{" "}
              <em>통째로 교체</em>됩니다. 예를 들어{" "}
              <code className="text-cyan-600 text-xs">permissions.allow</code>를 프로젝트 설정에서
              지정하면 사용자 설정의 allow 리스트는 무시됩니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              <code className="text-cyan-600 text-xs">loadJsonFile()</code>은 파일이 없거나 파싱에
              실패하면
              <code className="text-cyan-600 text-xs"> undefined</code>를 반환합니다. 에러를 던지지
              않는 &ldquo;조용한 실패&rdquo; 전략으로, 설정 파일이 선택적(optional)이라는 설계
              의도를 반영합니다.
            </p>

            <CodeBlock>
              <span className="text-[#8b949e]">
                {"// 파일이 없으면 undefined → 병합 단계를 건너뜀"}
              </span>
              {"\n"}
              <span className="text-[#ff7b72]">const</span>{" "}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                ...
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    <strong className="text-gray-900">원인 1:</strong> 더 높은 우선순위 소스가
                    덮어쓰고 있습니다.
                    <code className="text-cyan-600 text-xs"> sources.get(&quot;llm&quot;)</code>으로
                    실제 출처를 확인하세요.
                  </p>
                  <p className="mb-2">
                    <strong className="text-gray-900">원인 2:</strong> JSON 문법 오류.{" "}
                    <code className="text-cyan-600 text-xs">loadJsonFile</code>은 파싱 실패 시
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> 환경변수를 설정했는데 로컬 LLM에 연결이
                  안 돼요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                    ...
                    <code className="text-cyan-600 text-xs"> OPENAI_API_KEY</code>만 설정하면
                    자동으로 OpenAI API로 연결됩니다.
                  </p>
                  <p>
                    <strong className="text-gray-900">확인 2:</strong> URL에{" "}
                    <code className="text-cyan-600 text-xs">/chat/completions</code> 경로가 포함되어
                    있지 않은지 확인하세요. LLM 클라이언트가 자동으로 추가합니다. 올바른 예:{" "}
                    <code className="text-cyan-600 text-xs">http://localhost:11434/v1</code>
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> ConfigError: Invalid configuration 에러가
                  발생해요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p className="mb-2">
                  ...
                  <p>
                    흔한 원인: <code className="text-cyan-600 text-xs">temperature</code>가 0~2 범위
                    밖, <code className="text-cyan-600 text-xs">baseUrl</code>이 유효한 URL이 아님,{" "}
                    <code className="text-cyan-600 text-xs">permissionMode</code>에 잘못된 문자열을
                    넣었을 때 등.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-5">
                <h4 className="text-[14px] font-bold mb-2 flex items-center gap-2">
                  <span className="text-red-600">Q.</span> 프로젝트 설정의 permissions.allow가
                  사용자 설정과 합쳐지지 않아요
                </h4>
                <div className="text-[13px] text-gray-600 leading-relaxed">
                  <p>
                    <strong className="text-gray-900">이것은 의도된 동작입니다.</strong>{" "}
                    <code className="text-cyan-600 text-xs">deepMerge</code>는 배열을 재귀 병합하지
                    않고 <em>통째로 교체</em>합니다. 프로젝트 설정에서{" "}
                    <code className="text-cyan-600 text-xs">permissions.allow</code>를 지정하면
                    사용자 설정의 리스트가 완전히 대체됩니다. 두 리스트를 합치려면 프로젝트 설정에
                    사용자 설정의 항목도 포함시키세요.
                  </p>
                </div>
              </div>
