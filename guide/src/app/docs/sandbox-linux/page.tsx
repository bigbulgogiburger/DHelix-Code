...
export default function SandboxLinuxPage() {
        ...
        <RevealOnScroll>
          <div style={{ marginBottom: "48px" }}>
            ...
            <h1 className="text-3xl font-bold tracking-tight leading-[1.15] mt-4 mb-4">
              <span className="text-gray-900">Linux Bubblewrap 샌드박스</span>
            </h1>
            <div className="flex items-center gap-3 mb-5">
              <LayerBadge layer="leaf" />
            </div>
            <p className="text-[16px] text-gray-600 leading-relaxed">
              Linux 환경을 감지하고, bubblewrap(bwrap) 샌드박스의 설치 상태를 확인하며, bwrap 인수를
              생성하고 샌드박스 안에서 명령을 실행하는 모듈입니다.
            </p>
          </div>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="text-[14px] text-gray-600 leading-[1.85] mb-8 space-y-3">
              <p>
                <code className="text-cyan-600">linux.ts</code>는 Linux 환경에서 프로세스를 격리
                실행하기 위한 bubblewrap(bwrap) 샌드박스의 전반적인 환경 관리를 담당합니다. 네이티브
                Linux, WSL2, WSL1 환경을 자동으로 감지하고, bubblewrap 설치 여부와 버전을 확인하며,
                배포판별 설치 안내를 제공합니다.
              </p>
              <p>
                WSL2는 완전한 Linux 커널 네임스페이스를 지원하므로 bubblewrap을 사용할 수 있지만,
                WSL1은 네임스페이스 기능이 없어 bubblewrap을 사용할 수 없습니다. 이 모듈은 이러한
                환경 차이를 투명하게 처리합니다.
              </p>
              <p>
  ...
  style NP fill:#e0e7ff,stroke:#64748b,color:#1e293b`}
            ...
            <Callout type="info" icon="💡">
              <strong>비유:</strong> 건물에 들어가기 전 보안 게이트를 떠올리세요. linux.ts는
              &quot;이 건물에 보안 시스템이 설치되어 있는가?&quot;를 확인하고, 설치되어 있다면
              적절한 보안 규칙(마운트/격리)을 설정한 뒤 입장시키는 역할입니다.
            </Callout>
          </section>
        </RevealOnScroll>
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <ParamTable
              params={[
                ...
                { name: "cwd", type: "string", required: false, desc: "작업 디렉토리" },
                {
                  ...
                  type: "string",
                  required: true,
                  desc: "프로젝트 디렉토리 (읽기/쓰기 접근 허용)",
                },
                {
                  name: "homeDir",
                  type: "string",
                  required: false,
                  desc: "홈 디렉토리 (일부 읽기 전용 접근 허용)",
                },
                {
                  name: "timeoutMs",
                  type: "number",
            ...
            <ParamTable
              params={[
                ...
                {
                  name: "environment",
                  type: '"native-linux" | "wsl2" | "wsl1" | "unknown"',
                  required: true,
                  desc: "감지된 환경 타입",
                },
                {
                  name: "bubblewrapInstalled",
                  type: "boolean",
                  required: true,
                  desc: "bubblewrap 설치 여부",
                },
                {
                  name: "bubblewrapVersion",
                  type: "string | undefined",
                  required: false,
                  desc: "bubblewrap 버전 문자열",
                },
                {
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              WSL2 환경에서 실행 중인지 감지합니다.{" "}
              <code className="text-cyan-600">/proc/version</code>에 &quot;microsoft&quot; 문자열이
              있고, WSLInterop 파일이 존재하면 WSL2로 판단합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">isWSL2</span>():{" "}
            ...
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">isWSL1</span>():{" "}
              <span className="type">Promise</span>&lt;<span className="type">boolean</span>&gt;
            </CodeBlock>
            ...
            <CodeBlock>
              <span className="kw">async function</span> <span className="fn">hasBubblewrap</span>
              (): <span className="type">Promise</span>&lt;<span className="type">boolean</span>&gt;
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              bubblewrap 명령의 인수 배열을 생성합니다. 시스템 경로 읽기 전용 마운트, PID 격리, 세션
              격리, 선택적 네트워크 격리 등의 플래그를 포함합니다.
            </p>
            <CodeBlock>
              <span className="kw">function</span> <span className="fn">generateBwrapArgs</span>(
              <span className="prop">config</span>: <span className="type">LinuxSandboxConfig</span>
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              Linux bubblewrap 샌드박스 안에서 명령을 실행합니다. bwrap 미설치, WSL1, 타임아웃, 실행
              실패 시 <code className="text-cyan-600">BubblewrapError</code>를 던집니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span>{" "}
              <span className="fn">executeBubblewrap</span>(<span className="prop">config</span>:{" "}
              <span className="type">LinuxSandboxConfig</span>):{" "}
            ...
            <p className="text-[13px] text-gray-600 mb-3">
              Linux 샌드박스의 전체 준비 상태를 확인합니다. 환경 감지, bwrap 설치 확인, 배포판별
              설치 안내 제공 등을 종합적으로 수행합니다.
            </p>
            <CodeBlock>
              <span className="kw">async function</span>{" "}
              <span className="fn">checkLinuxSandboxReady</span>():{" "}
              <span className="type">Promise</span>&lt;
              <span className="type">SandboxReadiness</span>&gt;
            </CodeBlock>
            ...
            <ul className="text-[13px] text-gray-600 space-y-2 list-disc list-inside">
              <li>
                ...
                <code className="text-cyan-600">checkLinuxSandboxReady()</code>가 WSL2 업그레이드를
                권장합니다.
              </li>
              <li>
              ...
              <li>
                <code className="text-cyan-600">allowNetwork</code>가{" "}
                <code className="text-cyan-600">false</code>일 때만
                <code className="text-cyan-600">--unshare-net</code> 플래그가 추가됩니다. 기본값은{" "}
                <code className="text-cyan-600">true</code>이므로 네트워크가 열려 있습니다.
              </li>
              <li>
                최대 출력 버퍼는 10MB로 고정되어 있습니다. 대량의 stdout을 생성하는 명령은 버퍼 초과
                에러가 발생할 수 있습니다.
              </li>
            </ul>
          </section>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <CodeBlock>
              ...
              {"\n"}
              <span className="kw">const</span> <span className="prop">readiness</span> ={" "}
              <span className="kw">await</span> <span className="fn">checkLinuxSandboxReady</span>
              ();
              {"\n"}
              {"\n"}
              <span className="kw">if</span> (!<span className="prop">readiness</span>.
              <span className="prop">available</span>) {"{"}
              {"\n"}
              {"  "}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="str">`샌드박스 사용 불가: ${"{"}</span>
              <span className="prop">readiness</span>.<span className="prop">reason</span>
              <span className="str">{"}"}`</span>);
              {"\n"}
              {"  "}
              <span className="prop">readiness</span>.<span className="prop">recommendations</span>.
              <span className="fn">forEach</span>(<span className="prop">r</span> =&gt;{" "}
              <span className="fn">console</span>.<span className="fn">log</span>(
              ...
              {"\n"}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="str">`환경: ${"{"}</span>
              <span className="prop">readiness</span>.<span className="prop">environment</span>
              <span className="str">{"}"}`</span>);
              {"\n"}
              <span className="fn">console</span>.<span className="fn">log</span>(
              <span className="str">`bwrap 버전: ${"{"}</span>
              <span className="prop">readiness</span>.
              <span className="prop">bubblewrapVersion</span>
              <span className="str">{"}"}`</span>);
            </CodeBlock>
            ...
            <CodeBlock>
              ...
              <span className="kw">await</span> <span className="fn">executeBubblewrap</span>({"{"}
              {"\n"}
              {"  "}
            ...
            <Callout type="warn" icon="!">
              <strong>주의:</strong> <code>executeBubblewrap()</code>은 내부에서 bwrap 설치 여부와
              WSL1 환경을 자동으로 확인합니다. 하지만 사전에 <code>checkLinuxSandboxReady()</code>를
              호출하여 사용자에게 설치 안내를 제공하는 것이 좋습니다.
            </Callout>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">allowNetwork: false</code>를 설정하면 샌드박스
              내부에서 외부 네트워크에 접근할 수 없습니다. 신뢰할 수 없는 스크립트를 실행할 때
              유용합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// 네트워크 완전 차단된 샌드박스"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">isolated</span> ={" "}
              <span className="kw">await</span> <span className="fn">executeBubblewrap</span>({"{"}
              {"\n"}
              {"  "}
              ...
              {"  "}
              <span className="prop">args</span>: [
              <span className="str">&quot;untrusted_script.py&quot;</span>],
              {"\n"}
              {"  "}
              <span className="prop">projectDir</span>:{" "}
              <span className="str">&quot;/home/user/sandbox-test&quot;</span>,{"\n"}
              {"  "}
            ...
            <DeepDive title="마운트 구조 상세">
              <p className="mb-3">
                <code className="text-cyan-600">generateBwrapArgs()</code>가 생성하는 마운트
                구조입니다:
              </p>
              <ul className="list-disc list-inside space-y-1.5 text-gray-600">
                <li>
                  <strong>읽기 전용 시스템 경로:</strong> /usr, /bin, /lib, /lib64, /etc, /sbin
                </li>
                <li>
                  <strong>특수 파일 시스템:</strong> /proc (procfs), /dev (devfs)
                </li>
                <li>
                  <strong>휘발성 임시:</strong> /tmp (tmpfs)
                </li>
                <li>
                  <strong>읽기/쓰기:</strong> 프로젝트 디렉토리 (--bind)
                </li>
                <li>
        ...
        <RevealOnScroll>
          <section style={{ marginBottom: "64px" }}>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              <code className="text-cyan-600">checkLinuxSandboxReady()</code>는 환경을 단계적으로
              감지합니다. 먼저 Linux 플랫폼인지 확인하고, WSL 버전을 판별한 뒤, bubblewrap 설치를
              확인합니다.
            </p>
  ...
  style DISTRO fill:#fef3c7,stroke:#f59e0b,color:#92400e`}
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              ...
              <code className="text-cyan-600">/proc/sys/fs/binfmt_misc/WSLInterop</code> 파일을
              기반으로 합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// WSL2 감지 핵심 로직"}</span>
              {"\n"}
              <span className="kw">const</span> <span className="prop">version</span> ={" "}
              <span className="kw">await</span> <span className="fn">readFile</span>(
              <span className="str">&quot;/proc/version&quot;</span>,{" "}
              <span className="str">&quot;utf-8&quot;</span>);
              {"\n"}
              <span className="kw">const</span> <span className="prop">hasMicrosoftKernel</span> ={" "}
              <span className="str">/microsoft/i</span>.<span className="fn">test</span>(
              ...
              {"\n"}
              {"  "}
              <span className="kw">await</span> <span className="fn">access</span>(
              <span className="str">&quot;/proc/sys/fs/binfmt_misc/WSLInterop&quot;</span>);
              {"\n"}
              {"}"}
            </CodeBlock>
            ...
            <p className="text-[13px] text-gray-600 mb-4 leading-relaxed">
              bubblewrap가 미설치일 때, <code className="text-cyan-600">/etc/os-release</code>를
              파싱하여 배포판 계열(Debian, Fedora, Arch)에 맞는 설치 명령을 제공합니다.
            </p>
            <CodeBlock>
              <span className="cm">{"// 배포판별 설치 명령"}</span>
              {"\n"}
              <span className="str">&quot;debian&quot;</span> →{" "}
              <span className="str">&quot;sudo apt install bubblewrap&quot;</span>
              {"\n"}
              <span className="str">&quot;fedora&quot;</span> →{" "}
              <span className="str">&quot;sudo dnf install bubblewrap&quot;</span>
              {"\n"}
              <span className="str">&quot;arch&quot;</span> →{" "}
              <span className="str">&quot;sudo pacman -S bubblewrap&quot;</span>
              {"\n"}
          ...
          <section style={{ marginBottom: "64px" }}>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
                ...
                <code className="text-cyan-600">checkLinuxSandboxReady()</code>를 호출하면 배포판에
                맞는 설치 명령을 확인할 수 있습니다.
              </p>
              <CodeBlock>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5 mb-4">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                ...
                <code className="text-cyan-600">timeoutMs</code> 값을 늘려서 설정하거나, 실행하는
                명령 자체가 무한 루프에 빠지지 않았는지 확인하세요.
              </p>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-5">
              ...
              <p className="text-[13px] text-gray-600 leading-relaxed">
                <code className="text-cyan-600">process.platform</code>이 &quot;linux&quot;가 아닌
                경우입니다. macOS에서는 <code className="text-cyan-600">seatbelt.ts</code>를
                사용해야 합니다. Linux 샌드박스는 Linux 환경에서만 동작합니다.
              </p>
            </div>
          </section>
        </RevealOnScroll>
