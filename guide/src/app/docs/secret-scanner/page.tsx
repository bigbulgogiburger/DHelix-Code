  ...
  return (
        ...
        <RevealOnScroll>
          <section className="mb-16">
            ...
            <Callout type="danger" icon="🚨">
              <strong>실제 위협 시나리오</strong><br />
              1. 사용자가 <code>.env</code> 파일 분석을 요청 → LLM이 API 키를 응답에 포함<br />
              2. <code>git diff</code> 출력에 실수로 커밋된 비밀번호가 노출<br />
              3. DB 마이그레이션 스크립트에 하드코딩된 연결 문자열이 컨텍스트에 유입<br />
    ...
    style D fill:#1e293b,stroke:#64748b,color:#f1f5f9`}
            ...
            <DeepDive title="왜 서버 측 필터링이 아니라 클라이언트 측에서?">
              <p className="mb-2">
                dbcode는 <strong>로컬 우선(local-first)</strong> 철학을 따릅니다.
                비밀 정보가 네트워크를 타기 <em>전에</em> 클라이언트에서 제거해야 합니다.
                서버 측 필터링에 의존하면 전송 과정에서 이미 노출된 것이므로 의미가 없습니다.
              </p>
              <p>
                또한 자체 호스팅 LLM(<code>LOCAL_API_BASE_URL</code>)을 사용하는 경우에도
                동일한 보호가 적용되어야 하므로, 클라이언트 측 가드레일이 필수적입니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section className="mb-16">
            ...
            <ParamTable
              params={[
                { name: "found", type: "boolean", required: true, desc: "하나 이상의 비밀 정보가 발견되었는지 여부" },
                { name: "redacted", type: "string", required: true, desc: "[REDACTED]로 대체된 텍스트. 비밀 정보가 없으면 원본 그대로 반환" },
                { name: "patterns", type: "readonly string[]", required: true, desc: "탐지된 패턴의 이름 목록 (예: [\"AWS Access Key\", \"JWT Token\"])" },
              ]}
            />
            ...
            <ParamTable
              params={[
                { name: "text", type: "string", required: true, desc: "비밀 정보를 검사할 텍스트 (도구 실행 출력, 파일 내용 등)" },
              ]}
            />
            ...
            <Callout type="warn" icon="⚠️">
              <strong>주의사항:</strong> 정규식 기반 탐지이므로 100% 완벽하지 않습니다.
              새로운 서비스의 키 형식이 추가되면 패턴 목록을 업데이트해야 합니다.
              또한, Base64로 이중 인코딩된 시크릿은 탐지하지 못할 수 있습니다.
            </Callout>
          </section>
        </RevealOnScroll>
        ...
        <RevealOnScroll>
          <section className="mb-16">
            ...
            <Callout type="warn" icon="⚠️">
              <strong>오탐(False Positive) 가능성</strong><br />
              <code>sk-</code>로 시작하는 20자 이상의 문자열은 모두 OpenAI API 키로 인식됩니다.
              예를 들어 테스트 코드에서 <code>{`sk-abcdefghij1234567890`}</code> 같은 더미 값을 사용하면
          ...
          <section className="mb-16">
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden my-4">
              <table className="w-full border-collapse text-[12px]">
                ...
                <tbody>
                  {[
                    { cat: "☁️ 클라우드", name: "AWS Access Key", rule: "AKIA 또는 ASIA + 영대문자/숫자 16자" },
                    { cat: "☁️ 클라우드", name: "Google Cloud SA", rule: "\"type\": \"service_account\" JSON 패턴" },
                    { cat: "☁️ 클라우드", name: "Azure Connection", rule: "DefaultEndpointsProtocol / AccountKey / SharedAccessSignature" },
                    { cat: "🤖 AI/ML", name: "OpenAI API Key", rule: "sk- + 영숫자 20자 이상" },
                    { cat: "🤖 AI/ML", name: "Anthropic API Key", rule: "sk-ant- + 영숫자/하이픈/밑줄 20자 이상" },
                    { cat: "🐙 Git", name: "GitHub Token", rule: "ghp_ + 영숫자 36자" },
                    { cat: "🐙 Git", name: "GitHub OAuth", rule: "gho_ + 영숫자 36자" },
                    { cat: "🐙 Git", name: "GitHub App Token", rule: "ghu_ / ghs_ / ghr_ + 영숫자 36자" },
                    { cat: "💬 채팅", name: "Slack Token", rule: "xoxb- / xoxp- / xoxs- / xoxa- + 10자 이상" },
                    { cat: "💳 결제", name: "Stripe Secret Key", rule: "sk_live_ 또는 sk_test_ + 영숫자 20자 이상" },
                    { cat: "💳 결제", name: "Stripe Publishable", rule: "pk_live_ 또는 pk_test_ + 영숫자 20자 이상" },
                    { cat: "🔑 개인키", name: "RSA Private Key", rule: "-----BEGIN RSA PRIVATE KEY-----" },
                    { cat: "🔑 개인키", name: "EC Private Key", rule: "-----BEGIN EC PRIVATE KEY-----" },
                    { cat: "🔑 개인키", name: "OpenSSH Private Key", rule: "-----BEGIN OPENSSH PRIVATE KEY-----" },
                    { cat: "🔑 개인키", name: "Generic Private Key", rule: "-----BEGIN PRIVATE KEY-----" },
                    { cat: "🎫 토큰", name: "JWT Token", rule: "eyJ...eyJ...signature (Base64URL 3-part)" },
                    ...
                    { cat: "🌐 SaaS", name: "Twilio Auth Token", rule: "twilio_auth_token = 16진수 32자" },
                    { cat: "🗄️ DB", name: "PostgreSQL Connection", rule: "postgresql://..." },
                    { cat: "🗄️ DB", name: "MongoDB Connection", rule: "mongodb:// 또는 mongodb+srv://..." },
            ...
            <CodeBlock>
              ...
              <span className="text-[#e6edf3]">) {"{"}</span>{"\n"}
              <span className="text-[#8b949e]">{"    // 🔑 핵심: RegExp 객체를 새로 생성하여 lastIndex 초기화"}</span>{"\n"}
              <span className="text-[#e6edf3]">{"    "}</span>
              <span className="text-[#ff7b72]">const</span>{" "}
              ...
              <span className="text-[#8b949e]">{"      // test()가 lastIndex를 변경하므로 새 RegExp로 대체"}</span>{"\n"}
              <span className="text-[#e6edf3]">{"      "}</span>
              <span className="text-[#ff7b72]">const</span>{" "}
            ...
            <DeepDive title="왜 RegExp를 매번 새로 생성하나요?">
              <p className="mb-3">
                ...
                <strong>상태를 가집니다</strong>.
                <code>regex.test()</code>를 호출하면 내부적으로 <code>lastIndex</code> 프로퍼티가
                마지막 매칭 위치로 업데이트됩니다.
              </p>
              <p className="mb-3">
              ...
              <p>
                ...
                <code>lastIndex</code>를 <code>0</code>으로 초기화합니다.
                이는 JavaScript 정규식 작업의 흔한 함정(gotcha)입니다.
              </p>
            </DeepDive>
            ...
            <DeepDive title="SECRET_PATTERNS 배열의 순서가 중요한 이유">
              ...
              <ul className="list-disc list-inside space-y-1 mb-3">
                <li><code>sk-ant-</code> (Anthropic) 패턴이 <code>sk-</code> (OpenAI) 패턴보다 먼저 옵니다</li>
                <li><code>sk_live_</code> (Stripe) 패턴이 일반 <code>API_KEY</code> 패턴보다 먼저 옵니다</li>
              </ul>
              <p>
                만약 일반 패턴이 먼저 매칭되면, 해당 부분이 <code>[REDACTED]</code>로 대체되어
                구체적 패턴이 매칭할 기회를 잃습니다. <code>patterns</code> 배열에 정확한
                패턴 이름이 기록되어야 디버깅이 가능하므로, 순서가 중요합니다.
              </p>
            </DeepDive>
          </section>
        </RevealOnScroll>
