# dbcode vs Claude Code 격차 분석 및 해소 방안
...
| **Tier-based compaction**                           | ❌ 불필요 (고성능 모델만) | ✅ v1에서 추가               | ⬜ dbcode 우위 |
...
| Dual tool-call strategy (native + text-parsing) | ❌ native only             | ✅                 | ⬜ dbcode 우위 |
| **JSON 복구 (malformed output)**                | ❌ 불필요                  | ✅ v1에서 추가     | ⬜ dbcode 우위 |
| **Model capability tier system**                | ❌ 불필요                  | ✅ v1에서 추가     | ⬜ dbcode 우위 |
...
| **Locale section**                               | ❌ (영어 전용) | ✅ v1에서 추가 | ⬜ dbcode 우위 |
| **Tone profiles**                                | ❌             | ✅ v1에서 추가 | ⬜ dbcode 우위 |
| **CoT scaffolding (low-tier)**                   | ❌             | ✅ v1에서 추가 | ⬜ dbcode 우위 |
...
| **AgentStatus (fun messages)**             | ✅ 영어           | ✅ 한국어 v1           | ⬜ dbcode 우위 |
...
v1 구현 완료 후 dbcode는 **핵심 코어 기능의 90% 이상**을 Claude Code와 동등하게 갖추고 있습니다.
...
Sprint 1 (Prompt Caching)만 완성해도 dbcode의 실사용 비용이 Claude Code 수준으로 떨어지며, Sprint 2까지 완성하면 UX 격차가 사실상 해소됩니다.
