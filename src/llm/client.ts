...
function normalizeAzureUrl(baseURL: string): {
  ...
  if (!baseURL.includes(".openai.azure.com") && !baseURL.includes(".cognitiveservices.azure.com")) {
  ...
  // Strip /chat/completions, /responses, or other API-specific paths from the URL
  // so the OpenAI SDK can append its own /chat/completions path
