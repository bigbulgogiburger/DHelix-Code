...
/** 기본 설정 디렉토리: ~/.dbcode/ — 사용자 전역 설정이 저장되는 곳 */
export const CONFIG_DIR = join(homedir(), `.${APP_NAME}`);

/** 프로젝트 설정 파일명: DBCODE.md — 프로젝트 루트에 위치하는 인스트럭션 파일 */
export const PROJECT_CONFIG_FILE = `${APP_NAME.toUpperCase()}.md`;

/** 프로젝트 설정 디렉토리명: .dbcode — 프로젝트별 설정, 규칙, 스킬이 저장되는 곳 */
export const PROJECT_CONFIG_DIR = `.${APP_NAME}`;

/**
 * DBCODE.md 탐색 경로를 순서대로 반환
 *
 * 우선순위:
 * 1. {cwd}/DBCODE.md — 프로젝트 루트에 직접 (권장 방식)
 * 2. {cwd}/.dbcode/DBCODE.md — 하위 호환 폴백
 *
 * 모든 코드에서 DBCODE.md를 찾을 때 이 함수를 사용하여 일관성을 보장합니다.
 *
 * @param cwd - 탐색 시작 디렉토리
 * @returns DBCODE.md를 찾아볼 경로 배열 (우선순위 순)
 */
export function getProjectConfigPaths(cwd: string): readonly string[] {
  return [
    join(cwd, PROJECT_CONFIG_FILE), // DBCODE.md (프로젝트 루트 — 기본)
    join(cwd, PROJECT_CONFIG_DIR, PROJECT_CONFIG_FILE), // .dbcode/DBCODE.md (폴백)
  ];
}

/** 세션 저장 디렉토리: ~/.dbcode/sessions/ */
export const SESSIONS_DIR = join(CONFIG_DIR, "sessions");

/** 디버그 로그 파일 경로: ~/.dbcode/debug.log */
export const LOG_FILE = join(CONFIG_DIR, "debug.log");

/** 입력 히스토리 파일 경로: ~/.dbcode/input-history.json — 사용자 입력 기록 */
 ...
 * @returns {프로젝트}/.dbcode/memory/ 의 절대 경로
 */
