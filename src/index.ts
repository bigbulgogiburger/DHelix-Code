  ...
  .action(
    async (opts: {
      ...
      const [
        ...
        { HookRunner },
        { CommandRegistry },
        { builtinTools, createAgentTool },
        { builtinCommands, setHelpCommands },
      ] = await Promise.all([
      ...
      // ── 단계 5: LLM 클라이언트 생성 ──
      // ProviderRegistry 기반 통합 해상도 — 모델 이름으로 적절한 프로바이더 자동 선택
      // Claude → AnthropicProvider, Codex → ResponsesAPIClient, 기타 → OpenAICompatibleClient
