...
describe.skipIf(!hasApiKey)(`E2E: ${STACK_NAME} Multi-Turn Session`, () => {
  ...
  beforeAll(async () => {
    ...
    events.on("tool:start", ({ name, id, args }: { name: string; id: string; args?: Record<string, unknown> }) => {
  ...
  // ---- sendTurn Helper ----
  async function sendTurn(userMessage: string, turnName?: string): Promise<{
    iterations: number;
    lastContent: string;
  }> {
    currentTurn++;
...
}, { timeout: 1_800_000 }); // 30 minute total timeout
```
