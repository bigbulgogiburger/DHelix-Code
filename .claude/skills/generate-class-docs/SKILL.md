...
문서 페이지 전용으로 추가할 컴포넌트:

1. **DeepDive** — 접을 수 있는 고급 내용 영역
```tsx
function DeepDive({ title, children }: { title: string; children: React.ReactNode }) {
  // details/summary HTML 기반 구현
}
```

2. **ParamTable** — 파라미터 테이블
```tsx
...
3. **DocNavigation** — 문서 전용 사이드바 네비게이션
```tsx
// 좌측: 문서 목록 (레이어별 그룹)
// 우측: 현재 페이지의 "On this page" TOC
```
...
export default function ModuleNamePage() {
  return (
    <article className="max-w-[900px] mx-auto px-4 sm:px-8 py-20">
      {/* 1. 개요 */}
      <RevealOnScroll>
        <header className="mb-12">
          <FilePath path="src/core/module-name.ts" />
          <h1 className="text-3xl font-black mt-4 mb-3">ModuleName</h1>
          <p className="text-text-secondary text-lg">한 줄 설명</p>
          {/* 아키텍처 위치 다이어그램 */}
          <MermaidDiagram chart={archChart} title="아키텍처 위치" titleColor="purple" />
        </header>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section id="reference" className="mb-16">
          <h2 className="text-2xl font-bold mb-6">레퍼런스</h2>
          {/* 시그니처 + 파라미터 + 반환값 + Caveats */}
        </section>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section id="usage" className="mb-16">
          <h2 className="text-2xl font-bold mb-6">사용법</h2>
          {/* 시나리오별 코드 예시 */}
        </section>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section id="internals" className="mb-16">
          <h2 className="text-2xl font-bold mb-6">내부 구현</h2>
          {/* Mermaid 다이어그램 + 핵심 코드 발췌 */}
        </section>
      </RevealOnScroll>
      ...
      <RevealOnScroll>
        <section id="troubleshooting" className="mb-16">
          <h2 className="text-2xl font-bold mb-6">트러블슈팅</h2>
          {/* FAQ 형태 */}
        </section>
      </RevealOnScroll>
...
dbcode의 4-Layer 아키텍처에서 가장 핵심적인 모듈부터 문서화한다:

### Layer 2: Core (최우선)
1. `agent-loop.ts` — ReAct 패턴 메인 루프
2. `context-manager.ts` — 3-Layer 토큰 관리
3. `circuit-breaker.ts` — 무한 루프 방지
4. `recovery-executor.ts` — 에러 복구 전략
5. `system-prompt-builder.ts` — 시스템 프롬프트 조립
6. `checkpoint-manager.ts` — 상태 스냅샷
7. `observation-masking.ts` — 도구 출력 마스킹

### Layer 3: Infrastructure
8. `llm/client.ts` — LLM API 클라이언트
9. `llm/dual-model-router.ts` — 모델 전환
10. `tools/registry.ts` — 도구 등록
11. `tools/executor.ts` — 도구 실행
12. `permissions/manager.ts` — 권한 관리
13. `mcp/manager.ts` — MCP 서버 관리

### Layer 4: Leaf
14. `config/loader.ts` — 설정 로딩
15. `skills/manager.ts` — 스킬 관리
16. `instructions/loader.ts` — DBCODE.md 로딩

### Layer 1: CLI
17. `cli/hooks/useAgentLoop.ts` — Agent Loop 연결 훅
18. `cli/components/ActivityFeed.tsx` — Progressive Static Flushing
