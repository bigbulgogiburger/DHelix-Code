...
> dhelix `/init` vs Claude Code `/init` 비교 분석 및 개선 제안
> 작성일: 2026-03-18
...
### 1.1 dhelix `/init`의 현재 흐름

```
/init 입력
  → ensureConfigDir() — .dhelix/ 생성
  → ensureGitignoreEntry() — DHELIX.local.md를 .gitignore에 추가
  → buildAnalysisPrompt() — 프롬프트 구성
  → shouldInjectAsUserMessage: true — LLM에 주입
  → LLM이 도구로 분석 후 DHELIX.md 작성
...
/init
  → Phase 1: 사용자에게 생성할 항목 질문
      "다음 중 생성할 항목을 선택하세요:
       [x] DHELIX.md (프로젝트 가이드)
       [ ] .dhelix/rules/ (코딩 규칙 파일)
       [ ] .dhelix/skills/ (커스텀 스킬)
       [ ] .dhelix/hooks/ (자동 훅)"
...
11. [NEW] 기존 .dhelix/rules/ 파일 확인 — 중복 방지
...
11. [NEW] Check existing .dhelix/rules/ to avoid duplication
...
- **Keep it under 200 lines** — split to .dhelix/rules/ if needed
...
**Step 2**: Based on the subagent's findings, draft DHELIX.md content.

**Step 3**: Present the draft to the user for review before writing.

**Step 4**: After user approval, write DHELIX.md and split large sections
into .dhelix/rules/ files if over 200 lines.
...
This file provides guidance to dhelix...
```
...
## Architecture
상세 아키텍처: @.dhelix/docs/architecture.md

## Rules
@.dhelix/rules/code-style.md
@.dhelix/rules/testing.md
@.dhelix/rules/security.md
````
...
+   in .dhelix/rules/ (e.g., code-style.md, testing.md, security.md).
...
Claude Code의 `/init`이 우수한 이유는 **"무엇을 포함하지 말 것인가"**에 대한 가이드가 명확하기 때문입니다. dhelix의 프롬프트도 이 철학을 더 강하게 반영해야 합니다.

---

## 7. 결론

dhelix의 `/init`은 **기본 골격은 갖추고 있지만**, Claude Code 대비 3가지 핵심 차이가 있습니다:
