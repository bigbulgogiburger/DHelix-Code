...
dhelix는 복잡한 작업을 하위 에이전트(subagent)에게 위임할 수 있습니다. 단일 에이전트부터 다수 에이전트의 팀 조율까지 지원합니다.
...
graph TD
    ...
    SPAWNER --> CUSTOM[Custom Agent<br/>.dhelix/agents/*.md]
...
`.dhelix/agents/` 또는 `~/.dhelix/agents/`에 마크다운 파일로 정의:
...
- 에이전트가 학습한 패턴/선호도 저장
- 세션 간 유지
- 경로: `~/.dhelix/projects/{hash}/agent-memory/`
...
| `definition-loader.ts`   | .dhelix/agents/\*.md 로딩                   |
