...
> 참조 시점: dhelix CLI를 headless 모드로 E2E / QA 테스트할 때
...
```
QA 에이전트 → dhelix CLI 실행 → dhelix가 파일 생성/수정/실행
QA 에이전트는 결과 채점만 한다
```

- ✅ QA가 직접 만들어도 되는 것: **fixture 파일** (버그 있는 파일, 타입에러 파일 등 — dhelix가 수정할 대상), **NEXUS.md** 같은 컨텍스트 파일
- ❌ QA가 직접 만들면 안 되는 것: dhelix가 생성해야 하는 코드/파일
...
- T1에서 dhelix가 파일 생성/수정 → T2에서 "이 파일을 실행해줘" → 파일이 존재하므로 연결됨
- 대화 컨텍스트가 아닌 **파일 상태**로 턴을 연결하는 방식은 정상 작동

```bash
# 파일 기반 멀티턴 예시 (TC-03: 버그 수정 후 실행 확인)
# T1: dhelix가 파일 읽고 수정
node /c/Users/DBInc/dhelix/dist/index.js \
...
`NEXUS.md`는 dhelix의 공식 파일이 **아니다**. QA 에이전트가 **테스트 픽스처**로 생성하는 임의 파일 (이름은 무관). 헤드리스 호출 간 "공유 메모리" 역할을 파일시스템으로 구현하는 패턴.
...
[T1] dhelix → "NEXUS.md 읽고 테스트 프레임워크가 뭔지 알려줘"
       └─ 파일에서 Vitest 읽음 → 정확히 응답

[T2] dhelix → "NEXUS.md 읽고 DB가 뭔지 알려줘"
...
cat > /tmp/dhelix-qa-tc25/NEXUS.md << 'EOF'
...
`~/.dhelix/sessions/index.json`은 **배열(array)** — dict/object가 아님.

```bash
# Turn 1 전: 기존 세션 ID 수집
SESSIONS_BEFORE=$(node -e "
const data = JSON.parse(require('fs').readFileSync(
  process.env.USERPROFILE + '/.dhelix/sessions/index.json', 'utf8'));
console.log(data.map(s => s.id).join(' '));
" 2>/dev/null || echo "")

# Turn 1 실행
node /c/Users/DBInc/dhelix/dist/index.js -p "프롬프트" --output-format text
...
```bash
# 각 TC마다 독립된 디렉토리 생성
mkdir -p /tmp/dhelix-qa-tc01
cp /c/Users/DBInc/dhelix/.env /tmp/dhelix-qa-tc01/.env  # API 키 필수
cd /tmp/dhelix-qa-tc01

# dhelix 실행 (절대 경로 + Azure fallback)
node /c/Users/DBInc/dhelix/dist/index.js \
